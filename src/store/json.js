 ...
 */
export async function readJsonRequired(filePath) {
    ...
    await withLock(filePath, async () => {
        ...
        if (options?.validate) {
            try {
                ...
                throw new CleoError(ExitCode.VALIDATION_ERROR, `Validation failed before write: ${filePath}`, { cause: err });
            }
        }
        ...
        import('../core/hooks/registry.js')
            .then(({ hooks: h }) => h.dispatch('onFileChange', process.cwd(), {
        ...
        if (jsonEnd > 0) {
            ...
            try {
                ...
                if (initialJson && Array.isArray(initialJson.entries)) {
                    entries.push(...initialJson.entries);
                }
            }
            catch {
