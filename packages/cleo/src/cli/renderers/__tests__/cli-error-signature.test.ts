...
// T10131/B6 + T10132/B7 — the renderer dispatcher now imports many symbols
// from `@cleocode/core` (renderStart, renderBriefing, renderNexus*, etc. —
// migrated from the deleted `./system.js` and `./nexus.js`). Use
// importOriginal to preserve the real exports and only override
// `formatSuccess` so the cliError envelope path stays stubbed. These tests
// never invoke the migrated renderers — they just need the symbols to
// resolve so the dispatcher module loads.
