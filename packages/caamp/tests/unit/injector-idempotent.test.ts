...
describe('ensureAllProviderInstructionFiles()', () => {
  it('deduplicates providers sharing the same instruction file', async () => {
    ...
    const results = await ensureAllProviderInstructionFiles(['cursor', 'windsurf'], testDir, {
      delivery: 'verified-references',
      references: ['@AGENTS.md'],
    });
  ...
  it('throws for unknown provider in the list', async () => {
    await expect(
      ensureAllProviderInstructionFiles(['claude-code', 'fake-provider'], testDir, {
        delivery: 'verified-references',
        references: ['@AGENTS.md'],
      }),
...
describe('T12269 real instruction file byte boundary', () => {
  it('preserves exact user bytes through update and repeated injection', async () => {
    const target = join(testDir, 'BYTE-PRESERVATION.md');
    const prefix = Buffer.from('\uFEFF# 用户\r\n\r\n\r\n  ');
    const suffix = Buffer.from(' \t\r\n\r\nFooter λ  \t');
    const old = Buffer.from('<!-- CAAMP:START -->\r\nold\r\n<!-- CAAMP:END -->');
    await writeFile(target, Buffer.concat([prefix, old, suffix]));
    expect(await inject(target, 'new')).toBe('updated');
    const expected = Buffer.concat([
      prefix,
      Buffer.from('<!-- CAAMP:START -->\nnew\n<!-- CAAMP:END -->'),
      suffix,
    ]);
    expect(await readFile(target)).toEqual(expected);
    expect(await inject(target, 'new')).toBe('intact');
    expect(await readFile(target)).toEqual(expected);
  });
});
