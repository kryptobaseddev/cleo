...
if [[ -f "$_SM_LIB_DIR/paths.sh" ]]; then
    source "$_SM_LIB_DIR/paths.sh"
fi

# Source file-ops for atomic writes
if [[ -f "$_SM_LIB_DIR/file-ops.sh" ]]; then
    source "$_SM_LIB_DIR/file-ops.sh"
fi

# Source config for settings access
if [[ -f "$_SM_LIB_DIR/config.sh" ]]; then
