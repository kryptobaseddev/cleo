...
const { isCaampOwnedSkill, shouldOverrideSkill, checkSkillIntegrity, checkAllSkillIntegrity } = await import("../../src/core/skills/integrity.js");
let testDir;
beforeEach(async () => {
...
describe("checkSkillIntegrity()", () => {
    ...
    it("handles skills with multiple providers", async () => {
        ...
        const result = await checkSkillIntegrity(skillName, [
            { id: "claude-code", instructFile: "CLAUDE.md", pathGlobal: testDir },
            { id: "cursor", instructFile: "AGENTS.md", pathGlobal: testDir },
        ]);
        expect(result.status).toBe("intact");
...
describe("validateInstructionIntegrity()", () => {
    ...
    it("reports no issues for current files", async () => {
        const { validateInstructionIntegrity } = await import("../../src/core/skills/integrity.js");
        await writeFile(join(testDir, "GOOD.md"), "<!-- CAAMP:START -->\n@AGENTS.md\n<!-- CAAMP:END -->\n");
        const providers = [
            {
                id: "test-provider",
    ...
    it("reports outdated files when expected content differs", async () => {
        const { validateInstructionIntegrity } = await import("../../src/core/skills/integrity.js");
        await writeFile(join(testDir, "OLD.md"), "<!-- CAAMP:START -->\nold content\n<!-- CAAMP:END -->\n");
        const providers = [
            {
                id: "test-provider",
                instructFile: "OLD.md",
        ...
        const issues = await validateInstructionIntegrity(providers, testDir, "project", "new content");
        expect(issues.length).toBe(1);
        expect(issues[0]?.issue).toContain("outdated");
    });
});
//# sourceMappingURL=skill-integrity.test.js.map