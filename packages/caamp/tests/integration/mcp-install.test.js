...
describe("integration: mcp install command", () => {
    beforeEach(() => {
        ...
        mocks.mapCompatibilityInstallOptions.mockImplementation((value) => value);
    ...
    it("installs MCP config and records lock for successful providers", async () => {
        ...
        expect(mocks.recordMcpInstall).toHaveBeenCalledWith("acme", "@acme/mcp", "package", ["claude-code"], false);
    });
    ...
    it("routes to CLEO compatibility install when source is managed profile", async () => {
        ...
        expect(mocks.executeCleoInstall).toHaveBeenCalledWith("install", expect.anything(), "mcp.install");
