...
import { assertNoCycle, assertParentExists, countActiveChildren, ENFORCEMENT_PROFILES, resolveHierarchyPolicy, validateHierarchyPlacement, } from '../hierarchy-policy.js';
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
describe('validateHierarchyPlacement — unlimited siblings', () => {
    it('maxSiblings=0 allows unlimited children', () => {
        ...
        const children = Array.from({ length: 20 }, (_, i) => makeTask({ id: `T${String(i + 2).padStart(3, '0')}`, parentId: 'T001' }));
    ...
    it('maxSiblings=7 fails when parent already has 7 children', () => {
        const parent = makeTask({ id: 'T001', type: 'epic' });
        const children = Array.from({ length: 7 }, (_, i) => makeTask({ id: `T${String(i + 2).padStart(3, '0')}`, parentId: 'T001' }));
        const tasks = [parent, ...children];
    ...
    it('maxSiblings=7 succeeds when parent has fewer than 7 children', () => {
        ...
        const children = Array.from({ length: 2 }, (_, i) => makeTask({ id: `T${String(i + 2).padStart(3, '0')}`, parentId: 'T001' }));
        const tasks = [parent, ...children];
...
describe('validateHierarchyPlacement — maxActiveSiblings', () => {
    it('fails when active children equal maxActiveSiblings', () => {
        const parent = makeTask({ id: 'T001', type: 'epic' });
        const children = Array.from({ length: 32 }, (_, i) => makeTask({ id: `T${String(i + 2).padStart(3, '0')}`, parentId: 'T001', status: 'active' }));
        const tasks = [parent, ...children];
    ...
    it('succeeds when active children are one less than maxActiveSiblings', () => {
        const parent = makeTask({ id: 'T001', type: 'epic' });
        const children = Array.from({ length: 31 }, (_, i) => makeTask({ id: `T${String(i + 2).padStart(3, '0')}`, parentId: 'T001', status: 'active' }));
        const tasks = [parent, ...children];
        const policy = llmPolicy(); // maxActiveSiblings=32
        const result = validateHierarchyPlacement('T001', tasks, policy);
        expect(result.valid).toBe(true);
    ...
    it('maxActiveSiblings=0 disables active sibling check', () => {
        const parent = makeTask({ id: 'T001', type: 'epic' });
        const children = Array.from({ length: 50 }, (_, i) => makeTask({ id: `T${String(i + 2).padStart(3, '0')}`, parentId: 'T001', status: 'active' }));
        const tasks = [parent, ...children];
        const policy = {
            ...llmPolicy(),
            maxActiveSiblings: 0,
...
describe('validateHierarchyPlacement — countDoneInLimit', () => {
    it('done children do not count when countDoneInLimit=false', () => {
        const parent = makeTask({ id: 'T001', type: 'epic' });
        const doneChildren = Array.from({ length: 7 }, (_, i) => makeTask({ id: `T${String(i + 2).padStart(3, '0')}`, parentId: 'T001', status: 'done' }));
...
//# sourceMappingURL=hierarchy-policy.test.js.map