...
import { clearEmbeddingProvider, EMBEDDING_DIMENSIONS, embedText, getEmbeddingProvider, isEmbeddingAvailable, setEmbeddingProvider, } from '../brain-embedding.js';
/** Create a mock provider that returns a deterministic vector. */
function createMockProvider(overrides) {
    return {
        dimensions: EMBEDDING_DIMENSIONS,
        isAvailable: () => true,
        embed: vi.fn(async (_text) => new Float32Array(EMBEDDING_DIMENSIONS).fill(0.1)),
        ...overrides,
    };
}
describe('brain-embedding', () => {
    ...
    describe('dimension validation', () => {
        it('rejects provider with wrong dimensions', () => {
            ...
            expect(() => setEmbeddingProvider(badProvider)).toThrow(/dimensions \(768\) do not match vec0 table \(384\)/);
        });
    });
...
//# sourceMappingURL=brain-embedding.test.js.map