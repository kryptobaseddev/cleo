...
| **Agent-Per-Task Locking** | One agent per active task, session-bound locking |
...
| Active | Suspended | `session suspend` | Active tasks updated with progress |
...
→ ERROR (E_SESSION_EXISTS):
  ...
  Agent: opus-1 | Started: 2h ago | Active: T005
...
### 3.1 Active Task Within Sessions

Active task tracking is session-scoped, not global:
...
| Rule | Behavior |
|------|----------|
| One agent per task | If T005 active for agent A, agent B cannot start T005 |
| Session-bound | Lock releases when session suspended/ended |
| Blocked task check | Cannot start task blocked by dependencies |
| Scope validation | Cannot start task outside session scope |

### 3.3 Task Start Outcomes

```bash
# Success
cleo start T005
→ [SESSION session_20251228_...] Started T005: "Implement OAuth"

# Already active for another agent
cleo start T005
...
→ ERROR (E_TASK_BLOCKED):
  ...
  Complete these tasks first, or start an unblocked task.
...
### 3.4 Parallel Active Tasks (Multiple Agents)

Multiple agents can work on the same Epic session, each working on different tasks:

```
Session: session_20251228_175354 (Epic: T001)
├── Agent opus-1:   Active: T005
├── Agent sonnet-3: Active: T006
└── Agent haiku-1:  Active: T007
```

Rules:
- Each agent MUST have a unique active task
- No two agents can work on the same task
...
| `complete` | YES | Completion requires task lock |
| `start` | YES | Active task is session-scoped |
...
[INFO] Active session: session_20251228_175354 (Epic: T001)
       Active: T005 (you) | Other agents: sonnet-3 on T006
...
### 8.2 Active Task Commands (Session-Aware)

```bash
# Start working on task (within current session)
cleo start T005
...
# Add progress note to active task
...
| `warnOnNoFocus` | `true` | Warn if session active without a task started |
...
| This Spec | ✅ Draft | [EPIC-SESSION](./EPIC-SESSION) (MDX) / `docs/archive/specs-md/EPIC-SESSION-SPEC.md` (archived) | Vision document |
...
| Task Lock | Global in todo.json | Move to sessions.json per-session |
...
# 1. Start session
cleo session start --epic T001 --auto-focus
→ Session started, started T005 (highest priority)
...
→ Joined session_abc, select task to start

[sonnet-3]$ cleo start T006
→ Started T006 (T005 is with opus-1)
...
To start an available task: cleo start <task-id>
```
