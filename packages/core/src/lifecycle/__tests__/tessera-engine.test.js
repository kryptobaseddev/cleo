...
function buildTypedTemplate() {
    return {
        ...
        shape: {
            stages: [
                { id: 's1', name: 'Stage 1', category: 'research', skippable: false },
                { id: 's2', name: 'Stage 2', category: 'implementation', skippable: false },
            ],
            links: [{ from: 's1', to: 's2', type: 'linear' }],
            entryPoint: 's1',
            exitPoints: ['s2'],
        },
        gates: [],
...
describe('Tessera engine', () => {
    ...
    it('missing required variable (epicId) throws error', async () => {
        const { buildDefaultTessera, instantiateTessera } = await import('../tessera-engine.js');
        const template = buildDefaultTessera();
        await expect(instantiateTessera(template, { templateId: template.id, epicId: 'T8888', variables: {} }, tempDir)).rejects.toThrow('Missing required variable: epicId');
    });
    ...
    it('invalid variable type throws deterministic diagnostic', async () => {
        const { buildDefaultTessera, instantiateTessera } = await import('../tessera-engine.js');
        const template = buildDefaultTessera();
        await expect(instantiateTessera(template, {
            templateId: template.id,
            epicId: 'T8888',
            variables: { epicId: 'T8888', skipResearch: 'yes' },
        }, tempDir)).rejects.toThrow('Invalid variable type for "skipResearch": expected boolean, got string');
    });
    it.each([
        {
        ...
        await expect(instantiateTessera(buildTypedTemplate(), {
            templateId: 'tessera-typed-contract',
            epicId: 'T8888',
            variables,
        }, tempDir)).rejects.toThrow(message);
    });
    it('positive path still succeeds after invalid-type assertions', async () => {
        const { instantiateTessera } = await import('../tessera-engine.js');
        const instance = await instantiateTessera(buildTypedTemplate(), {
            templateId: 'tessera-typed-contract',
            epicId: 'T8888',
            variables: {
                epicId: 'T8888',
    ...
    it('invalid epicId format throws deterministic diagnostic', async () => {
        ...
        }, tempDir)).rejects.toThrow('Invalid variable format for "epicId": expected epicId like "T1234", got "epic-1"');
    });
    it('unknown input variable throws deterministic diagnostic', async () => {
        ...
        }, tempDir)).rejects.toThrow('Unknown variable: notDeclared. Allowed variables: epicId, projectName, skipResearch');
    });
    it('treats undefined required variable as missing', async () => {
        ...
        }, tempDir)).rejects.toThrow('Missing required variable: epicId');
    });
    it('performs deep substitution in nested chain structures', async () => {
        ...
        const template = {
            ...
            shape: {
                ...base.shape,
                stages: base.shape.stages.map((stage, index) => index === 0
                    ? {
                        ...stage,
    ...
    it('throws deterministic error for unknown substitution variable', async () => {
        ...
        await expect(instantiateTessera(template, {
            ...
            epicId: 'T8888',
            variables: { epicId: 'T8888' },
        }, tempDir)).rejects.toThrow('Unknown template variable "missingVar" at chain.description');
    });
...
//# sourceMappingURL=tessera-engine.test.js.map