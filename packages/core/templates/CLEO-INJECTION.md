...
| ID-only pipeline | `--output id` | `cleo list --parent EPIC --output id --limit 0 \| while read c; do …; done` — **`--limit 0` is REQUIRED on `list`** (without it `list` stops at 10 silently while `--output count` reports the true total). **NEVER pass `--limit 0` to `cleo find`** — there it returns ZERO rows and says "No matching tasks found". |
