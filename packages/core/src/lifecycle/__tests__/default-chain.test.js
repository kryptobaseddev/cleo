...
import { buildDefaultChain, DEFAULT_CHAIN_ID, DEFAULT_PROTOCOL_STAGE_MAP, } from '../default-chain.js';
...
describe('buildDefaultChain', () => {
    ...
    it('every STAGE_PREREQUISITE is represented as an entry gate', () => {
        ...
        for (const stage of PIPELINE_STAGES) {
            ...
            for (const prereq of prereqs) {
                const match = entryGates.find((g) => g.stageId === stage && g.check.type === 'stage_complete' && g.check.stageId === prereq);
    ...
    it('every VERIFICATION_GATE_ORDER gate is represented as an exit gate', () => {
        ...
        for (const gateName of VERIFICATION_GATE_ORDER) {
            const match = exitGates.find((g) => g.check.type === 'verification_gate' && g.check.gateName === gateName);
    ...
    it('every protocol type is represented as a protocol_valid gate', () => {
        ...
        for (const protocolType of PROTOCOL_TYPES) {
            const match = protocolGates.find((g) => g.check.type === 'protocol_valid' && g.check.protocolType === protocolType);
...
//# sourceMappingURL=default-chain.test.js.map