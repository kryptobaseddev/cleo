 ...
 * The canonical source is templates/cleo-gitignore.
 * @task T4640
 * @epic T4637
 */
 ...
 * Falls back to embedded CLEO_GITIGNORE_CONTENT if file not found.
 * @task T4700
 */
export function getGitignoreTemplate(): string {
  try {
    // Resolve relative to the package root (3 levels up from src/cli/commands/)
...
async function createCleoGitignore(cleoDir: string, force: boolean): Promise<boolean> {
  ...
  await writeFile(gitignorePath, getGitignoreTemplate());
  return true;
}
  ...
  program
    ...
    .action(async (projectName: string | undefined, opts: Record<string, unknown>) => {
      try {
        ...
        // Auto-detect project configuration (--detect flag)
        if (opts['detect']) {
          try {
            const { detectProjectType } = await import('../../store/project-detect.js');
            const info = detectProjectType(projRoot);
        ...
        // NEXUS auto-registration (best-effort, never blocks init)
        try {
          const { nexusInit, nexusRegister } = await import('../../core/nexus/registry.js');
          await nexusInit();
          const pName = (opts['name'] as string) || projectName || projRoot.split('/').pop() || 'unnamed';
          await nexusRegister(projRoot, pName);
          created.push('NEXUS registration');
        } catch (err) {
          // Silently handle NEXUS_PROJECT_EXISTS (already registered)
