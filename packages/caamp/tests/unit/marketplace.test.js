...
function makeMockAdapter(name, searchResults, getSkillResult = null) {
    return {
        name,
...
describe("MarketplaceClient", () => {
    ...
    it("respects limit parameter", async () => {
        const items = Array.from({ length: 10 }, (_, i) => makeResult({ scopedName: `@a/s${i}`, stars: 10 - i }));
...
describe("SkillsMPAdapter", () => {
    ...
    it("getSkill returns null when no match", async () => {
        globalThis.fetch = vi.fn().mockResolvedValue({
            ok: true,
            json: () => Promise.resolve({ skills: [], total: 0, limit: 1, offset: 0 }),
        });
        const adapter = new SkillsMPAdapter();
