...
|------------------|---------------------------|-------------|
| **The Hearth** | `packages/core/src/sessions/`, `packages/core/src/orchestration/` | Active session surface and orchestration context |
| **Living BRAIN** | `packages/core/src/memory/` | brain.db observations, patterns, learnings, decisions -- all retrieval and storage |
| **The Loom (lifecycle)** | `packages/core/src/lifecycle/` | RCASD-IVTR+C lifecycle gate enforcement |
| **Threads** | `packages/core/src/tasks/` | Task CRUD, hierarchy, dependency graph, completion semantics |
| **Tapestries / Cascade** | `packages/core/src/orchestration/` | Wave planning, multi-agent coordination, orchestration status |
| **The Proving** | `packages/core/src/validation/` | Schema, semantic, referential, and state-machine validation |
| **Tome** | `packages/core/src/research/`, `packages/core/src/adrs/` | Manifest ledger and ADR management |
| **Cogs** | `packages/core/src/skills/` | Skill discovery, dispatch, installation, and marketplace |
