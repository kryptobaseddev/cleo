...
<Warning>
  **NEVER ignore exit codes.** Failed commands mean tasks were NOT
  created/updated.
</Warning>
...
|  12  | `E_SIBLING_LIMIT`    | Configured sibling limit exceeded | Move task to different parent or increase `hierarchy.maxSiblings` |
...
<Tip>
  Recoverable errors (retry with backoff): 7, 20, 21, 22 Special codes (not
  errors): 100 = no data, 101 = already exists, 102 = no change needed
</Tip>
