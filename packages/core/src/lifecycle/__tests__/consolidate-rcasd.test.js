...
import { consolidateRcasd, migrateConsensusFiles, migrateContributionFiles, migrateLooseFiles, normalizeDirectoryNames, resolveEpicFromContent, resolveEpicFromFilename, } from '../consolidate-rcasd.js';
let testDir;
let cleoDir;
beforeEach(() => {
...
describe('migrateConsensusFiles', () => {
    it('migrates consensus files to epic subdirectory', () => {
        ...
        expect(records).toHaveLength(1);
        expect(records[0].status).toBe('success');
        expect(existsSync(join(cleoDir, 'rcasd', 'T4869', 'consensus', 'T4869-checkpoint-consensus.json'))).toBe(true);
    });
...
describe('migrateContributionFiles', () => {
    it('migrates contribution files to epic subdirectory', () => {
        ...
        expect(records).toHaveLength(1);
        expect(records[0].status).toBe('success');
        expect(existsSync(join(cleoDir, 'rcasd', 'T5000', 'contributions', 'T5000-session-a.json'))).toBe(true);
    });
...
//# sourceMappingURL=consolidate-rcasd.test.js.map