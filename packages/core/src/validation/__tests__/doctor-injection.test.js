...
import { checkAtReferenceTargetExists, checkCaampMarkerIntegrity, checkTemplateFreshness, checkTierMarkersPresent, } from '../doctor/checks.js';
...
describe('checkCaampMarkerIntegrity', () => {
    ...
    it('passes when both files have balanced markers', () => {
        ...
        writeFileSync(join(tempDir, 'AGENTS.md'), '<!-- CAAMP:START -->\n@~/.cleo/templates/CLEO-INJECTION.md\n<!-- CAAMP:END -->\n');
    ...
    it('warns when CAAMP markers are unbalanced', () => {
        ...
        expect(result.details.issues).toContainEqual(expect.stringContaining('1 CAAMP:START vs 0 CAAMP:END'));
    });
    it('warns when file has no CAAMP markers at all', () => {
        writeFileSync(join(tempDir, 'AGENTS.md'), '# No markers here\n');
        const result = checkCaampMarkerIntegrity(tempDir);
        expect(result.status).toBe('warning');
        expect(result.details.issues).toContainEqual(expect.stringContaining('no CAAMP markers found'));
    });
...
describe('checkTierMarkersPresent', () => {
    ...
    it('passes when all 3 tier markers are present with close tags', () => {
        mkdirSync(join(fakeCleoHome, 'templates'), { recursive: true });
        writeFileSync(join(fakeCleoHome, 'templates', 'CLEO-INJECTION.md'), [
            '<!-- TIER:minimal -->',
            'Minimal content',
            '<!-- /TIER:minimal -->',
            '<!-- TIER:standard -->',
    ...
    it('warns when a tier marker is missing', () => {
        ...
        expect(result.details.missing).toContain('orchestrator');
    });
    it('warns when a tier marker is unclosed', () => {
        ...
        expect(result.details.unclosed).toContain('standard');
    });
});
//# sourceMappingURL=doctor-injection.test.js.map