...
describe('Write Verification', () => {
    ...
    describe('verifyTaskWrite', () => {
        ...
        it('should fail verification for non-existent task', async () => {
            ...
            await expect(verifyTaskWrite('T999', undefined, tempDir, { strictMode: true })).rejects.toThrow('not found after write');
        });
    ...
    describe('verifySessionWrite', () => {
        ...
        it('should fail verification for non-existent session', async () => {
            const { verifySessionWrite } = await import('../data-safety.js');
            await expect(verifySessionWrite('sess-nonexistent', tempDir, { strictMode: true })).rejects.toThrow('not found after write');
        });
    });
    describe('safeCreateTask', () => {
        it('should create task with full safety pipeline', async () => {
            ...
            const result = await safeCreateTask(() => createTask(taskData), taskData, tempDir, {
        ...
        it('should detect collision during safe create', async () => {
            ...
            await expect(safeCreateTask(() => createTask({ ...taskData, title: 'Duplicate' }), taskData, tempDir, { autoCheckpoint: false, validateSequence: false })).rejects.toThrow('collision');
        });
    });
    describe('safeUpdateTask', () => {
        it('should update task with write verification', async () => {
            ...
            const result = await safeUpdateTask(() => updateTask('T001', { title: 'Updated' }), 'T001', { title: 'Updated' }, tempDir, { autoCheckpoint: false });
...
//# sourceMappingURL=write-verification.test.js.map