...
describe("integration: mcp command wrappers", () => {
    ...
    it("detects provider MCP state and outputs json", async () => {
        ...
        mocks.resolveConfigPath.mockImplementation((provider, scope) => {
            if (scope === "global")
                return `/tmp/${provider.id}.json`;
            return `/repo/.${provider.id}.json`;
        });
        mocks.existsSync.mockImplementation((path) => path.startsWith("/tmp/"));
        mocks.listMcpServers.mockImplementation((provider, scope) => {
            if (scope === "global") {
...
//# sourceMappingURL=mcp-command-wrappers.test.js.map