...
function createMockStore() {
    ...
    return {
        ...
        accessor: {
            ...
            upsertSingleSession: vi.fn().mockImplementation((session) => {
...
describe('Session handoff full round-trip', () => {
    ...
    beforeEach(() => {
        ...
        getAccessor.mockResolvedValue(store.accessor);
    });
    it('full round-trip: start → end → compute handoff → persist → retrieve → successor start', async () => {
        ...
        const lastByRoot = await getLastHandoff('/test', { type: 'epic', rootTaskId: 'T100' });
...
//# sourceMappingURL=handoff-integration.test.js.map