...
import { calculateHealthStatus, checkCliInstallation, checkCliVersion, checkDocsAccessibility, } from '../doctor/checks.js';
import { CACHE_VERSION, clearEntireCache, initCacheFile, loadCache, } from '../doctor/project-cache.js';
import { categorizeProjects, formatProjectHealthSummary, getJourneyGuidance, getProjectCategoryName, getProjectGuidance, getUserJourneyStage, isTempProject, } from '../doctor/utils.js';
// ============================================================================
// Doctor Utils
// ============================================================================
...
describe('categorizeProjects', () => {
    it('separates active, temp, and orphaned', () => {
        const projects = [
            { name: 'active', path: '/home/user/project', status: 'healthy', isTemp: false },
            { name: 'temp', path: '/tmp/test', status: 'healthy', isTemp: true },
            {
                name: 'orphaned',
                path: '/missing/path',
                status: 'orphaned',
...
describe('calculateHealthStatus', () => {
    it('returns 0 for all passed', () => {
        const checks = [
            { id: 'a', category: 'c', status: 'passed', message: '', details: {}, fix: null },
        ];
        expect(calculateHealthStatus(checks)).toBe(0);
    });
    it('returns 50 for warnings', () => {
        const checks = [
            { id: 'a', category: 'c', status: 'warning', message: '', details: {}, fix: null },
        ];
        expect(calculateHealthStatus(checks)).toBe(50);
    });
    it('returns 52 for failures', () => {
        const checks = [
            { id: 'a', category: 'c', status: 'failed', message: '', details: {}, fix: null },
        ];
