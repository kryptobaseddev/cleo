/**
 * Lifecycle Engine Operations — business logic layer.
 *
 * Contains all lifecycle domain logic migrated from
 * `packages/cleo/src/dispatch/engines/lifecycle-engine.ts` (ENG-MIG-9 / T1576).
 *
 * Sub-domains:
 *   lifecycle.*       - RCASD-IVTR+C pipeline stage management
 *   lifecycle.gate.*  - Stage gate pass/fail recording
 *   lifecycle.scope   - Subagent scope enforcement (T1162 / ADR-054)
 *
...
// ============================================================================
// Lifecycle Scope Guard (T1162 / ADR-054)
//
 ...
 * @task T1162
 */
export async function enforceScopeForLifecycleMutation(
  epicId: string,
  projectRoot?: string,
...
// ============================================================================
// Exported engine functions (EngineResult wrappers)
// ============================================================================
...
/**
 * lifecycle.check / lifecycle.status — Get lifecycle status for an epic.
 *
 * @param epicId      - Epic task ID to query.
 * @param projectRoot - Absolute project root path.
 *
 * @task T4785
 */
 ...
 *
 * @param taskId      - Task or epic ID to query.
 * @param projectRoot - Absolute project root path.
 *
 * @task T4785
 */
 ...
 *
 * @param targetStage - The pipeline stage to query prerequisites for.
 * @param _projectRoot - Unused (pure data lookup, no I/O).
 *
 * @task T4785
 */
export async function lifecyclePrerequisites(
  ...
  try {
    const data = getStagePrerequisites(targetStage);
 ...
 *
 * @param epicId      - The epic to validate stage prerequisites against.
 * @param targetStage - The target stage to check prerequisites for.
 * @param projectRoot - Absolute project root path.
 *
 * @task T4785
 */
 ...
 *
 * Enforces forward-only stage progression and lifecycle gate checks.
 * Applies the T1162 scope guard to prevent subagents from bypassing gates.
 *
 * @param taskId      - Epic or task ID to update.
 * @param stage       - Stage name (accepts aliases resolved via resolveStageAlias).
 * @param status      - New status for the stage.
 * @param notes       - Optional notes for the stage transition.
 * @param projectRoot - Absolute project root path.
 *
 * @task T4785
 ...
 *
 * Applies the T1162 scope guard to prevent subagents from skipping a parent
 * epic's lifecycle stages.
 *
 * @param taskId      - Epic or task ID to update.
 * @param stage       - Stage name to skip.
 ...
 *
 * Applies the T1162 scope guard to prevent subagents from resetting a parent
 * epic's lifecycle stages.
 *
 * @param taskId      - Epic or task ID to reset.
 * @param stage       - Stage name to reset.
 ...
 *
 * @param taskId      - Epic or task ID.
 * @param gateName    - Name of the gate to pass.
 * @param agent       - Agent that performed the gate check (defaults to 'system').
 * @param notes       - Optional notes for the gate pass.
 * @param projectRoot - Absolute project root path.
 *
 * @task T4785
 */
 ...
 *
 * @param taskId      - Epic or task ID.
 * @param gateName    - Name of the gate to fail.
 * @param reason      - Reason the gate failed.
 * @param projectRoot - Absolute project root path.
 *
 * @task T4785
 */
