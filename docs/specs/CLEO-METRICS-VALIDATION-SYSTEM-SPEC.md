...
|---------|---------|---------------|
| `packages/core/src/metrics/otel-integration.ts` | Capture actual Claude Code tokens | `getSessionTokens`, `compareSessions` |
| `packages/core/src/metrics/token-estimation.ts` | Fallback estimation when OTel unavailable | `estimateTokens`, `trackFileRead` |
| `packages/core/src/compliance/index.ts` | Real manifest entry validation | `validateAndLog`, `validateManifestEntry` |
| `packages/core/src/compliance/store.ts` | Metrics storage and retrieval | `logComplianceMetric`, `getComplianceMetrics` |
| `packages/core/src/skills/dispatch.ts` | Skill selection and composition | `prepareSkillSpawns` |
...
- `packages/core/src/metrics/otel-integration.ts` - OpenTelemetry integration
- `packages/core/src/metrics/token-estimation.ts` - Token estimation fallback
- `packages/core/src/compliance/index.ts` - Real manifest validation
- `packages/core/src/skills/dispatch.ts` - Multi-skill composition
