...
describe("core: mcp cleo", () => {
  ...
  describe("normalizeCleoChannel", () => {
    ...
    it("throws for invalid channel value", () => {
      expect(() => normalizeCleoChannel("nightly")).toThrow(
        'Invalid channel "nightly". Expected stable, beta, or dev.',
      );
    });

    it("throws for random string", () => {
      expect(() => normalizeCleoChannel("foobar")).toThrow(
        'Invalid channel "foobar"',
      );
    });
  });
  ...
  describe("parseEnvAssignments", () => {
    ...
    it("throws for value with no = sign", () => {
      expect(() => parseEnvAssignments(["INVALID"])).toThrow(
        'Invalid --env value "INVALID". Use KEY=value.',
      );
    });

    it("throws for value starting with = (empty key)", () => {
      expect(() => parseEnvAssignments(["=value"])).toThrow(
        'Invalid --env value "=value". Use KEY=value.',
      );
    });
