...
describe('brain.db PageIndex graph tables', () => {
  ...
  it('inserts and queries page nodes', async () => {
    ...
    nativeDb!.prepare(
      'INSERT INTO brain_page_nodes (id, node_type, label) VALUES (?, ?, ?)',
    ).run('doc:BRAIN-SPEC', 'doc', 'CLEO BRAIN Specification');

    const nodes = nativeDb!.prepare(
      'SELECT * FROM brain_page_nodes ORDER BY id',
    ).all() as Array<{ id: string; node_type: string; label: string; metadata_json: string | null }>;
  ...
  it('inserts and queries page edges', async () => {
    ...
    const edges = nativeDb!.prepare(
      'SELECT * FROM brain_page_edges WHERE from_id = ?',
    ).all('task:T5157') as Array<{ from_id: string; to_id: string; edge_type: string; weight: number }>;
  ...
  it('enforces composite primary key on edges', async () => {
    ...
    // Insert first edge
    nativeDb!.prepare(
      'INSERT INTO brain_page_edges (from_id, to_id, edge_type) VALUES (?, ?, ?)',
    ).run('A', 'B', 'relates_to');
  ...
  it('default weight is 1.0 when not specified', async () => {
    ...
    const edge = nativeDb!.prepare(
      'SELECT weight FROM brain_page_edges WHERE from_id = ? AND to_id = ?',
