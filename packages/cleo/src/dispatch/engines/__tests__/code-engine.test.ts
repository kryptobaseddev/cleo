...
describe('Code Engine', () => {
  ...
  describe('codeOutline', () => {
    it('should return success=true with file parameter', async () => {
      const result = await codeOutline({ file: testFile });
    ...
    it('should include export information in symbols', async () => {
      const result = await codeOutline({ file: testFile });
      const data = result.data as any;
      const codeCommandExport = data.symbols.find((s: any) => s.name === 'codeCommand');
    ...
    it('should return error when file parameter is missing', async () => {
      const result = await codeOutline({});
    ...
    it('should handle absolute paths correctly', async () => {
      const result = await codeOutline({ file: absPath });
      expect(result.success).toBe(true);
      const data = result.data as any;
      expect(data.filePath).toBeDefined();
    });
  ...
  describe('codeSearch', () => {
    it('should return success=true with query parameter', async () => {
      const result = await codeSearch({ query: 'codeCommand' });
      expect(result.success).toBe(true);
    ...
    it('should find known symbols by exact query', async () => {
      ...
      const found = results.some((r: any) => r.symbol.name === 'codeCommand');
      expect(found).toBe(true);
    });

    it('should include match type and score in results', async () => {
      const result = await codeSearch({ query: 'requireTreeSitter' });
      const results = result.data as any[];
      if (results.length > 0) {
        const r = results[0];
    ...
    it('should respect maxResults parameter', async () => {
      const result = await codeSearch({ query: 'export', max: 5 });
      const results = result.data as any[];
      expect(results.length).toBeLessThanOrEqual(5);
    });

    it('should default to 20 results when max not specified', async () => {
      const result = await codeSearch({ query: 'function' });
      const results = result.data as any[];
      expect(results.length).toBeLessThanOrEqual(20);
    });

    it('should filter by language when specified', async () => {
      const result = await codeSearch({ query: 'const', lang: 'typescript' });
      expect(result.success).toBe(true);
      expect(Array.isArray(result.data)).toBe(true);
    });
    ...
    it('should return empty array for non-matching query', async () => {
      const result = await codeSearch({ query: 'xyzABCDEF123999notfound' });
      expect(result.success).toBe(true);
  ...
  describe('codeUnfold', () => {
    it('should return success=true with file and symbol parameters', async () => {
      const result = await codeUnfold({ file: testFile, symbol: 'codeCommand' });
      expect(result.success).toBe(true);
      expect(result.data).toBeDefined();
    });
    ...
    it('should find nested symbols', async () => {
      ...
      const result = await codeUnfold({ file: testFile, symbol: 'outline' });
      expect(result.success).toBe(true);
    });

    it('should return found=false for non-existent symbol', async () => {
      const result = await codeUnfold({ file: testFile, symbol: 'nonExistentSymbol123' });
      expect(result.success).toBe(true);
      const data = result.data as any;
    ...
    it('should return error when file parameter is missing', async () => {
      const result = await codeUnfold({ symbol: 'codeCommand' });
      expect(result.success).toBe(false);
      expect(result.error?.code).toBe('E_INVALID_INPUT');
    ...
    it('should return error when symbol parameter is missing', async () => {
      const result = await codeUnfold({ file: testFile });
      expect(result.success).toBe(false);
      expect(result.error?.code).toBe('E_INVALID_INPUT');
    ...
    it('should handle absolute paths correctly', async () => {
      const result = await codeUnfold({ file: absPath, symbol: 'codeCommand' });
      expect(result.success).toBe(true);
    });
  });
  ...
  describe('codeParse', () => {
    it('should return raw AST parse result', async () => {
      const result = await codeParse({ file: testFile });
      expect(result.success).toBe(true);
      expect(result.data).toBeDefined();
    });
