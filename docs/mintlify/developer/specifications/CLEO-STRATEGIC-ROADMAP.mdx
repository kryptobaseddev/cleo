...
<Note>
  **Canonical source**: `docs/specs/CLEO-STRATEGIC-ROADMAP-SPEC.md` contains the
  full specification with complete RFC 2119 requirements, detailed success
  criteria, risk assessment, and rollback procedures. This page is an
  abbreviated summary for the documentation site.
</Note>
...
| **To-Do Comments**       | 1,111 → <100 | Unchanged                                          |
...
Two-tool CQRS gateway: `query` (45 read operations) and `mutate` (53 write operations), routing through 8 domains via CLI adapter to the Bash CLI.
...
- **T2971**: Research: BRAIN Vision Requirements _(archived — pre-SQLite migration)_
