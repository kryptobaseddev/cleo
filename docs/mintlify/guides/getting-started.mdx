...
description: "Comprehensive guide to setting up CLEO task management for AI coding agents"
---
...
```bash
cleo start <id>                # Set active task (one at a time)
cleo current                    # Show current focus
cleo stop                   # Clear focus
```
