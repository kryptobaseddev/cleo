#!/usr/bin/env bash
# lib/task-mutate.sh - Centralized task mutation functions
#
# LAYER: 2 (Data Layer)
...
if [[ -f "$_TM_LIB_DIR/file-ops.sh" ]]; then
    # shellcheck source=lib/file-ops.sh
    source "$_TM_LIB_DIR/file-ops.sh"
fi

# Source logging library for consistent logging
if [[ -f "$_TM_LIB_DIR/logging.sh" ]]; then
    # shellcheck source=lib/logging.sh
    source "$_TM_LIB_DIR/logging.sh"
fi
