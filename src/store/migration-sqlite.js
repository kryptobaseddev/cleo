...
import { createBatchCallback, createDrizzleCallback, openNativeDatabase, } from './node-sqlite-adapter.js';
...
export function countJsonRecords(cleoDir) {
    let tasks = 0;
    let archived = 0;
    let sessions = 0;
 ...
 */
export async function migrateJsonToSqliteAtomic(cwd, tempDbPath, logger) {
...
async function runMigrationDataImport(db, cleoDir, result, logger) {
    ...
    if (existsSync(todoPath)) {
        try {
            ...
            const tasks = topoSortTasks(todoData.tasks ?? []);
...
export async function migrateJsonToSqlite(cwd, options) {
    const cleoDir = getCleoDirAbsolute(cwd);
    const result = {
        success: false,
        tasksImported: 0,
    ...
    if (dbExists(cwd)) {
        ...
        if (options?.dryRun) {
            const countsMatch = existingCounts.tasks === jsonCounts.tasks &&
                existingCounts.archived === jsonCounts.archived &&
                existingCounts.sessions === jsonCounts.sessions;
            if (countsMatch) {
                result.warnings.push('Dry-run: Database already contains migrated data. No changes needed.');
            }
            else {
                const diffs = [];
                ...
                result.warnings.push(`Dry-run: Data mismatch detected - ${diffs.join('; ')}. Would import ${jsonCounts.tasks - existingCounts.tasks} tasks, ${jsonCounts.archived - existingCounts.archived} archived, ${jsonCounts.sessions - existingCounts.sessions} sessions.`);
            }
            result.success = true;
            return result;
        }
        ...
        if (!options?.force) {
            ...
            result.warnings.push(`Data mismatch detected: DB has ${existingCounts.tasks} tasks, ${existingCounts.archived} archived, ${existingCounts.sessions} sessions; JSON has ${jsonCounts.tasks} tasks, ${jsonCounts.archived} archived, ${jsonCounts.sessions} sessions. Use --force to re-import.`);
            result.success = true;
            return result;
        }
    ...
    if (options?.dryRun) {
        result.warnings.push(`Dry-run: Would import ${jsonCounts.tasks} tasks, ${jsonCounts.archived} archived tasks, ${jsonCounts.sessions} sessions.`);
        result.success = true;
        return result;
    }
    const db = await getDb(cwd);
        ...
        try {
            ...
            for (const session of sessions) {
                try {
                    ...
                    const validStatuses = ['active', 'ended', 'orphaned', 'suspended'];
                    const normalizedStatus = (validStatuses.includes(session.status) ? session.status : 'ended'); // 'archived' and any other legacy statuses -> 'ended'
                    // Provide default name for sessions with null/undefined names
                    const normalizedName = session.name || `session-${session.id}`;
 ...
 */
export async function exportToJson(cwd) {
...
//# sourceMappingURL=migration-sqlite.js.map