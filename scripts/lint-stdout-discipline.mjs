...
// Versioned identity baseline; updating remains an explicit maintenance operation.
if (UPDATE_BASELINE) {
...
if (!existsSync(BASELINE_PATH)) {
  console.error(`lint-stdout-discipline: ERROR — missing baseline at ${BASELINE_PATH}`);
  process.exit(1);
}
let baseline;
let comparison;
try {
  ...
  console.error(`lint-stdout-discipline: ERROR — ${error.message}`);
  process.exit(1);
}
const newViolations = comparison.added;
if (newViolations.length === 0) {
  console.info(
    `lint-stdout-discipline: OK — ${totalViolations} violation(s) (baseline: ${baseline.total}); ${comparison.removed} prior violation(s) removed`,
  );
  process.exit(0);
}
