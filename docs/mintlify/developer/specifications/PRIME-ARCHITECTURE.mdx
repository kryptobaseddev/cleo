...
description: "PRIME orchestration architecture for multi-agent coordination"
icon: "crown"
---
...
| **Direct Task Execution** | No `cleo start` by PRIME | Session Agents own tasks |
...
├─ Session Agent State (25%)      # Status, progress, active task
...
| **Progress Reporting** | Updates `progress note`, task status | PRIME visibility |
...
```bash
# Active task updates (PRIME polls sessions.json)
cleo start T998.1.2
...
while orchestration_active; do
    ...
    # Detect active task changes, completions, suspensions
...
cleo start T998.1.2              # Updates focus.json
...
| **Durability** | Append-only `todo-log.jsonl` | Audit trail survives failures |
...
**Task Locking:**
```
Session Agent 0: start T998.1.2
Session Agent 1: start T998.2.1

✓ ALLOWED: Different tasks (disjoint scopes)
✗ BLOCKED: Agent 1 tries start T998.1.3 (in Agent 0's scope)
```

**State Synchronization:**
```bash
# PRIME polls sessions.json every 5 seconds
# Session Agents write to sessions.json on active task/status change
...
- [ ] Session Agent uses `start` for active task
- [ ] Session Agent spawns Subagents for atomic tasks
- [ ] Session Agent reports progress via `progress note`
...
│  SESSION AGENTS                                                 │
│  ┌────────┬──────────┬─────────────────────┬──────────────────┐│
│  │ Agent  │ Status   │ Active Task         │ Progress         ││
│  ├────────┼──────────┼─────────────────────┼──────────────────┤│
...
|------------|--------|-------------|
| PRIME cannot start tasks | No scope binding | CLI rejects `start` from PRIME |
...
- [AGENTS.md](../../../../AGENTS.md) - Project agent instructions with CLEO context
- [CLEO-INJECTION.md](.cleo/templates/CLEO-INJECTION.md) - Global CLEO injection template (referenced via `@~/.cleo/templates/CLEO-INJECTION.md`)
