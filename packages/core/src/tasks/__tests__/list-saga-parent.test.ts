 ...
 *
 * When `--parent` targets a Saga (`type='saga'`), children are resolved via
...
import { LIST_BINDING_SAGA_GROUPS, listTasks } from '../list.js';
...
describe('listTasks — saga-aware --parent routing (T9658)', () => {
  ...
  describe('saga path', () => {
    it('resolves children for a Saga via parentId containment', async () => {
    ...
    it('ignores non-parent relations when resolving Saga members', async () => {
      await seedTasks(accessor, [
        {
          id: 'T9560',
          title: 'SG-X',
