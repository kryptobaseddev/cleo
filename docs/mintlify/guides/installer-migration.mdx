---
title: "Migrating to the New CLEO Installer"
description: "Guide for upgrading from the legacy install.sh to the modular installer"
---
...
cp ~/.cleo/todo-log.jsonl ~/cleo-backup/ 2>/dev/null
...
**No.** Task data in `todo.json`, `todo-archive.json`, and `todo-log.jsonl` is never modified by the installer. The migration only updates the CLI scripts and libraries.
