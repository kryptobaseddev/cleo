...
beforeAll(() => {
  ...
  testDb.exec(`
    ...
    CREATE TABLE IF NOT EXISTS nexus_relations (
      ...
      reason TEXT,
      FOREIGN KEY (source_node_id) REFERENCES nexus_nodes(id),
      FOREIGN KEY (target_node_id) REFERENCES nexus_nodes(id)
    );
  `);
  ...
  testDb.exec(`
    ...
    INSERT INTO nexus_relations (id, source_node_id, target_node_id, relation_type, confidence)
    VALUES
...
describe('compileCteAlias', () => {
  it('should compile callers-of alias', () => {
    const result = compileCteAlias('callers-of');
    expect(result.description).toContain('callers');
  ...
  it('should compile callees-of alias', () => {
  ...
  it('should compile community-members alias', () => {
...
describe('runNexusCte', () => {
  it('should execute raw CTE and return results', () => {
    const cte = 'SELECT id, label, kind FROM nexus_nodes';
    const result = runNexusCte(cte, []);
  ...
  it('should return empty results for query with no matches', () => {
  ...
  it('should handle parameter binding', () => {
    const cte = 'SELECT * FROM nexus_nodes WHERE id = ?';
    const result = runNexusCte(cte, ['fn-a']);
  ...
  it('should handle empty CTE', () => {
    const result = runNexusCte('', []);
    expect(result.success).toBe(false);
...
describe('Template CTE execution', () => {
  it('callers-of should find functions that call target', () => {
    const template = compileCteAlias('callers-of');
    const result = runNexusCte(template.cte, ['fn-c']);
  ...
  it('callees-of should find functions called by target', () => {
    const template = compileCteAlias('callees-of');
    const result = runNexusCte(template.cte, ['fn-a']);
  ...
  it('co-changed should find symbols in same file', () => {
  ...
  it('community-members should find symbols in same community', () => {
    const template = compileCteAlias('community-members');
    const result = runNexusCte(template.cte, ['community-1']);
...
describe('formatCteResultAsMarkdown', () => {
  it('should format successful results as markdown table', () => {
    const result = runNexusCte('SELECT id, label, kind FROM nexus_nodes LIMIT 2', []);
  ...
  it('should handle empty results', () => {
    const result = runNexusCte("SELECT * FROM nexus_nodes WHERE id = 'invalid'", []);
    const markdown = formatCteResultAsMarkdown(result);
  ...
  it('should handle errors in results', () => {
    const result = runNexusCte('SELECT * FROM invalid_table', []);
    const markdown = formatCteResultAsMarkdown(result);
