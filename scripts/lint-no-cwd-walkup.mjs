...
const EXPLICIT_FILES =
  filesIdx >= 0 ? args.slice(filesIdx + 1).map((f) => resolve(f)) : null;
...
const SKIP_DIR_SEGMENTS = new Set([
  ...
  '__snapshots__', '__mocks__', 'coverage', '.next', '.svelte-kit', 'fixtures',
]);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!isRule1Allowed && !raw.includes(ALLOW_GET_CLEO_DIR)) {
      if (GET_CLEO_DIR_ABSOLUTE_RE.test(raw)) {
        results.push({ file: relPath, line: lineNum, rule: 'RULE-1', text: raw.trim().substring(0, 120) });
      }
    }

    if (!isRule2Allowed) {
      if (GET_PROJECT_ROOT_CWD_RE.test(raw)) {
        results.push({ file: relPath, line: lineNum, rule: 'RULE-2', text: raw.trim().substring(0, 120) });
      }
    }
  }
  return results;
...
function buildCounts() {
  ...
  return { total: violations.length, byRule, rule1Count: byRule['RULE-1'] ?? 0, rule2Count: byRule['RULE-2'] ?? 0 };
}
...
if (!invokedDirectly && process.env['VITEST'] === 'true') {
  ...
  if (MODE_JSON) {
    ...
    process.stdout.write(JSON.stringify({ gate: 'lint-no-cwd-walkup', mode, ...counts, violations }, null, 2) + '\n');
  }
  ...
  if (MODE_BASELINE) {
    ...
    if (!MODE_JSON) {
      process.stdout.write(`[lint-no-cwd-walkup] Baseline written to scripts/.lint-no-cwd-walkup-baseline.json\n`);
      process.stdout.write(`[lint-no-cwd-walkup] RULE-1: ${counts.rule1Count} getCleoDirAbsolute reference(s) outside paths.ts\n`);
      process.stdout.write(`[lint-no-cwd-walkup] RULE-2: ${counts.rule2Count} getProjectRoot(process.cwd()) anti-pattern(s)\n`);
      process.stdout.write(`[lint-no-cwd-walkup] Use --check in CI to fail only on NEW violations above this baseline.\n`);
    }
    process.exit(0);
  }
  ...
  if (MODE_CHECK) {
    if (!existsSync(BASELINE_PATH)) {
      process.stderr.write(`[lint-no-cwd-walkup] ERROR: baseline not found at ${BASELINE_PATH}. Run --baseline first.\n`);
      process.exit(1);
    }
    let baseline;
    ...
    if (!MODE_JSON) {
      ...
      process.stderr.write(`=============================================================\n\n`);
      if (r1Bad) process.stderr.write(`RULE-1: ${blR1}→${current.rule1Count} (+${current.rule1Count - blR1}) getCleoDirAbsolute outside paths.ts\n`);
      if (r2Bad) process.stderr.write(`RULE-2: ${blR2}→${current.rule2Count} (+${current.rule2Count - blR2}) getProjectRoot(process.cwd())\n`);
  ...
  if (violations.length === 0) {
    if (!MODE_JSON) process.stdout.write(`[lint-no-cwd-walkup] PASS — no CWD-walk-up violations found.\n`);
    process.exit(0);
  }
  ...
  if (!MODE_JSON) {
    const counts = buildCounts();
    process.stderr.write(`\n=============================================================\n`);
    process.stderr.write(`CWD-WALKUP VIOLATION — ${counts.total} violation(s) (RULE-1: ${counts.rule1Count}, RULE-2: ${counts.rule2Count})\n`);
    process.stderr.write(`=============================================================\n\n`);
