...
import { DEFAULT_FETCH_TIMEOUT_MS, ensureOkResponse, fetchWithTimeout, formatNetworkError, NetworkError, } from "../../src/core/network/fetch.js";
afterEach(() => {
    vi.restoreAllMocks();
});
...
describe("ensureOkResponse", () => {
    ...
    it("throws NetworkError with http kind when ok is false", () => {
        const response = { ok: false, status: 502 };
    ...
    it("includes the status code in the error message", () => {
        ...
        expect(() => ensureOkResponse(response, "https://x.test")).toThrowError("Request failed with status 404");
    });
});
...
describe("fetchWithTimeout", () => {
    ...
    it("uses custom timeout value in error message", async () => {
        ...
        const mockFetch = vi.fn().mockRejectedValue(abortError);
        vi.stubGlobal("fetch", mockFetch);
        await expect(fetchWithTimeout("https://x.test", undefined, 3000)).rejects.toThrow("Request timed out after 3000ms");
    });
});
//# sourceMappingURL=network-fetch.test.js.map