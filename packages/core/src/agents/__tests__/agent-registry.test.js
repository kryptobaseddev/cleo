...
import { getAgentCapacity, getAgentSpecializations, getAgentsByCapacity, MAX_TASKS_PER_AGENT, recordAgentPerformance, updateAgentSpecializations, } from '../agent-registry.js';
...
describe('Agent Registry (T041)', () => {
    ...
    describe('getAgentCapacity', () => {
        ...
        it('counts active child agents toward capacity', async () => {
            ...
            for (let i = 0; i < 3; i++) {
                const child = await registerAgent({ agentType: 'executor', parentAgentId: parent.id }, tempDir);
    ...
    describe('getAgentsByCapacity', () => {
        ...
        it('filters by agent type', async () => {
            await registerAgent({ agentType: 'executor' }, tempDir).then((a) => updateAgentStatus(a.id, { status: 'active' }, tempDir));
            await registerAgent({ agentType: 'researcher' }, tempDir).then((a) => updateAgentStatus(a.id, { status: 'active' }, tempDir));
    ...
    describe('updateAgentSpecializations', () => {
        ...
        it('preserves other metadata keys', async () => {
            const agent = await registerAgent({ agentType: 'executor', metadata: { model: 'opus-4', region: 'us-east' } }, tempDir);
        ...
        it('returns null for non-existent agent', async () => {
            const result = await updateAgentSpecializations('agt_nonexistent_abc123', ['typescript'], tempDir);
            expect(result).toBeNull();
        });
    });
    // ==========================================================================
    ...
    describe('recordAgentPerformance', () => {
        it('returns null for non-existent agent', async () => {
            const result = await recordAgentPerformance('agt_nonexistent_abc123', { taskId: 'T001', taskType: 'task', outcome: 'success' }, tempDir);
            expect(result).toBeNull();
        });
        it('records performance and returns a decision ID', async () => {
            const agent = await registerAgent({ agentType: 'executor' }, tempDir);
            await updateAgentStatus(agent.id, { status: 'active' }, tempDir);
            const decisionId = await recordAgentPerformance(agent.id, {
                taskId: 'T041',
                taskType: 'task',
                outcome: 'success',
                durationMs: 3000,
        ...
        it('records failure with error metadata', async () => {
            ...
            await expect(recordAgentPerformance(agent.id, {
                taskId: 'T999',
                taskType: 'task',
                outcome: 'failure',
                errorMessage: 'ECONNREFUSED',
                errorType: 'retriable',
            }, tempDir)).resolves.not.toThrow();
        });
    });
});
