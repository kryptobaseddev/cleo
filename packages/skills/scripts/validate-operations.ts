 ...
 */
function extractRegistryOperations(registryPath: string): Set<string> {
...
/** A single operation reference found in a skill file. */
interface FoundRef {
  file: string;
  line: number;
  rawText: string;
 ...
 */
function buildSkipLineSet(lines: string[]): Set<number> {
 ...
 */
function isSpuriousOperation(operation: string): boolean {
  if (!operation || operation.endsWith('.')) return true;
  if (operation.includes('-')) return true;
  if (/^\.+$/.test(operation)) return true;
  if (/^[.]{2,}/.test(operation)) return true; // starts with ..
  if (FILE_EXTENSIONS.has(operation.toLowerCase())) return true;
  // Placeholder patterns like "...", "xxx"
  if (/^\.{2,}$/.test(operation)) return true;
  return false;
}
 ...
 */
function dedup(refs: FoundRef[]): FoundRef[] {
 ...
 */
function scanFile(filePath: string, canonicalDomains: Set<string>): FoundRef[] {
  ...
  const dotRe = new RegExp(
    `(?<![\\w.])(?:${domainsAlt})\\.([a-z][a-z0-9.]*)(?![a-z0-9.])`,
    'g',
  );
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    while ((m = dotRe.exec(line)) !== null) {
      ...
      if (!canonicalDomains.has(domain)) continue;
      if (isSpuriousOperation(operation)) continue;
      const key = `${domain}.${operation}`;
...
function main(): void {
  ...
  console.log(
    `Validating skills against registry (${registryOps.size} operations)...\n`,
  );
  ...
  for (const filePath of allFiles) {
    ...
    if (invalid.length === 0) {
      console.log(
        `  \u2713 ${relPath} \u2014 ${refs.length} reference${refs.length === 1 ? '' : 's'}, all valid`,
      );
    } else {
      failedFiles++;
