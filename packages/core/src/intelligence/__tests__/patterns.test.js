...
import { extractPatternsFromHistory, matchPatterns, storeDetectedPattern, updatePatternStats, } from '../patterns.js';
// ---- helpers ----------------------------------------------------------------
...
function mockBrainAccessor(patterns = [], observations = []) {
    return {
        ...
        addPattern: vi.fn().mockImplementation((row) => Promise.resolve(row)),
        getPattern: vi
            .fn()
            .mockImplementation((id) => Promise.resolve(patterns.find((p) => p.id === id) ?? null)),
...
describe('storeDetectedPattern', () => {
    it('stores a pattern via brain accessor', async () => {
        ...
        const call = brainAccessor.addPattern.mock.calls[0][0];
