...
import { predictAndStore, scoreVerificationConfidence, storePrediction, suggestGateFocus, } from '../adaptive-validation.js';
// ============================================================================
// Helpers
// ============================================================================
...
function mockBrainAccessor(patterns = [], observations = [], learnings = []) {
    return {
        ...
        addObservation: vi.fn().mockImplementation((row) => Promise.resolve(row)),
        addLearning: vi.fn().mockImplementation((row) => Promise.resolve(row)),
...
describe('scoreVerificationConfidence', () => {
    it('returns confidence 0 when all tracked gates failed', async () => {
        ...
        const result = await scoreVerificationConfidence('T001', verification, taskAccessor, brainAccessor, { dryRun: true });
    ...
    it('persists observation when not dry-run', async () => {
        ...
        const result = await scoreVerificationConfidence('T001', verification, taskAccessor, brainAccessor);
    ...
    it('extracts actionable learning when multiple gates fail', async () => {
        ...
        expect(brainAccessor.addLearning).toHaveBeenCalledTimes(1);
        const learningCall = brainAccessor.addLearning.mock.calls[0][0];
...
describe('storePrediction', () => {
    ...
    it('persists prediction observation to brain', async () => {
        ...
        const observationId = await storePrediction({
            ...
            blockers: ['Missing AC'],
            suggestions: [],
        }, brainAccessor, { project: 'test-project', sessionId: 'S-001' });
        expect(observationId).toMatch(/^O-pred-/);
        expect(brainAccessor.addObservation).toHaveBeenCalledTimes(1);
        const obs = brainAccessor.addObservation.mock.calls[0][0];
...
describe('confidence score boundary cases', () => {
    ...
    it('confidence is clamped between 0 and 1', async () => {
        ...
        const result = await scoreVerificationConfidence('T001', worstCase, taskAccessor, brainAccessor, { dryRun: true });
        expect(result.confidenceScore).toBeGreaterThanOrEqual(0);
        expect(result.confidenceScore).toBeLessThanOrEqual(1);
    });
});
//# sourceMappingURL=adaptive-validation.test.js.map