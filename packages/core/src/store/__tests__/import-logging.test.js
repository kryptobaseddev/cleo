...
import { extractPackageMeta, logImportConflict, logImportError, logImportStart, logImportSuccess, } from '../import-logging.js';
...
describe('logImportStart', () => {
    ...
    it('should log import start event', async () => {
        ...
        const entry = vi.mocked(appendJsonl).mock.calls[0][1];
        expect(entry.action).toBe('import');
        expect(entry.details.stage).toBe('start');
    });
});
describe('logImportSuccess', () => {
    ...
    it('should log import success with task details', async () => {
        await logImportSuccess('/path/to/export.json', ['T031', 'T032'], { T001: 'T031', T002: 'T032' }, [{ type: 'duplicate_title', resolution: 'rename' }], { parent: 'T001' }, 'session-1');
        expect(appendJsonl).toHaveBeenCalledTimes(1);
        const entry = vi.mocked(appendJsonl).mock.calls[0][1];
        expect(entry.action).toBe('import');
...
describe('logImportConflict', () => {
    ...
    it('should log import conflict with resolution', async () => {
        await logImportConflict('duplicate_title', 'T001', { existingId: 'T050', title: 'Duplicate Task' }, 'rename');
        expect(appendJsonl).toHaveBeenCalledTimes(1);
        const entry = vi.mocked(appendJsonl).mock.calls[0][1];
...
//# sourceMappingURL=import-logging.test.js.map