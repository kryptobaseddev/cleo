...
async function createTestProject(dir, projectId) {
...
describe('nexusReconcile', () => {
    ...
    it('scenario 4: hash conflict with different projectId throws CleoError', async () => {
        ...
        await writeFile(join(projectDir, '.cleo', 'project-info.json'), JSON.stringify({ projectId: projectId2, createdAt: new Date().toISOString() }));
        ...
        try {
            ...
            expect(err.code).toBe(75); // NEXUS_REGISTRY_CORRUPT
            expect(err.message).toContain('Project identity conflict');
        }
    });
