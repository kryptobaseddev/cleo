...
import { systemArchiveStats, validateCoherenceCheck, validateComplianceSummary, validateComplianceViolations, validateGateVerify, validateProtocol, validateProtocolConsensus, validateTestCoverage, validateTestStatus, } from '../../lib/engine.js';
describe('CheckHandler Operations', () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(validateComplianceSummary).mockReturnValue({ success: true, data: {} });
        vi.mocked(validateComplianceViolations).mockReturnValue({ success: true, data: {} });
        vi.mocked(validateTestStatus).mockReturnValue({ success: true, data: {} });
        vi.mocked(validateTestCoverage).mockReturnValue({ success: true, data: {} });
        vi.mocked(validateCoherenceCheck).mockResolvedValue({ success: true, data: {} });
        vi.mocked(validateProtocol).mockResolvedValue({ success: true, data: {} });
        vi.mocked(validateProtocolConsensus).mockResolvedValue({ success: true, data: {} });
        vi.mocked(validateGateVerify).mockResolvedValue({ success: true, data: {} });
        vi.mocked(systemArchiveStats).mockResolvedValue({ success: true, data: {} });
    });
...
//# sourceMappingURL=check-ops.test.js.map