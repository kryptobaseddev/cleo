...
description: "Bidirectional synchronization with Claude Code TodoWrite tool (ARCHIVED)"
icon: "arrows-rotate"
---
...
> **ARCHIVED**: This specification describes a Bash-era sync integration between CLEO and
> Claude Code's TodoWrite tool. The sync commands (`cleo sync --inject`, `cleo sync --extract`)
> were designed for the JSONL file storage era and have not been ported to the current
> TypeScript/SQLite architecture. CLEO now uses MCP (`cleo_query`/`cleo_mutate`) as the primary
> agent interface, making TodoWrite sync unnecessary for most workflows. This document is
...
| Tiered selection | Active task > dependencies > priority (max 8 tasks) |
...
- Current active task (from `.focus.currentTask`)

**Tier 2** (Include if capacity):
- Direct dependencies of active task (tasks in active task's `depends` array)

**Tier 3** (Fill remaining capacity):
- High/critical priority tasks in same phase as active task
...
3. Active task's phase - Inferred from `.focus.currentTask`
...
2. Active task's phase from session metadata
...
| `--focused-only` | flag | false | Only inject active task |
