...
- Sibling count exceeds configured `hierarchy.maxSiblings`
...
validate_custom_protocol() {
    ...
    check_output_file_exists "$task_id" ".cleo/agent-outputs" || {
...
{
  "id": "T2725-research",
  "file": ".cleo/agent-outputs/T2725-research.md",
  "title": "Protocol Metrics Research",
  "date": "2026-01-28",
  "status": "complete",
  "agent_type": "research",
  "topics": [
...
```bash
# Check manifest entry
grep "T2725" .cleo/agent-outputs/MANIFEST.jsonl | jq '.key_findings'
...
```bash
# Check status field
grep "T2726" .cleo/agent-outputs/MANIFEST.jsonl | jq '.status'
