...
describe("coverage: config command branches", () => {
    ...
    it("outputs json error when config file does not exist", async () => {
        ...
        await expect(program.parseAsync(["node", "test", "config", "show", "claude-code", "--json"])).rejects.toThrow("process-exit");
    ...
    it("exits with format conflict on config show", async () => {
        ...
        const program = new Command();
        registerConfigCommand(program);
        await expect(program.parseAsync(["node", "test", "config", "show", "claude-code", "--json", "--human"])).rejects.toThrow("process-exit");
    ...
    it("shows human error when provider not found on show", async () => {
        ...
        const program = new Command();
        registerConfigCommand(program);
        await expect(program.parseAsync(["node", "test", "config", "show", "unknown", "--human"])).rejects.toThrow("process-exit");
...
describe("coverage: doctor command branches", () => {
    ...
    it("exits via handleFormatError when both flags used", async () => {
        ...
        await expect(program.parseAsync(["node", "test", "doctor", "--json", "--human"])).rejects.toThrow("process-exit");
        expect(exitSpy).toHaveBeenCalledWith(1);
    });
...
describe("coverage: providers command branches", () => {
    ...
    it("exits with format conflict on providers show", async () => {
        ...
        await expect(program.parseAsync(["node", "test", "providers", "show", "claude-code", "--json", "--human"])).rejects.toThrow("process-exit");
        expect(exitSpy).toHaveBeenCalledWith(1);
    });
    ...
    it("shows human error when provider not found on show", async () => {
        ...
        const program = new Command();
        registerProvidersCommand(program);
        await expect(program.parseAsync(["node", "test", "providers", "show", "unknown", "--human"])).rejects.toThrow("process-exit");
        expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining("Provider not found: unknown"));
    });
    ...
    it("exits with format conflict on providers list", async () => {
        ...
        const program = new Command();
        registerProvidersCommand(program);
        await expect(program.parseAsync(["node", "test", "providers", "list", "--json", "--human"])).rejects.toThrow("process-exit");
        expect(exitSpy).toHaveBeenCalledWith(1);
    });
    ...
    it("exits with format conflict on providers detect", async () => {
        ...
        const program = new Command();
        registerProvidersCommand(program);
        await expect(program.parseAsync(["node", "test", "providers", "detect", "--json", "--human"])).rejects.toThrow("process-exit");
        expect(exitSpy).toHaveBeenCalledWith(1);
    });
});
//# sourceMappingURL=coverage-config-providers-doctor.test.js.map