...
import { orchestrateBootstrap, orchestrateCriticalPath, orchestrateUnblockOpportunities, } from '../../../dispatch/engines/orchestrate-engine.js';
import { sessionContextDrift, sessionDecisionLog, sessionRecordAssumption, sessionRecordDecision, } from '../../../dispatch/engines/session-engine.js';
 ...
 */
function writeDecisions(records) {
...
describe('E2E: Brain Operations', () => {
    ...
    describe('orchestrateBootstrap', () => {
        it('should return BrainState with expected fields for fast tier', async () => {
            ...
            const brain = (result.result ?? result.data);
        ...
        it('should default to fast speed when no speed parameter provided', async () => {
            ...
            expect(result.success).toBe(true);
            expect((result.result ?? result.data)._meta.speed).toBe('fast');
        });
        it('should include nextSuggestion when pending tasks exist', async () => {
            ...
            expect((result.result ?? result.data).nextSuggestion).toBeDefined();
            expect((result.result ?? result.data).nextSuggestion.id).toBeDefined();
            expect((result.result ?? result.data).nextSuggestion.title).toBeDefined();
            expect(typeof (result.result ?? result.data).nextSuggestion.score).toBe('number');
        });
    });
    // -------------------------------------------------------------------------
    ...
    describe('taskComplexityEstimate', () => {
        ...
        it('should score dependency depth correctly for chained tasks', async () => {
            ...
            expect(result.success).toBe(true);
            expect((result.result ?? result.data).dependencyDepth).toBeGreaterThanOrEqual(3);
        });
    });
    // -------------------------------------------------------------------------
    // 3. Coherence check
    ...
    describe('validateCoherenceCheck', () => {
        ...
        it('should report coherent for valid task graph', async () => {
            ...
            const result = await validateCoherenceCheck(TEST_ROOT);
            expect(result.success).toBe(true);
            expect((result.result ?? result.data).coherent).toBe(true);
            expect((result.result ?? result.data).issues.length).toBe(0);
        });
    });
    // -------------------------------------------------------------------------
    // 4. Critical path
    ...
    describe('orchestrateCriticalPath', () => {
        it('should find the longest dependency chain', async () => {
            ...
            const pathIds = data.path.map((p) => p.taskId);
    ...
    describe('orchestrateUnblockOpportunities', () => {
        it('should detect single-blocker tasks', async () => {
            ...
            const t106entry = singleBlockers.find((sb) => sb.taskId === 'T106');
        ...
        it('should identify high-impact completions', async () => {
            ...
            if (highImpact.length > 1) {
                expect(highImpact[0].wouldUnblock).toBeGreaterThanOrEqual(highImpact[highImpact.length - 1].wouldUnblock);
            }
        });
    ...
    describe('sessionRecordDecision / sessionDecisionLog', () => {
        ...
        it('should filter decisions by taskId', async () => {
            ...
            expect(result.success).toBe(true);
            const decisions = (result.result ?? result.data);
            expect(decisions.length).toBe(1);
        ...
        it('should return all decisions when no filters applied', async () => {
            ...
            expect(result.success).toBe(true);
            expect((result.result ?? result.data).length).toBe(SAMPLE_DECISIONS.length);
        });
        it('should return empty array when no decisions exist', async () => {
            await writeTodoJson(SAMPLE_TASKS);
            const result = await sessionDecisionLog(TEST_ROOT);
            expect(result.success).toBe(true);
            expect((result.result ?? result.data)).toHaveLength(0);
        });
    });
    // -------------------------------------------------------------------------
    ...
    describe('sessionContextDrift', () => {
        ...
        it('should return zero drift when no focus is set', async () => {
            ...
            const result = await sessionContextDrift(TEST_ROOT);
            expect(result.success).toBe(true);
            expect((result.result ?? result.data).score).toBe(0);
        });
    ...
    describe('systemInjectGenerate', () => {
        ...
        it('should include session info when active session exists', async () => {
            ...
            const result = await systemInjectGenerate(TEST_ROOT);
            expect(result.success).toBe(true);
            const mvi = (result.result ?? result.data).injection;
        ...
        it('should handle missing package.json gracefully', async () => {
            ...
            expect((result.result ?? result.data).injection.length).toBeGreaterThan(0);
            expect((result.result ?? result.data).sizeBytes).toBeLessThan(5 * 1024);
        });
    });
});
//# sourceMappingURL=brain-operations.test.js.map