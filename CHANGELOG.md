...
## [2026.5.62] (2026-05-13)

Auto-prepared by release.ship (T9232)

### Features
- **Implement max-tool-call budget enforcement in spawn prompt (ORC-006)**: Spawn prompts must include an explicit max tool call budget. When a worker exceeds N tool calls without completing, the harness should emit a warni... (T1656)
- Introduce ensureCleoDir(projectRoot) chokepoint in packages/core/src/paths.ts (T9195)
- **Implement cleo event append SDK operation and Conduit-backed transport**: Implement the agent event bus: packages/core/src/events/ module with appendEvent() function and Conduit-backed transport. CLI command cleo event ap... (T1651)
- **Implement cleo provenance <commitSha> command: show task+evidence from commit**: Given a commit SHA, return the CLEO task that produced it, the evidence gates that were verified, and the agent that made the commit. Uses git log ... (T1654)
- **Implement worker scope boundary enforcement: allowed-files ACL in spawn prompt**: Each worker spawn must include an explicit allowed-files ACL (Access Control List) derived from the task's files field. Workers that attempt to rea... (T1657)
- **Implement cleo orchestrator tail command for streaming worker event output**: Wire the orchestrator-side consumer: cleo orchestrator tail --epic T1234 streams events from all active worker agents under that epic in real-time.... (T1652)
- **Add runaway detection: flag workers that exceed wall-clock time or context budget**: Implement a monitoring sidecar that detects runaway workers: tasks that have been active for >2x the expected duration based on task size. Emit SIG... (T1658)
- **FISE-2 CANT validateSpawnRequest: reject implemented gate without upstream sub-agent commit**: Today validateSpawnRequest only checks role/tool restrictions not authorship (T9231)
- **Add METHOD_IMPLEMENTS schema entry + emission for interface/protocol implementations (currently missing)**: Owner directive 2026-05-04: pair with METHOD_OVERRIDES. Without it 'which classes implement this interface' is not graph-queryable. (T1847)
...
- **Tasks CRUD gap: relates.remove — implement removeRelation in DataAccessor + dispatch + CLI**: Per TASKS-CRUD-AUDIT.md: no way to delete a semantic relationship (blocks/duplicates/fixes/supersedes) once created. DataAccessor only has addRelat... (T9240)
...
- Fix nexus.ts base64url projectRoot path leak (6 sites at lines 301,310,389,574,581,1249) (T9199)
- **BBTT-W2-0 BUG: T1682 daemon auto-start verified-shipped but production has 0 automated dreams in 7+ days**: T1682 (DONE 2026-05-01) claimed daemon auto-start via systemd/launchd. T1636 (DONE 2026-05-01) claimed sentient hygiene background loop. Both used ... (T1898)
...
- **P0: rogue .cleo/ directories in cleo-spawned worktrees — T1864/T1873 regression**: P0: same disaster pattern as 2026-05-04 dead-end-DB incident. T1864 validateProjectRoot rejects package.json-only dirs, T1873 ALS bridge wires CLEO... (T9092)
...
- CI regression guard: cleo invocations with cwd outside registered project must create zero .cleo/ dirs (T9202)
- **BBTT-W2-2: Diagnose why sentient tick loop is not firing checkAndDream in production**: tick.ts:483-522 properly calls checkAndDream — code is wired. Failure is operational. Investigate: (a) cleo sentient status alive? (b) ~/.local/sha... (T1901)
- BUG: nexus.db shared-db test pollution — 4668 projects (89k+ nodes) from test runs bloating all query scans (T1835)
- **BUG: Project hash drift — same project resolving to TWO different hashes (4f2a513f66dcb422 vs 1e3146b7352ba279) — must be deterministic + registered per-project**: Owner observation 2026-05-05: /mnt/projects/cleocode has 2 active worktree hash dirs simultaneously: 4f2a513f66dcb422 (used by Wave 0 T1815/T1845/T... (T1891)
- **ADR-051 + ADR-062 ordering bug: evidence validation runs BEFORE merge, fails commit-reachability check**: Pattern observed across T1862, T1830 (and likely more): workers spawn from main snapshot, do work, commit on task/T<id>, capture commit:<sha> evide... (T1924)
- **BUG: Worker spawned task/T<id> branches inherit orphan history (test fixtures, unrelated commits) — must always start from clean main**: Pattern: worker prompts say 'git worktree add ... -b task/T<id> main' to create new branch from main. But task/T<id> may already exist from prior s... (T1927)
...
### Tests
- **BBTT-W3-1: Add origin column to tasks schema — production|test-fixture|imported|migrated**: 18 test-fixture-shaped rows in live tasks.db; 2 active (T932EP, E1) pollute every briefing. T1835 documents same pattern in nexus.db. Migration: AL... (T1899)
- Test fixture audit: replace /mock/project literal in tests with sandboxed tmpdir paths (T9201)
- **Test gap: integration test for spawn worktree provisioning failure paths**: All 10 T1140 spawn-prompt tests inject worktreePath directly, bypassing the provisioner. No integration test exercises createWorktree's stale-branc... (T1876)
...
- Remove legacy /.well-known/lafs.json discovery path from @cleocode/lafs (T9172)
...
- **T9047-FU migrate 16 direct DatabaseSync opens**: T9047 chokepoint shipped but consumer sweeps did NOT migrate existing direct opens (T9189)
- Migrate memory-bridge.ts (lines 246, 376) to ensureCleoDir chokepoint (T9196)
- Migrate nexus-bridge.ts:391 to ensureCleoDir chokepoint (T9197)
- Migrate harness-hint.ts, restore-conflict-report.ts, rcasd-index.ts, claude-code/context-monitor.ts to ensureCleoDir (T9198)
- **VS2-6 migrate 22 existing scripts/verify-*.mjs to canonical location**: Migration of campaign-era verifiers to v2 substrate (T9227)
- **Refactor ClaudeCodeSpawnProvider to consume SDK Tools (WorktreeIsolation)**: Refactor packages/adapters/src/providers/claude-code/spawn.ts (ClaudeCodeSpawnProvider) to consume the SDK Tools WorktreeIsolation surface promoted... (T1821)
...
- One-shot cleanup of existing stray .cleo/ and T### dirs in /mnt/projects/cleocode (T9203)
- **T9218-FU package-boundary refactor: move verifier runner+backfill from cleo CLI to core**: T9218 W6 implementation put SDK-level verifier resolution, child-process orchestration, and backfill enumeration inside packages/cleo/src/cli/comma... (T9219)
- **AUDIT: packages/cleo/src/dispatch/engines/ package-boundary violations — relocate non-dispatch business logic to packages/core/**: Smell: 2200-line orchestrate-engine.test.ts in packages/cleo/src/dispatch/engines/__tests__/ implies the source file (orchestrate-engine.ts) contai... (T1889)

### Changes
- **Design vendor-agnostic agent event bus API and choose transport (ADR)**: Research phase: evaluate transport options for agent event streaming (Conduit local, SSE, WebSocket, structured stdout). Write ADR choosing canonic... (T1650)
- **Enforce task ID in commit message via git commit-msg hook with configurable format**: Every commit must reference a CLEO task ID. Implement a git commit-msg hook that validates the commit message contains a T-prefixed task ID (e.g. '... (T1653)
- **IMPL: add DEFINES edges (file→symbol) to parse-loop — 20-line addition closes file-to-symbol traversal gap vs gitnexus 223,627 DEFINES edges**: T1042 audit gap (T1844-1): defines edge type is declared in packages/contracts/src/graph.ts:85 but never emitted. gitnexus emits 223,627 DEFINES (f... (T1836)
- **BBTT-W0-1: Gate relatedDocs on currentTaskId — briefing.ts:782-803**: Council Executor's 60-min action. Wrap the relatedDocs population block in computeDocsContext (briefing.ts lines 782-803) with 'if (currentTaskId !... (T1893)
...
- **P0 hotfix v2026.5.45: ESM smoke test fails — sqlite-pragmas.json path resolution breaks outside monorepo layout**: v2026.5.43 and v2026.5.44 release tags failed Release workflow at ESM smoke step. npm latest stuck at v2026.5.42. T1954+T1956 fixes in main but not... (T9157)
- **T9050-FU wire 6 sub-accessors UmbrellaDataAccessor**: T9050 marked done but UmbrellaDataAccessor only wires tasks; other 6 throw error (T9188)
- **VS2-1 location: relocate verifier scripts to .cleo/verifiers/<TID>.mjs**: Move canonical location from flat scripts/ to managed .cleo/verifiers/ (T9222)
- **FISE-3 ADR Bypass-Prevention Substrate**: Document the systemic defense in depth (T9229)
- **IMPL: add ACCESSES edges via new access-extractor.ts — member_expression/assignment_expression AST walk for read/write tracking (closes gitnexus 8293 ACCESSES gap)**: T1044 audit gap (T1844-2): accesses edge type declared in graph.ts:87 but no extractor walks assignment_expression or member_expression patterns. g... (T1837)
- **BBTT-W0-2: Filter test-fixture epics from computeActiveEpics — heuristic wedge**: Live cleo briefing surfaces T932EP 'T932 standalone epic with no files' and E1 'Test Epic' in activeEpics — both are LIVE production rows (verified... (T1894)
...
- **VS2-2 registry: add tasks.verifier_path TEXT NULL column**: Single source of truth for which verifier measures which task (T9223)
- **FISE-1 session-end hard gate: refuse close on Lead with delegate_task_count=0 + tasks_completed>0**: W5 instruments metric; now hard-gate enforce at session-end real-time not just session-grade warning (T9230)
- **Backfill git history: link existing commits to CLEO tasks via task-to-commit bridge**: Run a one-time backfill script that walks git log, matches commit messages to task IDs, and writes task_touches_commit rows to tasks.db. Uses the s... (T1655)
- **BBTT-W2-1: Add cleo memory dream --status CLI (engine liveness probe)**: No surface today reveals dream-cycle health. New subcommand returns JSON: lastConsolidatedAt, observationsSinceLastConsolidation, idleMinutesSinceL... (T1895)
- **Re-evaluate sqlite-native leaf-module invariant for sqlite-pragmas import**: T1331 documented sqlite-native as a leaf module that must not import any CLEO module to break the agent-resolver → dispatch-trace → ... → sqlite TD... (T9024)
...
- **T9025-FU real CI workflow gate for pragma drift**: T9025 added unit test only no CI workflow gate (T9190)
- **VS2-3 AC drift detection: acHash sha256 fingerprint**: When AC changes after verifier generation drift is silent today (T9224)
- **BBTT-W1-2: Pattern dedup at consolidation time — collapse near-duplicates**: Live: brain_patterns has 12,390 rows for 2,819 observations (4.4x ratio). Three near-identical Agent type X fails on task type Y rows in a 21-secon... (T1896)
...
- **T9064-FU cleo agent-outputs find CLI command**: T9064 utility shipped but CLI surface deferred per Final Lead (T9191)
- **VS2-4 GC: archive/delete lifecycle hooks for verifiers**: No GC today verifiers persist forever even after task archived (T9225)
- **BBTT-W3-2: Add origin + validated_at + provenance_chain to brain_observations**: Trust requires provenance. origin tracks producer (manual|auto-extract|transcript-ingest|session-debrief|test). validated_at records ground-truth v... (T1897)
...
- **PROTOCOL-HARDEN verifier-backed AC + auditor-loop**: Owner-mandated systemic fix after audit revealed scaffold-and-mark-done failure mode (T9192)
- **VS2-5 worktree weight: spawn-clone-exclude filter**: Worktree clones full scripts/verify-*.mjs today; grows linearly with tasks (T9226)
- Locate and fix sentient KMS .cleo/keys/ creator (creates empty keys dir in unregistered cwd) (T9200)
- **W2 auto-load ct-lead skill at tier-1 spawns**: ct-lead skill exists (T9084 merged) but spawn-prompt only auto-loads it for tier 2. Generalize the loader so tier 1 leads get ct-cleo+ct-lead. Pure... (T9213)
- **Write docs/architecture/sdk-tools.md — canonical SDK Tools reference**: Write the canonical SDK Tools reference at docs/architecture/sdk-tools.md. Document the 4-category taxonomy (isolation, manifest, tool-resolver, to... (T1820)
...
- **W4 atomicity gate UX hardening**: checkAtomicity() at packages/core/src/orchestration/atomicity.ts:104 already returns fixHint. Improve hint and add structured waiver flag to remove... (T9214)
- **VS2-7 ephemeral exemption: lifetime=session skips verifier**: Not every task needs persistent verifier; ephemeral session tasks should skip (T9228)
- **T-LW-W9: Fix Studio production build (Vite wasm import error from loro-crdt)**: Studio vite build fails with 'Unexpected character (1:0)' on loro-crdt's loro_wasm_bg.wasm (transitive from llmtxt CRDT). Need vite-plugin-wasm OR ... (T1693)
- **BBTT-W1-1: Add mode=recency|lexical|hybrid + since timestamp to searchBrainCompact**: Root staleness defect. session-memory.ts:411-415 ranks observations by literal-string BM25 against 'session', causing 11-day-old rows to top fresh ... (T1900)
- **W1 delegate_task JSON envelope sentinel**: Load-bearing wire. Define formal JSON envelope schema in contracts; adapter recognizers parse stdout for the sentinel and call orchestrateSpawnExec... (T9215)
- **Verify PiHarness SDK Tool consumption is complete and formally documented**: Audit packages/caamp/src/core/harness/pi.ts (PiHarness) to confirm SDK Tool consumption is complete. Verify provisionIsolatedShell is imported from... (T1822)
- **W3 audit phase in IVTR + cantbook fix-up**: Owner chose Option A: 5th IVTR phase + paired cantbook fix-up. Forward-only migration with schemaVersion shim. Fixes existing runtime/cantbook drif... (T9216)
- **P0: T1763 worktree isolation breach — direct commit to main bypassed git-shim cwd boundary check (T1761) and absolute-path Edit/Write**: T1763 cleo-subagent committed research files (.cleo/rcasd/T1763/research/imports-edge-gap.md and SUPERSESSION-EVIDENCE.md) directly to main as comm... (T1823)
- **BBTT-W5-1: ADR — per-worktree handoff schema (actor, scope, branch) — design only**: getLastHandoff returns globally-most-recent — race condition for parallel worktree orchestrators (per ADR-055/T1140). PRIME single-orchestrator is ... (T1902)
...
- **W5 Lead bypass detection in session grade**: Instrument delegate_task usage. Adds sub-metric under existing S3 dimension; no rubric replacement. Catches Lead self-implementation pattern. (T9217)
- **BBTT-W3-5: Auto-extract repair — only 11 brain_learnings exist for 2,819 brain_observations**: Live SQL: brain_learnings has 11 rows total, brain_observations has 2,819. Auto-extraction is silently broken — either disabled, threshold too aggr... (T1903)
- **W6 mandatory verifier strict + cleo verify backfill tool**: Owner chose strict from day one. Backfill tool reuses same auto-stub generator that tasks.add uses; smooth transition for existing tasks via per-ta... (T9218)
- **BBTT-W2-3: Opportunistic dream trigger from cleo briefing (cooldown-respected)**: Council Expansionist Finding 4: every cleo briefing call should give the dream cycle a chance to fire. Existing 5min cooldown + dreamInFlight guard... (T1904)
- **BBTT-W3-4: Test-DB isolation enforcement — CI gate that fails any test writing to .cleo/tasks.db**: Tests in epic-auto-complete, loom-auto-init, rcsd-pipeline-e2e, studio epic-rollup write to live tasks.db. T1864 ADR-067 hardened validateProjectRo... (T1906)
- **BBTT-W1-3: BriefingFieldContract types + assertion in computeBriefing**: Field names are unenforced promises today (recentObservations is not recent, relevantPatterns has 3 dups, activeEpics surfaces fixtures). Add Brief... (T1905)
- **BBTT-W2-5: Freshness sentinel CI gate — daily cleo memory dream --status check**: Daily CI check runs cleo memory dream --status against canary; alerts if isOverdue=true for >24h. The did-the-engine-actually-fire alarm. Catches f... (T1907)
- **BBTT-W2-4: cleo doctor brain — health dashboard CLI**: Centerpiece of trust observability. Aggregates: BRAIN row counts per table, dedup ratio, last consolidation, recentObservations max-age violations,... (T1908)
- **BBTT-W3-3: cleo doctor scan-test-fixtures-in-prod — heuristic scanner with quarantine**: Scan tasks.db for rows matching test heuristics (^E\\d+$, ^T\\d+EP$, titles containing Test Epic, with no files, fixture). Flag with confidence sco... (T1909)
- DECISION: Reject gitnexus O(m²) implicit wiring approach for Swift; explicit-import Swift extraction REQUIRED follow-up (split — see T1841 + T1842) (T1838)
- **Tasks CRUD gap: clearBlockedBy — blockedBy reason text can be set but never cleared**: Per TASKS-CRUD-AUDIT.md: blockedBy free-text reason persists as stale text after unblock. Add --clear-blocked-by flag on cleo update OR auto-clear ... (T9241)
- **T1870: rebase T1867 + extract bridge to core**: T1867 branched from pre-T1864 main so its paths.ts diff reverts T1864. Also inlined the env-ALS bridge in cleo/cli/index.ts which violates Package-... (T1873)
- **T1874: brain + studio getCleoHome — eliminate hand-rolled paths use env-paths or import from core**: Issue #102 / PR #103 surface a Windows path bug: brain and studio hand-roll getCleoHome which returns %LOCALAPPDATA%/cleo while core uses env-paths... (T1874)
- **LOC reduction: derive TasksHandler.getSupportedOperations() from QUERY_OPS Set (eliminate 3-source-of-truth)**: Pattern surfaced via T1923 + T1925 + T1923-finalize cascade: tasks query ops are declared in 3 places — QUERY_OPS Set (line 507), handler functions... (T1926)
---
...
### Features
- **Add global-identity guard to all projectRoot git user writes**: After T9087 fixes the read-check env, add a reusable guard function (e.g. hasGitIdentity(scope, cwd)) that checks both --global and --local config ... (T9088)
...
### Bug Fixes
- **Fix scaffold.ts cleanEnv git identity fallback**: In packages/core/src/scaffold.ts lines 898-914, the code uses cleanEnv (which strips HOME and other env vars) when calling git config user.email to... (T9087)
...
### Tests
- **Test and quality gates: verify git identity fix**: Run full quality gates after T9087+T9088 land: pnpm biome check --write, pnpm run typecheck (tsc -b), pnpm run test. Verify zero new test failures.... (T9089)
...
---
## [2026.5.42] (2026-05-07)

### Bug Fixes
- fix(T9079): fix unawaited teardownWorktree Promise causing unhandled rejections in tests (T9079)

---
## [2026.5.41] (2026-05-07)

### Bug Fixes
- fix(T9079): exclude integration tests from project-level vitest configs (T9079)

---
## [2026.5.40] (2026-05-07)

### Bug Fixes
- fix(T9079): move paths to Wave 1 — zero-dep root, prevents Wave 3 race condition (T9079)

---
## [2026.5.39] (2026-05-07)

### Refactors
- **Comprehensive worktree lifecycle overhaul**: Complete worktree infrastructure overhaul addressing critical bugs, missing lifecycle hooks, and copy-on-write provisioning (T1466)
- **Modernize vitest, parallelize build, fix contracts SSoT + 3 test failures**: Add test.projects array to root vitest.config.ts, parallelize build waves, fix contracts barrel SSoT (T9079)

### Chores
- **T1889-W8: Delete 13 dispatch engine shims + remove stray .fuse_hidden orphan**: 9 zero-importer dead-code shims removed; 4 importers redirected to @cleocode/core/internal; stray FUSE cruft cleaned from .claude/ (T9078)

---
## [2026.5.38] (2026-05-06)

Auto-prepared by release.ship (T1910)
...
- **Create @cleocode/paths package + implement primitives**: Scaffold packages/paths/ with composite tsconfig, package.json, vitest config, src/index.ts. Implement: createPlatformPathsResolver(appName, homeEn... (T1883)
...
- **T1878 follow-up: fix orchestrate-engine tests broken by T1878 worktree-provision-failure surfacing**: 5 tests in packages/cleo/src/dispatch/engines/__tests__/{orchestrate-engine.test.ts, orchestrate-engine-composer.test.ts} fail post-T1878 because t... (T1880)
- **BUG: surprisal-tree randomProjectionVector NaN propagation — randomBytes-as-Float32 produces NaN**: Worker investigation 2026-05-05: randomProjectionVector() in packages/core/src/memory/surprisal-tree.ts called randomBytes(dim*4) reinterpreted as ... (T1890)
...
- **Migrate @cleocode/core to consume @cleocode/paths**: Replace envPaths import in core/system/platform-paths.ts with import from @cleocode/paths. Replace getCleoTemplatesTildePath body in core/paths.ts ... (T1884)
...
- **Migrate @cleocode/worktree to consume @cleocode/paths**: Replace envPaths in worktree/src/paths.ts (resolveWorktreeRootForHash, computeProjectHash) and worktree/src/worktree-list.ts (listWorktrees data di... (T1885)
...
- **Migrate @cleocode/adapters/shared + @cleocode/brain to consume @cleocode/paths**: Replace duplicated getCleoTemplatesTildePath in adapters/providers/shared/paths.ts with re-export from @cleocode/paths (DELETE the inline implement... (T1886)
...
- **Migrate @cleocode/caamp to consume @cleocode/paths factory**: Replace duplicated createPlatformPathsResolver in caamp/core/platform-paths.ts with re-export from @cleocode/paths. CAAMP keeps its bound 'agents' ... (T1887)
...
- **Wire @cleocode/paths into release pipeline + final QA + commit**: Add @cleocode/paths to release.yml version-sync loop AND publish_pkg dependency-ordered list (BEFORE worktree, brain, adapters, core). Add CHANGELO... (T1888)
...
## [2026.5.27] (2026-05-05)

Auto-prepared by release.ship (T1881)
...
# ... fails commit if any PROTECTED_FILES are gitignored
```

Result: `cleo init` succeeded, then any `git commit` exited 1 with `"Critical CLEO files are being ignored by .gitignore"` — exactly the files T5158 says SHOULD be ignored.

### Fix

- **`packages/core/templates/git-hooks/pre-commit`**: Removed `config.json` and `project-info.json` from `PROTECTED_FILES` (they should be ignored per ADR-013 §9). `.cleo/.gitignore` and `project-context.json` retained — those are intended to be tracked.
- Added `Section 3` to the hook: positively block staging of `.cleo/{tasks.db,brain.db,config.json,project-info.json}` (the T5158 anti-pattern). This makes the hook align with ADR-013 §9 in both directions: don't require the files to be tracked, but actively reject any attempt to track them.
