...
const backupReadOnlyCalls = (backupPack.match(/applyPerfPragmas\(db,\s*\{[^}]*enableWal:\s*false[^}]*\}/g) || []);
if (backupReadOnlyCalls.length >= 2) {
  pass(`backup-pack.ts has ${backupReadOnlyCalls.length} read-only applyPerfPragmas calls`);
} else {
  fail(`backup-pack.ts must have >= 2 read-only applyPerfPragmas(db, {enableWal:false}) calls, found ${backupReadOnlyCalls.length}`);
}
...
const unpackReadOnlyCalls = (backupUnpack.match(/applyPerfPragmas\(db,\s*\{[^}]*enableWal:\s*false[^}]*\}/g) || []);
if (unpackReadOnlyCalls.length >= 1) {
  pass(`backup-unpack.ts has ${unpackReadOnlyCalls.length} read-only applyPerfPragmas calls`);
} else {
  fail(`backup-unpack.ts must have >= 1 read-only applyPerfPragmas(db, {enableWal:false}) call, found ${unpackReadOnlyCalls.length}`);
}
...
const atomicReadOnlyCalls = (atomic.match(/applyPerfPragmas\(db,\s*\{[^}]*enableWal:\s*false[^}]*\}/g) || []);
if (atomicReadOnlyCalls.length >= 1) {
  pass(`atomic.ts has ${atomicReadOnlyCalls.length} read-only applyPerfPragmas calls`);
} else {
  fail(`atomic.ts must have >= 1 applyPerfPragmas call with enableWal:false for inspection path, found ${atomicReadOnlyCalls.length}`);
}
