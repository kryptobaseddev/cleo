...
export async function createSession(session, cwd) {
...
/** Get a session by ID. */
export async function getSession(sessionId, cwd) {
    const db = await getDb(cwd);
    const rows = await db
        .select()
...
/** Update a session. */
export async function updateSession(sessionId, updates, cwd) {
    const db = await getDb(cwd);
    ...
    if (updates.scope !== undefined)
        updateRow.scopeJson = JSON.stringify(updates.scope);
    if (updates.endedAt !== undefined)
        updateRow.endedAt = updates.endedAt;
    if (updates.agent !== undefined)
        updateRow.agent = updates.agent;
    ...
    if (updates.nextSessionId !== undefined)
        updateRow.nextSessionId = updates.nextSessionId;
    if (updates.agentIdentifier !== undefined)
        updateRow.agentIdentifier = updates.agentIdentifier;
    ...
    if (updates.debriefJson !== undefined)
        updateRow.debriefJson = updates.debriefJson;
    if (updates.handoffJson !== undefined)
        updateRow.handoffJson = updates.handoffJson;
    db.update(schema.sessions).set(updateRow).where(eq(schema.sessions.id, sessionId)).run();
    return getSession(sessionId, cwd);
}
/** List sessions with optional filters. */
export async function listSessions(filters, cwd) {
    const db = await getDb(cwd);
    const conditions = [];
    if (filters?.active) {
...
/** End a session. */
export async function endSession(sessionId, note, cwd) {
...
export async function startTask(sessionId, taskId, cwd) {
    const db = await getDb(cwd);
    const now = new Date().toISOString();
    // Clear previous work history entry (set clearedAt)
    db.update(schema.taskWorkHistory)
        .set({ clearedAt: now })
        .where(and(eq(schema.taskWorkHistory.sessionId, sessionId), isNull(schema.taskWorkHistory.clearedAt)))
        .run();
...
export async function getCurrentTask(sessionId, cwd) {
    const db = await getDb(cwd);
    const rows = await db
        .select({
...
export async function workHistory(sessionId, limit = 50, cwd) {
    const db = await getDb(cwd);
    const rows = await db
        .select()
...
export async function gcSessions(maxAgeDays = 30, cwd) {
    const db = await getDb(cwd);
