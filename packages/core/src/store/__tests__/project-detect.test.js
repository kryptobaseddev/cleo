...
function assertSchemaValid(result, label = '') {
    const valid = validate(result);
    if (!valid) {
        throw new Error(`${label ? label + ' — ' : ''}Schema invalid: ${JSON.stringify(validate.errors, null, 2)}`);
    }
}
// ─── Scaffold helper ──────────────────────────────────────────────────────────
...
//# sourceMappingURL=project-detect.test.js.map