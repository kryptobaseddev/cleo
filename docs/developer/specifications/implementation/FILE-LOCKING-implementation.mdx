---
title: "File Locking Implementation"
description: "Implementation details for file locking and concurrency safety mechanisms"
icon: "code"
---
...
| T453 | Fix unprotected log_operation() in add.sh | **done** | medium | MEDIUM |
| T454 | Fix unprotected focus-clearing in complete.sh | **done** | medium | MEDIUM |
...
|--------|---------------------|------------------|------------------|--------|
| add.sh | YES | YES (main write + log_operation) | YES | **COMPLETE** |
| update.sh | YES | NO (via save_json) | YES | COMPLETE |
| complete.sh | YES | NO (via save_json) | YES (all writes) | **COMPLETE** |
...
| add.sh | todo-log.jsonl | ~~log_operation() unprotected~~ | ~~MEDIUM~~ | **FIXED (T453)** |
| complete.sh | todo.json | ~~Focus clear unprotected~~ | ~~MEDIUM~~ | **FIXED (T454)** |
...
| log.sh/lib/logging.sh | todo-log.jsonl | ~~atomic temp+mv but no flock~~ | ~~LOW~~ | **FIXED (T531)** |
...
- [x] update.sh - uses save_json()
- [x] focus.sh - uses save_json()
- [x] session.sh - uses save_json()
- [x] add.sh main write - uses lock_file()
- [x] add.sh log_operation() (T453) - uses lock_file() + atomic write
- [x] complete.sh focus clearing (T454) - uses save_json()
...
|----------|--------|----------|
| FILE-LOCKING-SPEC.md | COMPLETE | docs/archive/specs-md/ (archived) |
...
```bash
# In add.sh, wrap log_operation writes:
lock_fd=""
...
```bash
# In complete.sh, replace inline jq with:
...
| scripts/add.sh | log_operation() | lock_file() + atomic write |
| scripts/complete.sh | Focus clearing | save_json() |
