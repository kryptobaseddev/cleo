...
## Axis 2 — IMPORTS Edge Gap: Root Cause + All-Relation-Types Comparison

**Added by T1763 (2026-05-04)**. Full spec at `/mnt/projects/cleocode/.cleo/rcasd/T1763/research/imports-edge-gap.md`.

### Root Cause: Swift Implicit All-Pairs Wiring

The 8.4x headline gap (46,674 vs 390,893) is almost entirely explained by a single mechanism: gitnexus's Swift implicit-wiring model. In openclaw, 620 Swift files × 619 peers = **383,780 implicit IMPORTS edges** emitted by gitnexus. Cleo has no Swift extractor and no `wireImplicitImports` equivalent.

Stripping Swift, gitnexus has 7,113 non-Swift IMPORTS; cleo has 46,674 — **cleo is 6.5x ahead on non-Swift imports.**

### All-Relation-Types Comparison Table (openclaw, 2026-05-04)

| Relation Type | gitnexus (fresh 1.6.3) | cleo nexus (fresh 2026.5.16) | Delta | Notes |
|---------------|------------------------|------------------------------|-------|-------|
| IMPORTS / imports | 390,893 | 46,674 | **-344,219 (-88%)** | 383,780 of gitnexus total are Swift implicit all-pairs; stripping Swift cleo leads 6.5x |
| DEFINES / (none) | 223,627 | 0 | -223,627 | cleo emits no file→symbol DEFINES edges (pure omission per Axis 1) |
| CALLS / calls | 63,267 | 86,293 | **+23,026 (+36%)** | cleo leads — more resolved TS call edges |
| MEMBER_OF / (none) | 23,798 | 0 | -23,798 | blocked by T1764 Leiden failure; community-processor.ts exists but emits zero |
| CONTAINS / contains | 22,361 | 14,809 | -7,552 (-34%) | gitnexus leads; cleo emits folder→file contains edges only |
| HAS_PROPERTY / has_property | 8,491 | 1,068 | -7,423 (-87%) | cleo only extracts class body fields; missing interface/type-alias members |
| ACCESSES / (none) | 8,293 | 0 | -8,293 | missing: no AST walk for member_expression access patterns |
| HAS_METHOD / has_method | 5,917 | 1,231 | -4,686 (-79%) | cleo only extracts class method_definition; missing Swift/other methods |
| STEP_IN_PROCESS / step_in_process | 1,368 | 454 | -914 (-67%) | both emit; gitnexus has more process nodes |
| EXTENDS / extends | 271 | 136 | -135 (-50%) | both emit; gitnexus catches more via Swift/other heritage |
| METHOD_IMPLEMENTS / (none) | 158 | 0 | -158 | not in cleo's GraphRelationType |
| IMPLEMENTS / implements | 71 | 30 | -41 | both emit; partial parity |
| HANDLES_ROUTE / handles_route | 57 | 2 | -55 | both detect routes; cleo coverage sparse |
| ENTRY_POINT_OF / entry_point_of | 6 | 75 | **+69** | cleo leads — more entry points detected |
| METHOD_OVERRIDES / (none) | 3 | 0 | -3 | not in cleo |
| (none) / module→external | 0 | 10,711 | **+10,711** | cleo unique: external npm/node imports as first-class nodes |
| **Total** | **748,581** | **150,770** | **-597,811 (-80%)** | |

### Unique-to-cleo Relation Types

| Type | Count | Description |
|------|-------|-------------|
| `imports` (external module) | 10,711 | file → `module:npm-pkg` edges for unresolvable imports |
| `entry_point_of` | 75 | cleo 12.5x ahead |

### IMPORTS Breakdown by Language

| Language | gitnexus | cleo | Verdict |
|----------|---------|------|---------|
| Swift (implicit all-pairs) | 383,780 | 0 | gitnexus only — O(m²) SPM target visibility |
| TypeScript | 6,324 | 46,157 | cleo **7.3x ahead** |
| JavaScript | 205 | 517 | cleo 2.5x ahead |
| Markdown (cross-links) | 6 | 0 | gitnexus only |
| Go/Other | ~578 | 0 | cleo missing Go/Python import edges (extractors exist, not wired) |
| External modules | 0 | 10,711 | cleo unique |
| **Total** | **390,893** | **46,674** | |

### Far-Exceed Strategy (Axis 2)

Cleo can surpass gitnexus on IMPORTS on 6 vectors not available in gitnexus:

1. **External Module Enrichment**: Cleo's 10,711 external module nodes can be tagged with category (`ui-framework`, `nodejs-builtin`, etc.) and version from `package.json`. Enables "files depending on deprecated packages" queries — impossible in gitnexus.
2. **Brain-Anchored Import Semantics**: When a BRAIN observation references an import decision, link the `imports` edge to the memory entry via `import_rationale`. Structurally impossible in gitnexus (no memory substrate).
3. **Dynamic Import Detection**: `import(...)` expressions (tree-sitter call_expression with import callee) as `dynamic_imports` relation — runtime-conditional dependencies. Neither tool captures these today.
4. **Barrel Re-Export Chain Metadata**: Cleo already builds `BarrelExportMap`; surface transitive chain depth as `hopCount` metadata on edges. "Files with >3-hop barrel dependencies" — a module design quality indicator.
5. **Type-Only Import Tracking**: Cleo's extractor already handles `import type { ... }`. Adding `importKind: 'type' | 'value'` metadata distinguishes type-erasure-safe from value-required dependencies.
6. **Contract Imports Relation**: For imports resolving to `packages/contracts/src/*`, emit `contract_imports` relation. Enables full downstream surface of any contracts API change.
