...
describe("coverage: mcp remove", () => {
  ...
  it("exits with human error when no providers found", async () => {
    ...
    await expect(
      program.parseAsync(["node", "test", "remove", "srv", "--all", "--human"]),
    ).rejects.toThrow("process-exit");
  ...
  it("exits with format conflict error", async () => {
    ...
    await expect(
      program.parseAsync(["node", "test", "remove", "srv", "--json", "--human"]),
    ).rejects.toThrow("process-exit");
...
describe("coverage: mcp detect", () => {
  ...
  it("exits with format conflict error", async () => {
    ...
    await expect(
      program.parseAsync(["node", "test", "detect", "--json", "--human"]),
    ).rejects.toThrow("process-exit");
  ...
  it("shows human-readable detect output with providers and configs", async () => {
    ...
    mocks.resolveConfigPath.mockImplementation((_p: unknown, scope: string) =>
      scope === "global" ? "/global/claude.json" : "/project/claude.json",
    );
