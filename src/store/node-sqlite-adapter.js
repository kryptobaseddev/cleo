 ...
 */
export function openNativeDatabase(path, options) {
    ...
    if (options?.enableWal !== false) {
        ...
        if (!walSet) {
            ...
            if (finalMode !== 'wal') {
                db.close();
                throw new Error(`CRITICAL: Failed to set WAL journal mode after ${MAX_WAL_RETRIES} attempts. ` +
 ...
 */
export function createDrizzleCallback(db) {
 ...
 */
export function createBatchCallback(db) {
    return async (batch) => {
