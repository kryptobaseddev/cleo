...
function makeProvider(id, priority = "medium") {
    return {
        id,
        toolName: id,
        vendor: "test",
...
function parseEnvelope(spy, callIndex = 0) {
    return JSON.parse(String(spy.mock.calls[callIndex]?.[0] ?? "{}"));
}
describe("coverage: advanced commands", () => {
    ...
    beforeEach(() => {
        ...
        mocks.getProvider.mockImplementation((id) => {
            if (id === "alpha")
                return alpha;
            if (id === "beta")
                return beta;
        ...
        mocks.selectProvidersByMinimumPriority.mockImplementation((providers) => providers);
    ...
    it("readSkillOperations: invalid skillName", async () => {
        ...
        vi.spyOn(process, "exit").mockImplementation((() => { throw new Error("x"); }));
        await expect(createProgram().parseAsync(["node", "test", "advanced", "batch", "--agent", "alpha", "--skills-file", "/tmp/skills.json"])).rejects.toThrow("x");
        expect(parseEnvelope(errorSpy)["error"].message).toContain("Invalid skillName");
    });
    it("readSkillOperations: invalid isGlobal", async () => {
        ...
        vi.spyOn(process, "exit").mockImplementation((() => { throw new Error("x"); }));
        await expect(createProgram().parseAsync(["node", "test", "advanced", "batch", "--agent", "alpha", "--skills-file", "/tmp/skills.json"])).rejects.toThrow("x");
        expect(parseEnvelope(errorSpy)["error"].message).toContain("Invalid isGlobal");
    });
    it("readMcpOperations: non-array JSON", async () => {
        mocks.readFile.mockResolvedValue(JSON.stringify({ serverName: "test" }));
        const errorSpy = vi.spyOn(console, "error").mockImplementation(() => { });
        vi.spyOn(process, "exit").mockImplementation((() => { throw new Error("x"); }));
        await expect(createProgram().parseAsync(["node", "test", "advanced", "conflicts", "--agent", "alpha", "--mcp-file", "/tmp/mcp.json"])).rejects.toThrow("x");
        expect(parseEnvelope(errorSpy)["error"].message).toContain("must be a JSON array");
    });
    it("readMcpOperations: non-object item", async () => {
        ...
        vi.spyOn(process, "exit").mockImplementation((() => { throw new Error("x"); }));
        await expect(createProgram().parseAsync(["node", "test", "advanced", "conflicts", "--agent", "alpha", "--mcp-file", "/tmp/mcp.json"])).rejects.toThrow("x");
        expect(parseEnvelope(errorSpy)["error"].message).toContain("Invalid MCP operation at index 0");
    });
    it("readMcpOperations: empty serverName", async () => {
        ...
        vi.spyOn(process, "exit").mockImplementation((() => { throw new Error("x"); }));
        await expect(createProgram().parseAsync(["node", "test", "advanced", "conflicts", "--agent", "alpha", "--mcp-file", "/tmp/mcp.json"])).rejects.toThrow("x");
        expect(parseEnvelope(errorSpy)["error"].message).toContain("Invalid serverName");
    });
    it("readMcpOperations: invalid config (string)", async () => {
        ...
        vi.spyOn(process, "exit").mockImplementation((() => { throw new Error("x"); }));
        await expect(createProgram().parseAsync(["node", "test", "advanced", "conflicts", "--agent", "alpha", "--mcp-file", "/tmp/mcp.json"])).rejects.toThrow("x");
        expect(parseEnvelope(errorSpy)["error"].message).toContain("Invalid config");
    });
    ...
    it("readMcpOperations: invalid scope", async () => {
        ...
        vi.spyOn(process, "exit").mockImplementation((() => { throw new Error("x"); }));
        await expect(createProgram().parseAsync(["node", "test", "advanced", "conflicts", "--agent", "alpha", "--mcp-file", "/tmp/mcp.json"])).rejects.toThrow("x");
        expect(parseEnvelope(errorSpy)["error"].message).toContain("Invalid scope");
    });
    ...
    it("readSkillOperations: non-object item", async () => {
        ...
        vi.spyOn(process, "exit").mockImplementation((() => { throw new Error("x"); }));
        await expect(createProgram().parseAsync(["node", "test", "advanced", "batch", "--agent", "alpha", "--skills-file", "/tmp/skills.json"])).rejects.toThrow("x");
        expect(parseEnvelope(errorSpy)["error"].message).toContain("Invalid skill operation");
    });
    it("readSkillOperations: empty sourcePath", async () => {
        ...
        vi.spyOn(process, "exit").mockImplementation((() => { throw new Error("x"); }));
        await expect(createProgram().parseAsync(["node", "test", "advanced", "batch", "--agent", "alpha", "--skills-file", "/tmp/skills.json"])).rejects.toThrow("x");
        expect(parseEnvelope(errorSpy)["error"].message).toContain("Invalid sourcePath");
    });
    it("readSkillOperations: with isGlobal=true", async () => {
        mocks.readFile.mockImplementation(async (path) => {
            if (path.includes("skills"))
                return JSON.stringify([{ sourcePath: "./s", skillName: "s", isGlobal: true }]);
            if (path.includes("mcp"))
                return JSON.stringify([{ serverName: "fs", config: { command: "npx" } }]);
    ...
    it("readTextInput: both content and content-file", async () => {
        ...
        expect(parseEnvelope(errorSpy)["error"].message).toContain("either inline content or a content file");
    });
    it("readTextInput: unreadable content file", async () => {
        ...
        expect(parseEnvelope(errorSpy)["error"].message).toContain("Failed to read content file");
    });
    it("readJsonFile: file read failure", async () => {
        mocks.readFile.mockRejectedValue(new Error("EACCES"));
        const errorSpy = vi.spyOn(console, "error").mockImplementation(() => { });
        vi.spyOn(process, "exit").mockImplementation((() => { throw new Error("x"); }));
        await expect(createProgram().parseAsync(["node", "test", "advanced", "conflicts", "--agent", "alpha", "--mcp-file", "/tmp/bad.json"])).rejects.toThrow("x");
        expect(parseEnvelope(errorSpy)["error"].message).toContain("Failed to read JSON file");
    });
    // ── configure.ts ───────────────────────────────────────────────────────
    it("configure: no operations provided", async () => {
        const errorSpy = vi.spyOn(console, "error").mockImplementation(() => { });
        vi.spyOn(process, "exit").mockImplementation((() => { throw new Error("x"); }));
        await expect(createProgram().parseAsync(["node", "test", "advanced", "configure", "--agent", "alpha"])).rejects.toThrow("x");
        expect(parseEnvelope(errorSpy)["error"].message).toContain("No configuration operations");
    });
    it("configure: MCP global write failures", async () => {
        ...
        const errorSpy = vi.spyOn(console, "error").mockImplementation(() => { });
        vi.spyOn(process, "exit").mockImplementation((() => { throw new Error("x"); }));
        await expect(createProgram().parseAsync(["node", "test", "advanced", "configure", "--agent", "alpha", "--global-mcp-file", "/tmp/global-mcp.json"])).rejects.toThrow("x");
        expect(parseEnvelope(errorSpy)["error"].message).toContain("MCP writes failed");
    });
    it("configure: MCP project write failures", async () => {
        ...
        const errorSpy = vi.spyOn(console, "error").mockImplementation(() => { });
        vi.spyOn(process, "exit").mockImplementation((() => { throw new Error("x"); }));
        await expect(createProgram().parseAsync(["node", "test", "advanced", "configure", "--agent", "alpha", "--project-mcp-file", "/tmp/project-mcp.json"])).rejects.toThrow("x");
        expect(parseEnvelope(errorSpy)["error"].message).toContain("MCP writes failed");
    });
    it("configure: shared --instruction flag (string mode)", async () => {
        ...
        const result = parseEnvelope(logSpy)["result"];
        expect(result["constraints"]["instructionMode"]).toBe("shared");
    });
    ...
    it("configure: scoped global+project instructions", async () => {
        ...
        const result = parseEnvelope(logSpy)["result"];
        expect(result["constraints"]["instructionMode"]).toBe("scoped");
    });
    it("configure: instructionMode 'none' with only MCP", async () => {
        ...
        const result = parseEnvelope(logSpy)["result"];
        expect(result["constraints"]["instructionMode"]).toBe("none");
    });
    ...
    it("batch: no providers resolved", async () => {
        ...
        const errorSpy = vi.spyOn(console, "error").mockImplementation(() => { });
        vi.spyOn(process, "exit").mockImplementation((() => { throw new Error("x"); }));
        await expect(createProgram().parseAsync(["node", "test", "advanced", "batch", "--agent", "alpha", "--mcp-file", "/tmp/mcp.json"])).rejects.toThrow("x");
        expect(parseEnvelope(errorSpy)["error"].message).toContain("No target providers");
    });
    ...
    it("providers: --details returns full provider objects", async () => {
        ...
        const result = parseEnvelope(logSpy)["result"];
        const data = result["data"];
        expect(data[0]).toHaveProperty("toolName");
    });
    ...
    it("providers: --all flag sets selectionMode to registry", async () => {
        ...
        const result = parseEnvelope(logSpy)["result"];
        expect(result["constraints"]["selectionMode"]).toBe("registry");
    });
    ...
    it("batch: failed with specific error message", async () => {
        ...
        await expect(createProgram().parseAsync(["node", "test", "advanced", "batch", "--agent", "alpha", "--mcp-file", "/tmp/mcp.json"])).rejects.toThrow("x");
        const envelope = parseEnvelope(errorSpy);
        expect(envelope["error"].message).toBe("Specific rollback failure");
    });
    it("batch: failed without error message uses default", async () => {
        ...
        await expect(createProgram().parseAsync(["node", "test", "advanced", "batch", "--agent", "alpha", "--mcp-file", "/tmp/mcp.json"])).rejects.toThrow("x");
        const envelope = parseEnvelope(errorSpy);
        expect(envelope["error"].message).toBe("Batch operation failed.");
    });
    ...
    it("configure: result with undefined instructions", async () => {
        ...
        const result = parseEnvelope(logSpy)["result"];
        const data = result["data"];
        const updates = data["instructionUpdates"];
    ...
    it("readJsonFile: handles non-Error thrown value", async () => {
        ...
        await expect(createProgram().parseAsync(["node", "test", "advanced", "conflicts", "--agent", "alpha", "--mcp-file", "/tmp/bad.json"])).rejects.toThrow("x");
        const envelope = parseEnvelope(errorSpy);
        expect(envelope["error"].message).toContain("Failed to read JSON file");
    });
    ...
    it("readTextInput: handles non-Error thrown value", async () => {
        ...
        const envelope = parseEnvelope(errorSpy);
        expect(envelope["error"].message).toContain("Failed to read content file");
    });
    ...
    it("resolveProviders: throws when some agent IDs are unknown", async () => {
        ...
        const envelope = parseEnvelope(errorSpy);
        expect(envelope["error"].message).toContain("Unknown provider(s)");
    });
    ...
    it("parsePriority: fails on invalid tier", async () => {
        ...
        const envelope = parseEnvelope(errorSpy);
        expect(envelope["error"].message).toContain("Invalid tier: ultra");
    });
    ...
    it("instructions: fails on invalid scope", async () => {
        ...
        const envelope = parseEnvelope(errorSpy);
        expect(envelope["error"].message).toContain("Invalid scope");
    });
    ...
    it("instructions: fails on empty content", async () => {
        ...
        const envelope = parseEnvelope(errorSpy);
        expect(envelope["error"].message).toContain("Instruction content is required");
    });
    // ── batch.ts: no operations ────────────────────────────────────────────
    it("batch: fails when no operations provided", async () => {
        const errorSpy = vi.spyOn(console, "error").mockImplementation(() => { });
        vi.spyOn(process, "exit").mockImplementation((() => { throw new Error("x"); }));
        await expect(createProgram().parseAsync(["node", "test", "advanced", "batch", "--agent", "alpha"])).rejects.toThrow("x");
        const envelope = parseEnvelope(errorSpy);
        expect(envelope["error"].message).toContain("No operations provided");
    });
    ...
    it("conflicts: fails when no providers resolved", async () => {
        ...
        const envelope = parseEnvelope(errorSpy);
        expect(envelope["error"].message).toContain("No target providers");
    });
    ...
    it("configure: fails when provider not found", async () => {
        ...
        const envelope = parseEnvelope(errorSpy);
        expect(envelope["error"].message).toContain("Unknown provider");
    });
});
//# sourceMappingURL=coverage-advanced.test.js.map