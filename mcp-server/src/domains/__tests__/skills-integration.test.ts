...
jest.mock('@cleocode/ct-skills', () => {
  ...
  return {
    listSkills: jest.fn(() => mockSkillEntries.map((s) => s.name)),
    getSkill: jest.fn((name: string) => mockSkillEntries.find((s) => s.name === name)),
    getCoreSkills: jest.fn(() => mockSkillEntries.filter((s) => s.core)),
    getSkillsByCategory: jest.fn((cat: string) =>
      mockSkillEntries.filter((s) => s.category === cat)
    ),
    getSkillDependencies: jest.fn((name: string) => {
      const entry = mockSkillEntries.find((s) => s.name === name);
      return entry?.dependencies ?? [];
    }),
    resolveDependencyTree: jest.fn((names: string[]) => {
    ...
    getDispatchMatrix: jest.fn(() => mockManifest.dispatch_matrix),
    validateSkillFrontmatter: jest.fn((name: string) => ({
      valid: true,
      issues: [],
    })),
    validateAll: jest.fn(() => {
...
describe('Skills Domain Integration', () => {
  ...
  let mockExecutor: jest.Mocked<CLIExecutor>;

  beforeEach(() => {
    mockExecutor = createMockExecutor() as jest.Mocked<CLIExecutor>;
