...
| 23 | Injection `--field` pointers (T12192) | `scripts/lint-injection-field-pointers.mjs` | none (zero-tolerance, scope-printing) | Gates 14/15 assert the VERB half of `cleo <verb> --field <pointer>` resolves; NOTHING asserted the POINTER did — and the pointer is the half that carries data back. Measured 2026-09-14 (gh#1420): CLEO-INJECTION.md told every spawned agent to read `cleo verify` with `--field /data/task/verification`, the nested READ shape, on a verb that returns the FLAT mutate record. It cannot resolve, and gate 14 was green the whole time because the verb exists. Checks every `--field` pointer in the template against its operation's declared `fieldPointers` in `OUTPUT_CONTRACTS`, accepting prefix-extensions (`/data/verification/gates` under `/data/verification`). Verbs whose op has NO explicit contract are SKIPPED and **counted in the output**, not failed — most ops fall through to `genericObjectContract` (`fieldPointers: []`), and failing against that would fail every pointer in the document and get the gate switched off within a day (gh#1353: a gate that will not say what it looked at hides its own scope). Adding a contract to `OUTPUT_CONTRACTS` tightens this gate with no edit to the script. Zero pointers found = exit 2, fails CLOSED. |
