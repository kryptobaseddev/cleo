...
const TEST_ROOT_BRAIN = join(process.cwd(), '.test-memory-engine-compat-brain');
...
describe('Memory Engine Compat', () => {
  beforeEach(() => {
    mkdirSync(join(TEST_ROOT_BRAIN, '.cleo'), { recursive: true });
  });

  afterEach(() => {
    if (existsSync(TEST_ROOT_BRAIN)) {
      rmSync(TEST_ROOT_BRAIN, { recursive: true, force: true });
    }
  });
  ...
  describe('memoryShow', () => {
    it('should return error for unknown ID prefix', async () => {
      ...
      const result = await memoryShow('T001-research', TEST_ROOT_BRAIN);
...
describe('Pipeline Manifest Compat (moved from memory domain)', () => {
  ...
  describe('pipelineManifestList', () => {
    it('should list all entries', () => {
      const result = pipelineManifestList({}, TEST_ROOT);
      expect(result.success).toBe(true);
      expect((result.data as any).total).toBe(3);
    });

    it('should filter by status', () => {
      const result = pipelineManifestList({ status: 'completed' }, TEST_ROOT);
      expect(result.success).toBe(true);
      expect((result.data as any).total).toBe(1);
    });

    it('should filter by topic', () => {
      const result = pipelineManifestList({ topic: 'engine' }, TEST_ROOT);
      expect(result.success).toBe(true);
      expect((result.data as any).total).toBe(2);
    });

    it('should filter by type', () => {
      const result = pipelineManifestList({ type: 'research' }, TEST_ROOT);
      expect(result.success).toBe(true);
      expect((result.data as any).total).toBe(1);
    });

    it('should apply limit', () => {
      const result = pipelineManifestList({ limit: 2 }, TEST_ROOT);
      expect(result.success).toBe(true);
      expect((result.data as any).total).toBe(2);
    });
  });
  ...
  describe('pipelineManifestFind', () => {
    it('should search by title', () => {
      const result = pipelineManifestFind('Research', {}, TEST_ROOT);
      expect(result.success).toBe(true);
      expect((result.data as any).total).toBeGreaterThan(0);
    });

    it('should search by topic', () => {
      const result = pipelineManifestFind('mcp', {}, TEST_ROOT);
      expect(result.success).toBe(true);
      expect((result.data as any).total).toBeGreaterThan(0);
    });

    it('should return error for empty query', () => {
      const result = pipelineManifestFind('', {}, TEST_ROOT);
      expect(result.success).toBe(false);
      expect(result.error?.code).toBe('E_INVALID_INPUT');
    });

    it('should apply confidence threshold', () => {
      const result = pipelineManifestFind('Research', { confidence: 0.5 }, TEST_ROOT);
      expect(result.success).toBe(true);
    });
  });
  ...
  describe('pipelineManifestPending', () => {
    it('should return partial and blocked entries', () => {
      const result = pipelineManifestPending(undefined, TEST_ROOT);
      expect(result.success).toBe(true);
      expect((result.data as any).total).toBe(2);
    });

    it('should filter by epicId', () => {
      const result = pipelineManifestPending('T002', TEST_ROOT);
      expect(result.success).toBe(true);
      expect((result.data as any).total).toBe(1);
    });
  });
  ...
  describe('pipelineManifestStats', () => {
    it('should compute stats', () => {
      const result = pipelineManifestStats(undefined, TEST_ROOT);
      expect(result.success).toBe(true);
    ...
    it('should filter by epicId', () => {
      const result = pipelineManifestStats('T001', TEST_ROOT);
      expect(result.success).toBe(true);
      const data = result.data as any;
  ...
  describe('pipelineManifestLink', () => {
    it('should link task to research entry', () => {
      const result = pipelineManifestLink('T999', 'T001-research', undefined, TEST_ROOT);
      expect(result.success).toBe(true);
      expect((result.data as any).linked).toBe(true);
    });

    it('should handle already linked', () => {
      const result = pipelineManifestLink('T001', 'T001-research', undefined, TEST_ROOT);
      expect(result.success).toBe(true);
      expect((result.data as any).alreadyLinked).toBe(true);
    });

    it('should return error for missing entry', () => {
      const result = pipelineManifestLink('T999', 'T999-missing', undefined, TEST_ROOT);
      expect(result.success).toBe(false);
  ...
  describe('pipelineManifestAppend', () => {
    it('should append valid entry', () => {
      ...
      const result = pipelineManifestAppend(newEntry, TEST_ROOT);
      expect(result.success).toBe(true);
      expect((result.data as any).appended).toBe(true);
    });

    it('should reject invalid entry', () => {
      const result = pipelineManifestAppend({} as any, TEST_ROOT);
      expect(result.success).toBe(false);
  ...
  describe('pipelineManifestArchive', () => {
    it('should archive entries before date', () => {
      const result = pipelineManifestArchive('2026-02-01', TEST_ROOT);
      expect(result.success).toBe(true);
    ...
    it('should return 0 when no entries match', () => {
      const result = pipelineManifestArchive('2020-01-01', TEST_ROOT);
      expect(result.success).toBe(true);
  ...
  describe('pipelineManifestContradictions', () => {
    it('should return empty array when no contradictions', () => {
      const result = pipelineManifestContradictions(TEST_ROOT);
      expect(result.success).toBe(true);
  ...
  describe('pipelineManifestSuperseded', () => {
    it('should return empty array when no superseded entries', () => {
      const result = pipelineManifestSuperseded(TEST_ROOT);
      expect(result.success).toBe(true);
  ...
  describe('pipelineManifestCompact', () => {
    it('should compact manifest successfully', () => {
      const result = pipelineManifestCompact(TEST_ROOT);
      expect(result.success).toBe(true);
  ...
  describe('pipelineManifestValidate', () => {
    it('should validate entries for task', () => {
      const result = pipelineManifestValidate('T001', TEST_ROOT);
      expect(result.success).toBe(true);
    ...
    it('should return error for empty taskId', () => {
      const result = pipelineManifestValidate('', TEST_ROOT);
      expect(result.success).toBe(false);
      expect(result.error?.code).toBe('E_INVALID_INPUT');
    });

    it('should handle task with no entries', () => {
      const result = pipelineManifestValidate('T999', TEST_ROOT);
      expect(result.success).toBe(true);
  ...
  describe('pipelineManifestRead', () => {
    it('should read all entries without filter', () => {
      const result = pipelineManifestRead(undefined, TEST_ROOT);
      expect(result.success).toBe(true);
      expect((result.data as any).total).toBe(3);
    });

    it('should apply filter', () => {
      const result = pipelineManifestRead({ status: 'completed' }, TEST_ROOT);
      expect(result.success).toBe(true);
      expect((result.data as any).total).toBe(1);
    });
  });
