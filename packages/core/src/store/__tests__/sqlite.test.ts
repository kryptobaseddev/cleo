...
describe('SQLite store', () => {
  ...
  it('reopens database from persisted file after close', async () => {
    ...
    await getDb();
    const rows = getNativeDb()!.prepare("SELECT id FROM tasks WHERE id = 'T001'").all() as Array<{
      id: string;
    }>;
  ...
  describe('singleton reset behavior', () => {
    it('resetDbState clears all singleton references', async () => {
      const { getDb, getNativeDb, resetDbState, closeDb: close } = await import('../sqlite.js');
      close();
      ...
      // Reset should close and clear singleton
      resetDbState();

      // Next getDb should create a new instance backed by a fresh native handle
      const db2 = await getDb();
    ...
    it('resetDbState persists data for file-backed node:sqlite', async () => {
      ...
      const db2 = await getDb();
      const rows = await db2.all<{ id: string }>(sql`SELECT id FROM tasks WHERE id = 'T002'`);
      expect(rows).toHaveLength(1);
    });
  });
