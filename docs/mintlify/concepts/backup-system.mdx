...
The atomic write system is implemented in `src/store/atomic.ts`:

```typescript
// Atomic write with validation and backup
await atomicWrite(filePath, content);

// SQLite operations use WAL mode for atomicity
// See src/store/database.ts
```
...
If an atomic rename fails, the system automatically restores from the numbered backup. SQLite operations use WAL (Write-Ahead Logging) mode for crash-safe transactions.
...
<Tabs>
  <Tab title="Tracked Files">
    ...
    - `.cleo/project-context.json` - Detected project type
  </Tab>

  <Tab title="Ignored Files">
    Data and backup files are NOT tracked:

    - `.cleo/tasks.db` - SQLite database (and WAL/SHM files)
...
|----------|---------|---------|
| `templates/git-hooks/pre-commit` | Pre-commit validation | Schema snapshot checks |
| `templates/git-hooks/commit-msg` | Commit message enforcement | Task ID reference (T####) |
...
<CardGroup cols={2}>
  ...
  <Card title="Upgrade Command" icon="database" href="/commands/upgrade">
    Schema migrations and data repairs
  </Card>
</CardGroup>
