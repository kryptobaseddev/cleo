...
import { addMigrationError, addMigrationWarning, canResumeMigration, clearMigrationState, completeMigration, createMigrationState, failMigration, getMigrationSummary, isMigrationInProgress, loadMigrationState, updateMigrationPhase, updateMigrationProgress, updateMigrationState, verifySourceIntegrity, } from '../state.js';
describe('migration state tracking', () => {
    ...
    describe('updateMigrationState', () => {
        ...
        it('should throw if no state exists', async () => {
            await expect(updateMigrationState(cleoDir, { phase: 'import' })).rejects.toThrow('No migration state exists');
        });
    ...
    describe('updateMigrationPhase', () => {
        ...
        it('should update through all phases', async () => {
            await createMigrationState(cleoDir);
            const phases = ['backup', 'validate', 'import', 'verify', 'cleanup', 'complete'];
    ...
    describe('verifySourceIntegrity', () => {
        ...
        it('should detect changed files', async () => {
            await createMigrationState(cleoDir);
            // Modify file
            writeFileSync(join(cleoDir, 'todo.json'), JSON.stringify({ tasks: [{ id: '1', title: 'Modified' }] }));
            const result = await verifySourceIntegrity(cleoDir);
            expect(result.valid).toBe(false);
