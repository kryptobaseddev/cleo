...
function setupMockAccessor(sessions, tasks = makeMockTasks()) {
    const mockAccessor = {
...
describe('computeHandoff scope resolution', () => {
    ...
    it('falls back to global when epic scope has no rootTaskId or epicId', async () => {
        ...
        delete session.scope.epicId;
        delete session.scope.rootTaskId;
        setupMockAccessor([session]);
        const handoff = await computeHandoff('/fake/project', {
            sessionId: 'session-test-1',
        });
    ...
    it('includes explicitTaskIds in scope set', async () => {
        ...
        session.scope.explicitTaskIds = ['T200', 'T300'];
        setupMockAccessor([session]);
        const handoff = await computeHandoff('/fake/project', {
            sessionId: 'session-test-1',
        });
    ...
    it('throws when session not found', async () => {
        setupMockAccessor([]);
        await expect(computeHandoff('/fake/project', { sessionId: 'session-nonexistent' })).rejects.toThrow(/not found/);
    });
});
// ---------------------------------------------------------------------------
