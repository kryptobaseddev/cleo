...
| `cleoTaskId` | `string \| null` | Yes | Mapped CLEO task ID, or null if the task is new / unmatched. |
...
| Property | Type | Required | Description |
|----------|------|----------|-------------|
| `injectedTaskIds` | `string[]` | Yes | CLEO task IDs that were injected into the provider's task list. |
| `injectedPhase` | `string \| undefined` | No | Optional phase context when tasks were injected. |
| `taskMetadata` | `Record<string, { phase?: string \| undefined; }> \| undefined` | No | Per-task metadata at injection time. |
| `lastSyncAt` | `string \| undefined` | No | ISO timestamp of the last successful reconciliation. |
...
| Property | Type | Required | Description |
|----------|------|----------|-------------|
| `providerId` | `string` | Yes | Provider ID (e.g. 'claude-code', 'cursor'). |
...
| `externalId` | `string` | Yes | The external task that triggered this action. |
...
| `summary` | `{ completed: number; activated: number; created: number; removed: number; skipped: number; conflicts: number; applied: number; }` | Yes | Summary counts. |
| `sessionCleared` | `boolean` | Yes | Whether sync session state was cleared after apply. |
...
| `pushTaskState` | `((tasks: readonly { id: string; title: string; status: string; }[], projectDir: string) => Promise<void>) \| undefined` | No | Optionally push CLEO task state back to the provider. Not all providers support bidirectional sync. |
| `cleanup` | `((projectDir: string) => Promise<void>) \| undefined` | No | Clean up provider-specific sync artifacts (e.g. state files). |
...
| `taskSync` | `AdapterTaskSyncProvider \| undefined` | No | taskSync |
...
| Property | Type | Required | Description |
|----------|------|----------|-------------|
| `direction` | `"push" \| "pull" \| "bidirectional" \| undefined` | No | direction |
...
|----------|------|----------|-------------|
| `content` | `string` | Yes | content |
| `status` | `TodoWriteItemStatus` | Yes | status |
...
|----------|------|----------|-------------|
| `taskId` | `string` | Yes | taskId |
| `action` | `TodoWriteChangeAction` | Yes | action |
...
| `sessionCleared` | `boolean \| undefined` | No | sessionCleared |
