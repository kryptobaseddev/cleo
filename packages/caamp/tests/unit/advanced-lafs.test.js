...
import { LAFSCommandError, emitError, emitSuccess, runLafsCommand, } from "../../src/commands/advanced/lafs.js";
describe("advanced/lafs", () => {
    ...
    it("constructs LAFS command errors with inferred category", () => {
        const error = new LAFSCommandError("E_ADVANCED_VALIDATION_PRIORITY", "Invalid tier", "Use high, medium, or low");
    ...
    it("emits structured registered LAFS errors", () => {
        ...
        mocks.isRegisteredErrorCode.mockImplementation((code) => code === "E_ADVANCED_VALIDATION_PRIORITY");
        emitError("advanced.batch", new LAFSCommandError("E_ADVANCED_VALIDATION_PRIORITY", "Invalid tier", "Use one of: high, medium, low.", true, { field: "tier" }));
    ...
    it("falls back to internal code for unknown command errors", () => {
        const errorSpy = vi.spyOn(console, "error").mockImplementation(() => { });
        emitError("advanced.batch", new LAFSCommandError("E_CUSTOM_NOT_FOUND", "Missing", "Check inputs", false));
        const output = JSON.parse(String(errorSpy.mock.calls[0]?.[0] ?? "{}"));
    ...
    it("runs commands through success and failure envelope paths", async () => {
        ...
        await expect(runLafsCommand("advanced.batch", "standard", async () => Promise.reject(new LAFSCommandError("E_ADVANCED_CONFLICT", "conflict", "retry", false)))).rejects.toThrow("process-exit");
        expect(errorSpy).toHaveBeenCalledTimes(1);
        expect(exitSpy).toHaveBeenCalledWith(1);
    });
});
//# sourceMappingURL=advanced-lafs.test.js.map