...
| ID-only pipeline | `--output id` | `cleo list --parent EPIC --output id --limit 0 \| while read c; do …; done` — **`--limit 0` is REQUIRED**: without it `list` silently stops at 10 while `--output count` reports the true total |
