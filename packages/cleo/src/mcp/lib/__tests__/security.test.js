...
describe('Security Module', () => {
    describe('sanitizeTaskId', () => {
        ...
        it('should throw SecurityError with correct code', () => {
            try {
                ...
                expect(e.code).toBe('E_INVALID_TASK_ID');
                expect(e.field).toBe('taskId');
            }
        });
        it('should reject non-string inputs', () => {
            expect(() => sanitizeTaskId(123)).toThrow(SecurityError);
            expect(() => sanitizeTaskId(null)).toThrow(SecurityError);
        });
    });
    describe('sanitizePath', () => {
        ...
        it('should throw SecurityError with E_PATH_TRAVERSAL code', () => {
            try {
                ...
                expect(e).toBeInstanceOf(SecurityError);
                expect(e.code).toBe('E_PATH_TRAVERSAL');
            }
        });
        it('should reject non-string inputs', () => {
            expect(() => sanitizePath(123, projectRoot)).toThrow(SecurityError);
        });
    ...
    describe('sanitizeContent', () => {
        ...
        it('should throw SecurityError with E_CONTENT_TOO_LARGE', () => {
            try {
                ...
                expect(e).toBeInstanceOf(SecurityError);
                expect(e.code).toBe('E_CONTENT_TOO_LARGE');
            }
        });
        ...
        it('should reject non-string inputs', () => {
            expect(() => sanitizeContent(123)).toThrow(SecurityError);
        });
    });
    describe('validateEnum', () => {
        ...
        it('should include allowed values in error message', () => {
            try {
                ...
                expect(e.code).toBe('E_INVALID_ENUM');
                expect(e.field).toBe('status');
            }
        });
        it('should reject non-string inputs', () => {
            expect(() => validateEnum(123, allowed, 'status')).toThrow(SecurityError);
        });
    });
    ...
    describe('sanitizeParams', () => {
        ...
        it('should skip null and undefined values', () => {
            const result = sanitizeParams({ taskId: null, status: undefined });
        ...
        it('should reject path traversal in params', () => {
            const paramRoot = join(tmpdir(), 'cleo-param-test-project');
            expect(() => sanitizeParams({ path: '../../../etc/passwd' }, paramRoot)).toThrow(SecurityError);
        });
    });
