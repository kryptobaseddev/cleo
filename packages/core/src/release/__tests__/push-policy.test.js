...
describe('pushRelease push policy', () => {
    it('should throw when push is disabled and no explicit push flag', async () => {
        ...
        await expect(pushRelease('v1.0.0', undefined, testDir)).rejects.toThrow('Push is disabled by config');
    });
    it('should throw when push is disabled even with remote override', async () => {
        ...
        await expect(pushRelease('v1.0.0', 'upstream', testDir)).rejects.toThrow('Push is disabled by config');
    });
    it('should allow push when disabled but explicitPush is true', async () => {
        ...
        await expect(pushRelease('v1.0.0', undefined, testDir, { explicitPush: true })).rejects.toThrow(/git/i);
        // Verify it did NOT throw the config error
        try {
            await pushRelease('v1.0.0', undefined, testDir, { explicitPush: true });
        }
        catch (e) {
            expect(e.message).not.toContain('disabled by config');
        }
    });
    ...
    it('should use remote from config when no explicit remote', async () => {
        ...
        try {
            ...
            expect(e.message).not.toContain('disabled by config');
            expect(e.message).not.toContain('not in allowed branches');
        }
    });
...
//# sourceMappingURL=push-policy.test.js.map