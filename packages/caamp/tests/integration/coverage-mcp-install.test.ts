...
describe("coverage: mcp install", () => {
  beforeEach(() => {
    ...
    mocks.getProvider.mockImplementation((name: string) =>
      name === "claude-code" ? provider : undefined,
    );
  ...
  it("parses headers from --header flag", async () => {
    ...
    expect(mocks.buildServerConfig).toHaveBeenCalledWith(
      expect.anything(),
      "http",
  ...
  it("exits with human error when no providers found", async () => {
    ...
    await expect(
      program.parseAsync(["node", "test", "install", "@acme/mcp", "--all", "--human"]),
  ...
  it("exits with format conflict error", async () => {
    ...
    await expect(
      program.parseAsync(["node", "test", "install", "@acme/mcp", "--json", "--human"]),
    ).rejects.toThrow("process-exit");
  ...
  it("installs to global scope with --global flag in human mode", async () => {
    ...
    expect(mocks.installMcpServerToAll).toHaveBeenCalledWith(
      expect.anything(), "acme", expect.anything(), "global",
    );
  });
