...
describe("coverage: scanner.ts line scanning detail branches", () => {
  ...
  it("scanFile with custom rules - various severities (lines 53, 59, 69)", async () => {
    ...
    const { scanFile } = await import("../../src/core/skills/audit/scanner.js");
    const result = await scanFile("/test/SKILL.md", [
      { id: "T1", name: "t1", description: "c", pattern: /danger/, severity: "critical" as const, category: "injection" },
      { id: "T2", name: "t2", description: "h", pattern: /word/, severity: "high" as const, category: "injection" },
      { id: "T3", name: "t3", description: "m", pattern: /here/, severity: "medium" as const, category: "injection" },
      { id: "T4", name: "t4", description: "l", pattern: /nope/, severity: "low" as const, category: "injection" },
      { id: "T5", name: "t5", description: "i", pattern: /nah/, severity: "info" as const, category: "injection" },
    ]);
  ...
  it("scanDirectory with mixed entry types (line 104 isSymbolicLink branch)", async () => {
    mockExistsSyncFn.mockImplementation((path: string) => {
      ...
      if (path.includes("dir1") && path.endsWith("SKILL.md")) return true;
      if (path.includes("link1") && path.endsWith("SKILL.md")) return true;
      return false;
    });
