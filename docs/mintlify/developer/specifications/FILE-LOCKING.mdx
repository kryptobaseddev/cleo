...
<Warning>
**ARCHIVED** — This specification describes the legacy Bash `flock`-based locking system. The TypeScript codebase uses `src/store/lock.ts` with a different implementation. Retained for historical reference only.
</Warning>
...
- Locking `todo.json` MUST NOT block operations on `todo-log.jsonl`
...
|----------|---------|-------------|
| P0 (Critical) | add.sh, update.sh, complete.sh | MUST use locking |
