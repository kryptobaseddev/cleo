...
describe("SkillLibrary via buildLibraryFromFiles", () => {
    ...
    beforeEach(() => {
        ...
        writeFileSync(join(fixtureRoot, "skills", "dep-skill", "SKILL.md"), "# Dep Skill\nThis skill depends on test-skill.");
        // Manifest
        ...
        writeFileSync(join(fixtureRoot, "skills", "_shared", "helper.md"), "# Helper\nShared resource content.");
        // Protocols
        ...
        writeFileSync(join(fixtureRoot, "profiles", "minimal.json"), JSON.stringify({
            name: "minimal",
            description: "Minimal profile",
            skills: ["test-skill"],
            includeProtocols: [],
        }));
        writeFileSync(join(fixtureRoot, "profiles", "full.json"), JSON.stringify({
            name: "full",
...
describe("SkillLibrary protocol path discovery", () => {
    ...
    it("falls back to skills/protocols/ when root protocols/ is absent", () => {
        ...
        mkdirSync(join(fixtureRoot, "skills", "protocols"), { recursive: true });
        writeFileSync(join(fixtureRoot, "skills", "protocols", "consensus.md"), "# Consensus Protocol");
        const library = buildLibraryFromFiles(fixtureRoot);
        const protocols = library.listProtocols();
...
//# sourceMappingURL=skill-library.test.js.map