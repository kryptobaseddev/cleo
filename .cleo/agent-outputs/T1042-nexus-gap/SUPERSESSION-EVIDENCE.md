...
**Updated**: 2026-05-04 — T1042 Close-Out Audit (architecture subagent)  
**Original Task**: T1736 — D3 Validation Campaign  
**Date**: 2026-05-04  
**Repository**: /mnt/projects/openclaw  
**Commit**: d2e2d971b6d23e8b727250a0d76cbe41b8f4e1f4  
**cleo version at T1736**: 2026.5.16 → **cleo version at T1042 audit**: 2026.5.17  
**gitnexus version**: 1.6.3  

---

## T1042 Close-Out Audit Summary (2026-05-04, v2026.5.17)

**Updated by**: far-exceed-final-audit.md architecture subagent  
**Full audit doc**: `.cleo/rcasd/T1042/architecture/far-exceed-final-audit.md`

### Swift Decision (Owner Question)

**Decision**: Do NOT add gitnexus O(m²) Swift implicit wiring. Consider explicit Swift import extraction as a separate task.

**Rationale**: gitnexus's 383,780 Swift IMPORTS are 98.2% of its total IMPORTS, generated by SPM module visibility (every file in a target "imports" every other file). This is not semantic import data — it is structural noise. Adding it to cleo would inflate the graph 54x on Swift-heavy codebases with no accuracy improvement. Cleo already leads non-Swift IMPORTS by 7.7x (54,644 vs 7,113). Explicit Swift `import ModuleName` parsing (O(files × actual_imports) rather than O(m²)) is the correct approach if Swift support is needed. See T1838.

### Updated Stats (v2026.5.17 after T1733 AFL Leiden swap)

| Metric | gitnexus 1.6.3 | cleo nexus 2026.5.17 | Status |
|--------|----------------|----------------------|--------|
| Total nodes | 251,832 | 67,451 | -73.2% (mostly Const/Variable noise in gitnexus) |
| Total relations | 748,581 | 176,968 | -76.4% |
| CALLS edges | 63,267 | 99,122 | **+57% cleo leads** |
| non-Swift IMPORTS | 7,113 | 54,644 | **+7.7x cleo leads** |
| Communities (Leiden) | 7,183 | 3,179 (modularity=0.594) | Leiden quality now proven |
| Full analyze time | ~18 minutes | 76 seconds | **cleo 14x faster** |
| Context query p50 | 0.67s | 2.4s | 3.6x behind — T1834 |
| Augmenter | 0.04s, returns content | 1.8s, returns empty | Critical bug — T1832 |
| Wiki | 0 (needs API key) | 0 (query bug) | Both broken, T1833 |
| search-code | not available | returns 0 results | Bug — T1832 |
| DEFINES edges | 223,627 | 0 | Gap — T1836 |
| ACCESSES edges | 8,293 | 0 | Gap — T1837 |

### New Gaps Found in This Audit (T1832-T1839)

| Task | Description | Severity |
|------|-------------|---------|
| T1832 | augment.ts SQL column names wrong — `target_node_id`/`source_node_id`/`relation_type` should be `target_id`/`source_id`/`type` | Critical (blocks augment + search-code) |
| T1833 | wiki community query groups by NULL community_id on community nodes — 0 communities rendered | High (blocks wiki) |
| T1834 | Full-table scan in context/impact/clusters — loads 89k+ rows from all projects | High (3.6x query slowdown) |
| T1835 | 4,668 test project entries in shared nexus.db (test pollution) | Medium |
| T1836 | DEFINES edges never emitted despite type declared | High |
| T1837 | ACCESSES edges missing (no access-extractor.ts) | Medium |
| T1838 | Swift extractor decision — explicit imports vs deliberate omission | Medium (decision) |
| T1839 | FTS5 virtual table needed for real BM25 search-code/augment | High |

### Close Recommendation

**NOT READY TO CLOSE T1042.** Minimum gate: T1832 + T1833 + T1834 + T1836 + T1839 must ship. When those 5 ship, cleo nexus will genuinely far-exceed gitnexus on: analysis speed (14x), CALLS (+57%), non-Swift IMPORTS (+7.7x), TypeAlias (278x), Interface (9x), Living Brain (unique), Route/API (unique), cross-substrate traversal (unique).
