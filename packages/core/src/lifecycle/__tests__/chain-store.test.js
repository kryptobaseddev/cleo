...
let tempDir;
function makeMinimalChain(overrides) {
    return {
        id: 'test-chain',
        name: 'Test Chain',
        version: '1.0.0',
...
function makeThreeStageChain(overrides) {
...
describe('WarpChain chain-store', () => {
    ...
    it('rejects illegal transition and preserves instance state', async () => {
        ...
        await addChain(chain, tempDir);
        const instance = await createInstance({ chainId: 'three-stage-chain', epicId: 'T9999' }, tempDir);
        ...
        await expect(advanceInstance(instance.id, 'stage-c', [], tempDir)).rejects.toThrow('Invalid stage transition: "stage-a" -> "stage-c" is not allowed in chain "three-stage-chain"');
    ...
    it('rejects terminal-state transitions and unknown stage transitions', async () => {
        ...
        await addChain(chain, tempDir);
        const instance = await createInstance({ chainId: 'test-chain', epicId: 'T9999' }, tempDir);
        await expect(advanceInstance(instance.id, 'stage-z', [], tempDir)).rejects.toThrow('Invalid stage transition: stage "stage-z" is not defined in chain "test-chain"');
        const db = await getDb(tempDir);
        await db
            .update(warpChainInstances)
            .set({ status: 'completed' })
            .where(eq(warpChainInstances.id, instance.id));
        await expect(advanceInstance(instance.id, 'stage-b', [], tempDir)).rejects.toThrow(`Chain instance "${instance.id}" cannot advance from terminal status "completed"`);
...
//# sourceMappingURL=chain-store.test.js.map