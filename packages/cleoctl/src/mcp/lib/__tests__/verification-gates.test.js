...
import { createVerificationGate, GATE_SEQUENCE, GateLayer, getWorkflowGateDefinition, isValidWorkflowGateName, WORKFLOW_GATE_DEFINITIONS, WORKFLOW_GATE_SEQUENCE, WorkflowGateName, WorkflowGateTracker, } from '../verification-gates.js';
describe('VerificationGate', () => {
    ...
    describe('Layer 3: Referential Validation', () => {
        ...
        it('should fail on invalid dependency reference', async () => {
            ...
            const result = await gate.verifyOperation(context);
            expect(result.passed).toBe(false);
            expect(result.layers[GateLayer.REFERENTIAL].violations[0].code).toBe('E_INVALID_DEPENDENCY_REF');
        });
    ...
    describe('Layer 4: Protocol Validation', () => {
        ...
        it('should validate manifest status enum', async () => {
            ...
            const result = await gate.verifyOperation(context);
            expect(result.passed).toBe(false);
            expect(result.layers[GateLayer.PROTOCOL].violations.some((v) => v.code === 'E_INVALID_MANIFEST_STATUS')).toBe(true);
        });
    });
    ...
    describe('Section 7.2: Gate Definitions', () => {
        it('should assign correct agents to gates', () => {
            ...
            for (const [gateName, agent] of Object.entries(agentMap)) {
                const def = getWorkflowGateDefinition(gateName);
...
//# sourceMappingURL=verification-gates.test.js.map