...
import { getPipelineStageOrder, isPipelineTransitionForward, isValidPipelineStage, resolveDefaultPipelineStage, TASK_PIPELINE_STAGES, validatePipelineStage, validatePipelineTransition, } from '../pipeline-stage.js';
...
describe('validatePipelineTransition', () => {
    ...
    it('throws for backward transitions', () => {
        expect(() => validatePipelineTransition('implementation', 'research')).toThrow('cannot move backward');
        expect(() => validatePipelineTransition('testing', 'specification')).toThrow('cannot move backward');
    });
...
describe('addTask pipeline stage auto-assignment', () => {
    ...
    it('assigns implementation to a standalone task by default', async () => {
        const result = await addTask({ title: 'Standalone task', description: 'A standalone task without explicit stage' }, env.tempDir, accessor);
    ...
    it('rejects invalid pipelineStage on creation', async () => {
        ...
        }, env.tempDir, accessor)).rejects.toThrow('Invalid pipeline stage');
    });
...
describe('updateTask pipeline stage transitions', () => {
    ...
    it('allows forward stage transition', async () => {
        ...
        const result = await updateTask({ taskId: 'T001', pipelineStage: 'validation' }, env.tempDir, accessor);
    ...
    it('allows same-stage transition (no-op)', async () => {
        ...
        const result = await updateTask({ taskId: 'T001', pipelineStage: 'implementation' }, env.tempDir, accessor);
        expect(result.task.pipelineStage).toBe('implementation');
    });
    it('rejects backward stage transition', async () => {
        ...
        await expect(updateTask({ taskId: 'T001', pipelineStage: 'implementation' }, env.tempDir, accessor)).rejects.toThrow('cannot move backward');
    });
    it('rejects invalid stage on update', async () => {
        ...
        await expect(updateTask({ taskId: 'T001', pipelineStage: 'not_real' }, env.tempDir, accessor)).rejects.toThrow('Invalid pipeline stage');
    });
...
//# sourceMappingURL=pipeline-stage.test.js.map