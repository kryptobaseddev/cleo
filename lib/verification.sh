...
if [[ -f "$_VERIFICATION_LIB_DIR/exit-codes.sh" ]]; then
    source "$_VERIFICATION_LIB_DIR/exit-codes.sh"
fi

# Source config (required for verification settings)
if [[ -f "$_VERIFICATION_LIB_DIR/config.sh" ]]; then
    source "$_VERIFICATION_LIB_DIR/config.sh"
fi
