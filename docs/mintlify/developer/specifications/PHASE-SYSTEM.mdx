---
title: "Phase System"
description: "Project-level phase lifecycle management with dual-level model, wave computation, and permissive cross-phase work"
icon: "file-code"
---
...
### 2.4 Active Task Integration

The active task state syncs with project phase:
...
- `log_phase_rollback()` in todo-log.jsonl
...
4. Update active task's `currentPhase` if matches
...
5. Action logged to todo-log.jsonl
...
- New tasks inherit phase from active task or `defaults.phase`
