...
const errors = [];
let source;
try { source = readFileSync(PROJECT_FILE, 'utf-8'); } catch { errors.push(`File not found: ${PROJECT_FILE}`); finish(); }
if (!source.includes("from '@cleocode/contracts'")) errors.push('Missing import from @cleocode/contracts');
...
for (const s of subs) { if (!source.includes(`'${s}'`) && !source.includes(`"${s}"`)) errors.push(`Missing subcommand: ${s}`); }
...
function finish() {
  ...
  console.log('PASSED: All three verbs wrap output in RenderableEnvelope (kind: section).'); process.exit(0);
}
