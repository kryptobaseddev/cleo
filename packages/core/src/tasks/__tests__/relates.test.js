...
describe('relates.ts addRelation persistence (T5168)', () => {
    ...
    it('should throw on non-existent source task', async () => {
        await expect(addRelation('T999', 'T002', 'blocks', 'test', testDir, accessor)).rejects.toThrow('T999 not found');
    });
    it('should throw on non-existent target task', async () => {
        await expect(addRelation('T001', 'T999', 'blocks', 'test', testDir, accessor)).rejects.toThrow('T999 not found');
    });
...
//# sourceMappingURL=relates.test.js.map