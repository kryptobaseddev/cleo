...
import { createTestEnvironment, destroyTestEnvironment, } from './test-environment.js';
...
class CLIExecutor {
    ...
    timeout;
    maxRetries;
    constructor(cliPath, timeout = 60000, maxRetries = 1) {
     ...
     */
    mapToCliCommand(domain, operation) {
        ...
        if (domain === 'session') {
            if (operation === 'focus')
                return { command: 'focus', addOperationAsSubcommand: false };
    ...
    async execute(options) {
        ...
        for (const [key, value] of Object.entries(flags)) {
            ...
            parts.push(`--${key}`, this.escapeArg(value));
        }
        ...
        for (let attempt = 0; attempt <= retries; attempt++) {
            ...
            try {
                ...
                const result = this.parseOutput(stdout, stderr, 0);
    ...
    parseOutput(stdout, stderr, exitCode) {
        ...
        if (!rawOutput) {
            if (exitCode === 0) {
                return { success: true, data: undefined, stdout, stderr, exitCode };
            }
            return {
                success: false,
        ...
        try {
            ...
            // Non-JSON output
            if (exitCode === 0) {
                return { success: true, data: stdout.trim(), stdout, stderr, exitCode };
            }
            return {
                success: false,
                error: {
 ...
 */
function createExecutor(cliPath, timeout, maxRetries) {
...
export function unwrapPayload(data) {
    if (data &&
        ...
        return unwrapPrimaryField(payload);
    }
    return data;
}
/**
 ...
 */
function unwrapPrimaryField(payload) {
    if (!payload || typeof payload !== 'object' || Array.isArray(payload)) {
        return payload;
    }
...
class WrappedExecutor {
    executor;
    defaultCwd;
    constructor(executor, defaultCwd) {
...
export async function setupIntegrationTest() {
    ...
    if (!canConnect) {
        throw new Error(`Cannot connect to CLEO CLI at ${cliPath}. ` +
 ...
 */
export async function cleanupIntegrationTest(context) {
    if (!context) {
        return;
    }
 ...
 */
export async function createTestTask(context, title, description, options) {
    ...
    if (!result.success || !taskId) {
        throw new Error(`Failed to create test task: ${result.error?.message || 'No task ID in response'}` +
 ...
 */
export async function createTestEpic(context, title, description) {
 ...
 */
export async function startTestSession(context, epicId) {
    const result = await context.executor.execute({
        domain: 'session',
        operation: 'start',
        flags: {
 ...
 */
export async function getCleoVersion(executor) {
    return executor.getVersion();
}
/**
 * Check if a task exists
 */
export async function taskExists(executor, taskId, cwd) {
 ...
 */
export function createManifestEntry(taskId, overrides) {
    return {
 ...
 */
export function verifyResponseFormat(response, _expectedGateway, _expectedDomain, _expectedOperation) {
