...
<CardGroup cols={2}>
  ...
  <Card title="BASE-005" icon="crosshairs">
    **MUST** set focus before starting work
...
<AccordionGroup>
  ...
  <Accordion title="Skipping Focus">
    **Problem**: Violates BASE-005, breaks session tracking

    **Solution**: Always `cleo focus set` before work
...
<Steps>
  <Step title="Focus Set">
    Task focus was set via `cleo focus set {{TASK_ID}}`
