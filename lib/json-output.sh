...
if [[ -f "$_JSON_OUTPUT_LIB_DIR/version.sh" ]]; then
    # shellcheck source=lib/version.sh
    source "$_JSON_OUTPUT_LIB_DIR/version.sh"
fi

# Source platform compatibility for get_iso_timestamp (Layer 0)
if [[ -f "$_JSON_OUTPUT_LIB_DIR/platform-compat.sh" ]]; then
    # shellcheck source=lib/platform-compat.sh
    source "$_JSON_OUTPUT_LIB_DIR/platform-compat.sh"
fi
