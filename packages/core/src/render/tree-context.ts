/**
 * Tree-rendering invocation context.
 *
 * Singleton that carries per-invocation flags for `cleo tree` and any command
 * that delegates to the tree/orchestration renderers. Set by the CLI command
 * handler before dispatching; read by the orchestration tree renderer when
 * deciding whether to inline dependency or blocker chains.
 *
 * The pattern mirrors `format-context.ts` and `field-context.ts`: a
 * module-level store set once at command entry and read by the renderer so
 * that the renderer signature (`data`, `quiet`) stays stable.
 *
 * Originally lived under `packages/cleo/src/cli/tree-context.ts`. Migrated to
 * `@cleocode/core/render` (T10131 — B6) so the orchestration renderers can
 * be hosted in core without inverting the package dependency.
 *
 * @task T1205
 * @task T1206
 * @task T10131
