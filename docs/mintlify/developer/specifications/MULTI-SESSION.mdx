...
description: "Concurrent LLM agent coordination with scoped isolation, per-scope validation, and conflict detection"
icon: "file-code"
---
...
This specification defines the multi-session architecture for cleo, enabling multiple concurrent LLM agents to work on different task groups within the same project. Each session maintains isolated active task state and scope boundaries while sharing the underlying task data.
...
| **Per-Scope Validation** | One active task constraint applies per-scope, not globally |
...
├── todo-log.jsonl          # Audit log (session-aware)
...
| Both | Active task changes, task completion | Sequential (sessions → todo) |
...
multiSession.enabled = true
  ...
  └── Per-session active task in sessions.json
```
...
validate_new_session(new_scope):
  for session in active_sessions:
    ...
    switch overlap.type:
      case "hard":
        ERROR: "Task {id} already active in session {session.id}"
...
### 3.3 Task Activation Validation

When starting a task within a session:
...
A session MUST have an active task to start. This ensures every session has a clear purpose and trackable work context.

```bash
# Explicit active task (REQUIRED: one of these)
cleo session start --scope epic:T001 --focus T005 --name "Auth impl"

# Auto-select: picks highest priority pending task in scope
cleo session start --scope epic:T001 --auto-focus --name "Auth impl"

# ERROR: No active task specified
cleo session start --scope epic:T001
# → ERROR (E_ACTIVE_TASK_REQUIRED): Session requires --focus <task-id> or --auto-focus
```

**Active Task Requirement Rationale**:
- Sessions track "what is being worked on" - a session without an active task has no purpose
...
**Auto-Select Active Task** (when `--auto-focus` specified):
...
4. **Validate active task exists and is in scope**
5. **If `--auto-focus`: select active task per algorithm above**
6. Generate session ID
7. Create session entry with status="active" and active task (`focus.currentTask`) set
8. Update `_meta.totalSessionsCreated`
9. Lock todo.json
10. **Set active task status = "active"**
11. Save both files
12. Update todo.json `_meta.activeSessionCount`
13. Log `session_start` with scope and active task context
14. Unlock both
...
3. Preserve active task state (not cleared)
...
6. Restore active task from preserved state
...
5. Clear active task in todo.json if needed
...
## Part 5: Active Task Management

### 5.1 Per-Session Active Task

Each session maintains an independent active task:
...
```json
{
  "focus": {
    ...
    "focusHistory": [
      ...
      {"taskId": "T005", "timestamp": "...", "action": "started"}
    ]
  }
}
```

### 5.2 Active Task Commands

```bash
# Start task (uses current session or env var)
cleo start T005

# Explicit session
cleo start T005 --session session_20251227_...

# Stop active task
cleo stop

# Show active task (session-aware)
cleo current
```
...
```
sessions.json → todo.json → todo-log.jsonl
```
...
```bash
# Example: Start task / set active task (requires both files)
...
# Explicit session
cleo start T005 --session <id>
cleo current --session <id>
```
...
2. If `._meta.activeSession` exists:
   ...
   - Move `.focus` contents to `session.focus` (active task data)
...
- `.focus` field in todo.json preserved for fallback
...
### Phase 4: Active Task Integration
...
# Each has own active task tracking
```
