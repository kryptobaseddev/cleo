...
describe('reconcileJournal — Scenario 2 (T632: Sub-case A/B stale journal discrimination)', () => {
  ...
  it('Sub-case B: marks migrations applied only when their DDL targets already exist (T632 root-cause fix)', async () => {
    ...
    try {
      ...
      // Always close the SQLite connection before afterEach cleanup.
      // On Windows, SQLite WAL sidecar files (.db-shm, .db-wal) remain locked
      // until the connection is explicitly closed, causing EBUSY on rm().
      nativeDb.close();
    }
  });
