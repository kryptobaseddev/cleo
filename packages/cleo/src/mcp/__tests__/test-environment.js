 ...
 */
async function cleoExec(cliPath, cwd, command) {
    try {
...
export async function createTestEnvironment() {
    ...
    const projectCli = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', '..', '..', 'dist', 'cli', 'index.js');
    const defaultCliPath = await fs.access(projectCli).then(() => `node ${projectCli}`, () => 'cleo');
    ...
    try {
        ...
        const epicResult = await cleoExec(cliPath, tmpBase, 'add "Test Epic" --description "Epic for integration testing" --json');
        ...
        try {
            ...
            throw new Error(`Failed to parse epic creation output (exit ${epicResult.exitCode}): ` +
                `stdout=${JSON.stringify(epicResult.stdout)}, stderr=${JSON.stringify(epicResult.stderr)}`);
        }
        const epicId = epicParsed.result?.task?.id ??
        ...
        for (const task of testTasks) {
            const result = await cleoExec(cliPath, tmpBase, `add "${task.title}" --description "${task.desc}" --parent ${epicId} --json`);
            ...
            try {
                ...
                throw new Error(`Failed to parse task creation output for "${task.title}" (exit ${result.exitCode}): ` +
                    `stdout=${JSON.stringify(result.stdout)}, stderr=${JSON.stringify(result.stderr)}`);
            }
            const taskId = parsed.result?.task?.id ??
                parsed.result?.id ??
                parsed.data?.task?.id ??
                parsed.data?.id ??
                parsed.task?.id ??
 ...
 */
export async function destroyTestEnvironment(env) {
