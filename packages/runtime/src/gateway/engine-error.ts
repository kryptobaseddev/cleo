/**
 * Centralized engine error helper for the transport-agnostic gateway.
 *
 * Every domain handler (and the engine barrel) imports `engineError()` /
 * `engineSuccess()` from this module to produce consistently typed error
 * results with correct exit codes and pino logging.
 *
 ...
 *
 * @remarks
 * Relocated from `packages/cleo/src/dispatch/engines/_error.ts` into
 * `@cleocode/runtime/gateway` (R3-K1 · T11455 · SG-RUNTIME-UNIFICATION) so the
 * runtime owns the shared handler dependencies and can assemble the handler map
 * without importing `@cleocode/cleo`. A thin re-export shim remains at the old
 * `engines/_error.ts` path so in-package import sites compile unchanged.
 *
