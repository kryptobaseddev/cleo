 ...
 */
export function resolveNexusMigrationsFolder() {
 ...
 * @task T5365
 */
async function runNexusMigrations(nativeDb, db) {
    ...
    if (tableExists(nativeDb, 'project_registry') && !tableExists(nativeDb, '__drizzle_migrations')) {
        ...
        if (baseline) {
            nativeDb
                .prepare(`CREATE TABLE IF NOT EXISTS "__drizzle_migrations" (id SERIAL PRIMARY KEY, hash text NOT NULL, created_at numeric)`)
                .run();
            nativeDb
...
export async function getNexusDb() {
    ...
    _nexusInitPromise = (async () => {
        ...
        nativeDb
            .prepare(`INSERT OR IGNORE INTO nexus_schema_meta (key, value) VALUES ('schemaVersion', '${NEXUS_SCHEMA_VERSION}')`)
            .run();
 ...
 */
export function getNexusNativeDb() {
 ...
 */
export { nexusSchema };
//# sourceMappingURL=nexus-sqlite.js.map