...
<template>
@/tmp/pomodoro-bench/gsd/.claude/get-shit-done/templates/UAT.md
</template>
...
```bash
INIT=$(node "/tmp/pomodoro-bench/gsd/.claude/get-shit-done/bin/gsd-tools.cjs" init verify-work "${PHASE_ARG}")
...
```
UI_PHASE_FLAG=$(node "/tmp/pomodoro-bench/gsd/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
```bash
CHECKPOINT=$(node "/tmp/pomodoro-bench/gsd/.claude/get-shit-done/bin/gsd-tools.cjs" uat render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
node "/tmp/pomodoro-bench/gsd/.claude/get-shit-done/bin/gsd-tools.cjs" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
Read and follow `/tmp/pomodoro-bench/gsd/.claude/get-shit-done/workflows/transition.md`.
...
```bash
node "/tmp/pomodoro-bench/gsd/.claude/get-shit-done/bin/gsd-tools.cjs" audit-open --json 2>/dev/null
```
...
- Follow @/tmp/pomodoro-bench/gsd/.claude/get-shit-done/workflows/diagnose-issues.md
