...
describe("skills commands - protocol format resolution error branches", () => {
    beforeEach(() => {
        ...
        commonMocks.tokenizeCriteriaValue.mockImplementation((value) => value.split(",").map((e) => e.trim().toLowerCase()).filter(Boolean));
    });
    ...
    describe("list command - format error", () => {
        it("exits with error when resolveOutputFormat throws", async () => {
            ...
            await expect(program.parseAsync(["node", "test", "list"])).rejects.toThrow("process-exit");
            expect(exitSpy).toHaveBeenCalledWith(1);
        });
    });
    describe("find command - format error in non-json mode", () => {
        it("exits with error logged to stderr when resolveOutputFormat throws and --json is not set", async () => {
            ...
            await expect(program.parseAsync(["node", "test", "find", "test"])).rejects.toThrow("process-exit");
...
//# sourceMappingURL=skills-protocol-format-errors.test.js.map