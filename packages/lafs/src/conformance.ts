import { type ConformanceTier, getChecksForTier } from './conformanceProfiles.js';
import { getRegistryCode, getTransportMapping, isRegisteredErrorCode } from './errorRegistry.js';
import { LAFSFlagError, resolveOutputFormat } from './flagSemantics.js';
...
export function runEnvelopeConformance(
  ...
  if (
    typed.error &&
    typed.error.agentAction !== undefined &&
    isRegisteredErrorCode(typed.error.code)
  ) {
    const registryEntry = getRegistryCode(typed.error.code);
    const registryAction = (registryEntry as Record<string, unknown> | undefined)?.[
      'agentAction'
    ] as string | undefined;
    if (registryAction) {
    ...
    pushCheck(
      ...
      true,
      typed.error
        ? 'agentAction absent or code unregistered — skipped'
        : 'no error present — skipped',
    );
  }
  ...
  {
    const ext = (typed._extensions ?? {}) as Record<string, unknown>;
    const ledger = (ext['contextLedger'] ?? ext['context']) as Record<string, unknown> | undefined;
    ...
    if (!ledger || typeof ledger !== 'object') {
      ...
      if (!typed.success && typed.error && ledger['required'] === true) {
        ...
        if (
          !stale &&
          typed.error.code !== 'E_CONTEXT_MISSING' &&
          typed.error.code !== 'E_CONTEXT_STALE'
        ) {
          pass = false;
