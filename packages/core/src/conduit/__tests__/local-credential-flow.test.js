...
async function setupTestEnvironment() {
    const dir = join(tmpdir(), `credential-flow-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    mkdirSync(join(dir, '.cleo'), { recursive: true });
    // Create a minimal .cleo/project-context.json so paths resolve
    writeFileSync(join(dir, '.cleo', 'project-context.json'), JSON.stringify({ schemaVersion: '1.0.0', projectTypes: ['node'], primaryType: 'node' }));
    return dir;
}
// ============================================================================
// Test suite
...
//# sourceMappingURL=local-credential-flow.test.js.map