...
if (existsSync(benchPath)) {
  ...
  if (benchContent.includes('p50') && benchContent.includes('p95') && benchContent.includes('p99')) {
  ...
  if (benchContent.includes('regression') && (
...
if (scripts['bench:startup:update-baseline']) {
  pass(`package.json has bench:startup:update-baseline: "${scripts['bench:startup:update-baseline']}"`);
} else {
  fail('package.json missing bench:startup:update-baseline script (needed to update baseline.json)');
}
