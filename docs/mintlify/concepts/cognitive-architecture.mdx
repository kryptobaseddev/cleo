...
|-----------|---------------|-------------|
| `~/.cleo/nexus.db` | Brain | Central intelligence hub (SQLite, per ADR-006) |
...
| `tasks.db` (tasks table) | Dendrites | Input structure (tasks to process, SQLite per ADR-006) |
| `tasks.db` (archived tasks) | Long-term memory | Completed task storage |
...
```bash
# Cross-project dependency syntax
cleo add "Create login page" --depends T001,project-backend:T003
```
...
CLEO implements five retrieval methods:
...
The LCA algorithm (implemented in `src/core/tasks/`) enables:
...
└── nexus.db               # Global Registry SQLite database (per ADR-006)
