/**
 * Gate Validators for 4-Layer Operation Verification System
 *
...
} from './operation-verification-gates.js';
...
export async function validateLayer1Schema(context: OperationContext): Promise<LayerResult> {
  ...
  // Status validation — domain-aware
...
export async function validateLayer2Semantic(context: OperationContext): Promise<LayerResult> {
  ...
  // Manifest date must not be future
  if (context.params?.manifestEntry) {
    const entry = context.params.manifestEntry as Record<string, unknown>;
    if (entry.date) {
...
export async function validateLayer3Referential(context: OperationContext): Promise<LayerResult> {
  ...
  if (context.params?.depth !== undefined) {
    ...
    if (typeof depth === 'number' && depth > GATE_VALIDATION_RULES.MAX_DEPTH) {
      violations.push({
        layer: GateLayer.REFERENTIAL,
        severity: ErrorSeverity.ERROR,
        code: 'E_DEPTH_EXCEEDED',
        message: `Hierarchy depth ${depth} exceeds maximum of ${GATE_VALIDATION_RULES.MAX_DEPTH}`,
        field: 'depth',
        value: depth,
        constraint: `max depth: ${GATE_VALIDATION_RULES.MAX_DEPTH}`,
        fix: 'Flatten hierarchy to max 3 levels (epic -> task -> subtask)',
      });
    }
  }

  // Sibling limit validation
  if (context.params?.siblingCount !== undefined) {
    ...
    if (
      ...
      GATE_VALIDATION_RULES.MAX_SIBLINGS > 0 &&
      siblingCount >= GATE_VALIDATION_RULES.MAX_SIBLINGS
    ) {
      violations.push({
        layer: GateLayer.REFERENTIAL,
        severity: ErrorSeverity.ERROR,
        code: 'E_SIBLING_LIMIT',
        message: `Parent already has ${siblingCount} children (max ${GATE_VALIDATION_RULES.MAX_SIBLINGS})`,
        field: 'siblingCount',
        value: siblingCount,
        constraint: `max siblings: ${GATE_VALIDATION_RULES.MAX_SIBLINGS}`,
...
export async function validateLayer4Protocol(
  ...
  // Key findings validation for research protocol
  if (context.protocolType === 'research' && context.operation === 'complete') {
    ...
    if (manifestEntry?.key_findings) {
      ...
      if (
        ...
        findings.length < GATE_VALIDATION_RULES.KEY_FINDINGS_MIN ||
        findings.length > GATE_VALIDATION_RULES.KEY_FINDINGS_MAX
      ) {
        violations.push({
          layer: GateLayer.PROTOCOL,
          severity: ErrorSeverity.ERROR,
          code: 'E_KEY_FINDINGS_COUNT',
          message: `Research must have ${GATE_VALIDATION_RULES.KEY_FINDINGS_MIN}-${GATE_VALIDATION_RULES.KEY_FINDINGS_MAX} key findings, got ${Array.isArray(findings) ? findings.length : 0}`,
          field: 'manifestEntry.key_findings',
          value: Array.isArray(findings) ? findings.length : findings,
          constraint: `count: ${GATE_VALIDATION_RULES.KEY_FINDINGS_MIN}-${GATE_VALIDATION_RULES.KEY_FINDINGS_MAX}`,
 ...
 */
export const GATE_VALIDATION_RULES = {
