...
async function writeReport(
  ...
  // T9092: anchor to canonical project root to prevent rogue .cleo/ creation in worktrees.
  const canonicalRoot = getCleoProjectRoot(projectRoot);
  const cleoDir = getCleoDirAbsolute(canonicalRoot);
  const reportPath = join(cleoDir, 'agent-outputs', 'T1824-5-backfill-report.md');
  await mkdir(join(cleoDir, 'agent-outputs'), { recursive: true });
