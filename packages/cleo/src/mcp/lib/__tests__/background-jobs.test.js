...
describe('BackgroundJobManager', () => {
    ...
    describe('job creation and execution', () => {
        ...
        it('should set initial job status to running', async () => {
            const jobId = await manager.startJob('test.operation', () => new Promise(() => { }));
    ...
    describe('job cancellation', () => {
        ...
        it('should not update result after cancellation', async () => {
            let resolveExecutor;
            const jobId = await manager.startJob('op.cancel-race', () => new Promise((resolve) => {
    ...
    describe('concurrent job limits', () => {
        it('should enforce max concurrent jobs', async () => {
            ...
            await expect(manager.startJob('op.4', () => new Promise(() => { }))).rejects.toThrow('Maximum concurrent jobs reached (3)');
        });
    ...
    describe('error handling', () => {
        ...
        it('should handle rejected promise in executor', async () => {
            const jobId = await manager.startJob('op.reject', () => Promise.reject(new Error('rejected')));
            await new Promise((resolve) => setTimeout(resolve, 50));
            const job = manager.getJob(jobId);
            expect(job.status).toBe('failed');
    ...
    describe('default configuration', () => {
        it('should use default maxJobs of 10', async () => {
            ...
            await expect(defaultManager.startJob('op.11', () => new Promise(() => { }))).rejects.toThrow('Maximum concurrent jobs reached (10)');
...
//# sourceMappingURL=background-jobs.test.js.map