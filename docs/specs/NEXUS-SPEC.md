...
NEXUS is CLEO's **cross-project intelligence system** -- a global coordination layer that enables task discovery, dependency analysis, and relationship mapping across multiple registered CLEO projects. It uses the workshop vocabulary (the Star Road, the Hearth, the Central Chamber, the Axle) to model the global task graph.
...
|---------------------|-----------------------------------|----------------------------|
| **The Star Road**   | Global Task Graph                 | Cross-project visibility   |
| **The Hearth**      | Tasks (across all projects)       | Units of isolation & work  |
| **The Axle**        | Relationships (`depends` field)   | Connections between work   |
| **The Central Chamber**| Persistent global graph cache     | Cross-session context      |
| **Tension**         | Similarity scores + boosts        | Relevance ranking          |
| **Resonance**       | Score threshold (0.0-1.0)         | Filtering noise            |
...
- `nexus.status` - Show NEXUS registry status
- `nexus.list` - List registered projects
- `nexus.show` - Show a specific registered project
...
- `nexus.path.show` - Show cross-project critical path
- `nexus.blockers.show` - Show blocking impact for a task query
- `nexus.orphans.list` - List orphaned cross-project dependencies
...
- `nexus.permission.set` - Update project permissions
...
- **Resonance discovery scoring**: The `discover_across_projects()` function from `graph-rag.sh` that implements label/description/hierarchy scoring. Deferred until GraphRAG port.
