 ...
 */
function contextAwareMatch(text, primaryPatterns, contextWords, contextRadius = 200) {
    const details = [];
    const snippets = [];
...
async function safeFetch(url, verbose) {
    try {
        if (verbose) {
        ...
        if (verbose) {
            console.error(`  [fetch] ${url} -> ERROR: ${err instanceof Error ? err.message : String(err)}`);
        }
        return null;
    }
}
 ...
 */
function parseVercelSkills(raw) {
    const agents = {};
    ...
    while ((match = objectPattern.exec(raw)) !== null) {
        ...
        const extractField = (field) => {
            const fieldMatch = new RegExp(`${field}\\s*:\\s*["'\`]([^"'\`]+)["'\`]`).exec(block);
            return fieldMatch?.[1];
        };
    ...
    const arrayPattern = /["'`]([a-z][\w-]*)["'`]\s*,\s*\{([^{}]*)\}/gs;
...
function parseNeonAddMcp(raw) {
    ...
    const namedBlockPattern = /["'`]([a-z][\w-]*)["'`]\s*:\s*\{([^{}]*(?:\{[^{}]*\}[^{}]*)*)\}/gs;
    let match;
    ...
    const objectPattern = /\{\s*(?:id|name)\s*:\s*["'`]([^"'`]+)["'`]([^{}]*(?:\{[^{}]*\}[^{}]*)*)\}/gs;
    while ((match = objectPattern.exec(raw)) !== null) {
...
async function fetchAggregateSources(config, verbose) {
    const results = [];
    for (const source of config.aggregateSources) {
        ...
        if (verbose) {
            console.error(`  [aggregate] ${source.id}: parsed ${Object.keys(agents).length} agent(s)`);
        }
        results.push({
            sourceId: source.id,
 ...
 */
function isExcludedMatch(providerId, candidateId) {
...
// ---------------------------------------------------------------------------
function matchAggregateToProvider(providerId, aliases, aggregateData) {
    const findings = [];
    ...
    for (const agg of aggregateData) {
        for (const [agentId, agentData] of Object.entries(agg.agents)) {
            ...
            const isMatch = matchKeys.some((key) => normalizedId === key ||
                normalizedName === key ||
                ...
                new RegExp(`\\b${escapeRegex(key)}\\b`).test(normalizedName)) && !isExcludedMatch(providerId, normalizedId);
            if (isMatch) {
                findings.push({
...
// ---------------------------------------------------------------------------
async function researchProviderReadme(provider, sources, verbose) {
    const caps = {
    ...
    const hooksResult = contextAwareMatch(combined, HOOKS_PATTERNS, HOOKS_CONTEXT);
    ...
    const spawnResult = contextAwareMatch(combined, SPAWN_PATTERNS, SPAWN_CONTEXT);
    ...
    const configKeyMatch = /["']?(mcpServers|mcp_servers|extensions|mcp|servers|context_servers)["']?\s*:/i.exec(combined);
    if (configKeyMatch) {
...
// ---------------------------------------------------------------------------
function buildDiff(provider, aggregateFindings, discovered) {
    const diff = [];
    ...
    for (const finding of aggregateFindings) {
        ...
        if (finding.skillsPath) {
            const regSkillsPath = provider.capabilities?.skills?.agentsProjectPath ??
    ...
    if (discovered.hooks.detected) {
        ...
        if (!regHooks) {
            ...
            if (discoveredEvents.length > 0 &&
                ...
                const newEvents = discoveredEvents.filter((e) => !registeredEvents.includes(e));
    ...
    if (aggregateFindings.some((f) => f.skillsPath) &&
...
// ---------------------------------------------------------------------------
function buildRecommendations(provider, aggregateFindings, discovered, diff, totalSources) {
    const recs = [];
    // High-confidence diffs from aggregate sources
    const aggDiffs = diff.filter((d) => d.source === "aggregate" && d.confidence === "high");
    for (const d of aggDiffs) {
        recs.push(`[HIGH] ${d.field}: registry="${d.registry}" vs aggregate="${d.discovered}"`);
    }
    // Capability additions
    if (discovered.hooks.detected &&
        !provider.capabilities?.hooks) {
        recs.push(`Add capabilities.hooks — hook-related patterns found in documentation`);
    }
    if (discovered.spawn.detected &&
        !provider.capabilities?.spawn) {
        recs.push(`Add capabilities.spawn — spawn/subagent patterns found in documentation`);
    }
    ...
    for (const d of medDiffs) {
        if (d.action === "add") {
            ...
            recs.push(`[MED] Investigate ${d.field}: registry="${d.registry}" vs discovered="${d.discovered}"`);
        }
    }
    // Low-confidence diffs that merit investigation
    const lowDiffs = diff.filter((d) => d.confidence === "low" && d.action === "investigate");
    for (const d of lowDiffs) {
        recs.push(`[LOW] Verify ${d.field}: registry="${d.registry}" vs readme="${d.discovered}"`);
    }
    // Unreachable sources
    if (discovered.errors.length > 0 &&
        discovered.errors.length === totalSources) {
        recs.push(`All documentation sources unreachable — verify URLs in provider-sources.json`);
    }
    // No aggregate data found
    if (aggregateFindings.length === 0) {
        recs.push(`No aggregate source data found — consider adding this provider to aggregate source configs`);
    }
    return recs;
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
function generateLLMPrompt(provider, sourceEntry, aggregateFindings, discovered, diff) {
    const lines = [];
    lines.push("# Provider Research Prompt");
    lines.push("");
    lines.push(`Research the **${provider.toolName}** (${provider.id}) AI coding agent by **${provider.vendor}**.`);
    lines.push("");
    ...
    if (aggregateFindings.length > 0) {
        ...
        for (const f of aggregateFindings) {
            ...
            if (f.configPath)
                lines.push(`- Config path: ${f.configPath}`);
            if (f.instructionFile)
                lines.push(`- Instruction file: ${f.instructionFile}`);
            if (f.skillsPath)
                lines.push(`- Skills path: ${f.skillsPath}`);
            if (f.configKey)
                lines.push(`- Config key: ${f.configKey}`);
            if (f.transports)
    ...
    if (diff.length > 0) {
        ...
        lines.push("");
        for (const d of diff) {
            lines.push(`- **${d.field}** [${d.confidence}/${d.source}]: registry="${d.registry}" vs discovered="${d.discovered}"`);
        }
        lines.push("");
    }
    // Questions to answer
    ...
    if (!provider.capabilities?.hooks) {
        lines.push(`1. Does ${provider.toolName} support lifecycle hooks? If so, which events are available (onSessionStart, onToolStart, etc.)? What is the hook configuration format?`);
    }
    else {
        lines.push(`1. Are there any NEW lifecycle hook events for ${provider.toolName} beyond: ${provider.capabilities.hooks.supported.join(", ")}?`);
    }
    if (!provider.capabilities?.spawn) {
        lines.push(`2. Does ${provider.toolName} support spawning subagents or multi-agent workflows? What mechanism is used (native, MCP, CLI, API)?`);
    }
    else {
        lines.push(`2. Has ${provider.toolName}'s spawn/subagent support changed? Current: mechanism=${provider.capabilities.spawn.spawnMechanism}, parallel=${provider.capabilities.spawn.supportsParallelSpawn}`);
    }
    if (!provider.capabilities?.skills) {
        lines.push(`3. Does ${provider.toolName} support an .agents/skills directory convention? What is the skill loading precedence (vendor-only, agents-first, agents-canonical)?`);
    }
    else {
        lines.push(`3. Has ${provider.toolName}'s skills precedence changed? Current: ${provider.capabilities.skills.precedence}, globalPath=${provider.capabilities.skills.agentsGlobalPath}, projectPath=${provider.capabilities.skills.agentsProjectPath}`);
    }
    lines.push(`4. What MCP transports does ${provider.toolName} currently support? (stdio, sse, streamable-http)`);
    lines.push(`5. What is the correct MCP config path and config key for ${provider.toolName}?`);
    lines.push(`6. Does ${provider.toolName} support custom headers in MCP server configurations?`);
    ...
    if (diff.length > 0) {
        ...
        let q = 7;
        for (const d of diff) {
            lines.push(`${q}. ${d.field}: Our registry says "${d.registry}" but ${d.source} data suggests "${d.discovered}". Which is correct?`);
            q++;
        }
    }
    lines.push("");
    ...
    if (sourceEntry) {
        ...
        lines.push("No documentation sources configured for this provider in provider-sources.json.");
    }
    lines.push("");
    lines.push("Please use Context7 or web search to find the most current documentation. Focus on the official docs and source code rather than blog posts.");
    return lines.join("\n");
}
// ---------------------------------------------------------------------------
...
function printConsoleReport(report) {
    ...
    for (const p of report.providers) {
        ...
        console.log(`  Vendor: ${p.vendor} | Status: ${p.registrySummary.status} | Priority: ${p.registrySummary.priority}`);
        console.log(thinLine);
        // Registry summary
        console.log("  Registry:");
        console.log(`    configKey:       ${p.registrySummary.configKey}`);
        console.log(`    configFormat:    ${p.registrySummary.configFormat}`);
        console.log(`    transports:      ${p.registrySummary.supportedTransports.join(", ")}`);
        console.log(`    headers:         ${p.registrySummary.supportsHeaders}`);
        console.log(`    skills:          ${p.registrySummary.agentSkillsCompatible}`);
        ...
        if (caps) {
            ...
            if (caps.hooks) {
                console.log(`      hooks:         ${caps.hooks.supported.join(", ")}`);
            }
            if (caps.spawn) {
                console.log(`      spawn:         mechanism=${caps.spawn.spawnMechanism}, subagents=${caps.spawn.supportsSubagents}, parallel=${caps.spawn.supportsParallelSpawn}`);
            }
            if (caps.skills) {
                console.log(`      skills:        precedence=${caps.skills.precedence}, global=${caps.skills.agentsGlobalPath ?? "n/a"}, project=${caps.skills.agentsProjectPath ?? "n/a"}`);
            }
        }
        // Aggregate findings
        if (p.aggregateFindings.length > 0) {
            ...
            for (const f of p.aggregateFindings) {
                ...
                if (f.configPath)
                    console.log(`      configPath:      ${f.configPath}`);
                if (f.instructionFile)
                    console.log(`      instructionFile: ${f.instructionFile}`);
                if (f.skillsPath)
                    console.log(`      skillsPath:      ${f.skillsPath}`);
                if (f.configKey)
                    console.log(`      configKey:       ${f.configKey}`);
                if (f.transports)
        ...
        console.log(`    hooks:           ${p.discovered.hooks.detected ? `yes (${p.discovered.hooks.events.slice(0, 5).join(", ")})` : "no"}`);
        console.log(`    spawn:           ${p.discovered.spawn.detected ? `yes (${p.discovered.spawn.details.slice(0, 5).join(", ")})` : "no"}`);
        console.log(`    mcp transports:  ${p.discovered.mcp.transportsFound.join(", ") || "none detected"}`);
        console.log(`    instruction:     ${p.discovered.instructionFile ?? "none detected"}`);
        console.log(`    config format:   ${p.discovered.configFormat ?? "none detected"}`);
        console.log(`    config key:      ${p.discovered.configKey ?? "none detected"}`);
    ...
    console.log("  SUMMARY");
    console.log(line);
    console.log(`  Providers with hooks support:       ${report.summary.withHooks.length > 0 ? report.summary.withHooks.join(", ") : "none detected"}`);
    console.log(`  Providers with spawn/subagent:      ${report.summary.withSpawn.length > 0 ? report.summary.withSpawn.join(", ") : "none detected"}`);
    console.log(`  Providers with capabilities obj:    ${report.summary.withCapabilities.length > 0 ? report.summary.withCapabilities.join(", ") : "none"}`);
    console.log(`  Unreachable providers:              ${report.summary.unreachable.length > 0 ? report.summary.unreachable.join(", ") : "none"}`);
    console.log(`  Total recommendations:              ${report.summary.recommendationCount}`);
    console.log(line);
    console.log();
}
// ---------------------------------------------------------------------------
...
`);
    ...
    try {
        ...
        console.error(`Failed to read registry.json: ${err instanceof Error ? err.message : String(err)}`);
        process.exit(1);
    }
    let sourcesConfig;
    try {
        ...
        console.error(`Failed to read provider-sources.json: ${err instanceof Error ? err.message : String(err)}`);
        process.exit(1);
    }
    ...
    if (values.provider && !registry.providers[values.provider]) {
        console.error(`Provider "${values.provider}" not found in registry. Available: ${allProviderIds.join(", ")}`);
        process.exit(1);
    }
    ...
    if (!values["skip-aggregate"]) {
        if (!values.json && !values["generate-prompt"]) {
            console.log(`Fetching ${sourcesConfig.aggregateSources.length} aggregate source(s)...`);
        }
        aggregateData = await fetchAggregateSources(sourcesConfig, values.verbose ?? false);
        if (!values.json && !values["generate-prompt"]) {
            console.log(`  Parsed ${aggregateData.reduce((s, a) => s + Object.keys(a.agents).length, 0)} agent definition(s) from aggregate sources`);
            console.log();
        }
    }
    ...
    if (values["generate-prompt"]) {
        ...
        const aggregateFindings = matchAggregateToProvider(provider.id, provider.aliases, aggregateData);
        ...
        const discovered = values["skip-readme"]
            ...
            : await researchProviderReadme(provider, readmeSources, values.verbose ?? false);
        const diff = buildDiff(provider, aggregateFindings, discovered);
        const prompt = generateLLMPrompt(provider, sourceEntry, aggregateFindings, discovered, diff);
        console.log(prompt);
        process.exit(0);
    }
    // --- Standard research mode ---
    if (!values.json) {
        console.log(`Researching ${targetIds.length} provider(s) with configured sources...`);
        console.log(`(${allProviderIds.length - targetIds.length} provider(s) skipped — no sources configured)`);
        console.log();
    }
    ...
    for (const id of targetIds) {
        ...
        const discovered = values["skip-readme"]
            ...
            : await researchProviderReadme(provider, readmeSources, values.verbose ?? false);
        const diff = buildDiff(provider, aggregateFindings, discovered);
        const recommendations = buildRecommendations(provider, aggregateFindings, discovered, diff, readmeSources.length);
        ...
        if (!values.json) {
            ...
            const status = errorCount > 0 && errorCount === readmeSources.length
                ? " unreachable"
                : diffCount > 0
    ...
    const report = {
        ...
        summary: {
            ...
            unreachable: providerReports
                .filter((p) => p.discovered.errors.length > 0 &&
                p.discovered.errors.length ===
                    (sourcesConfig.providers[p.id]?.sources.length ?? 0))
                .map((p) => p.id),
            recommendationCount: providerReports.reduce((sum, p) => sum + p.recommendations.length, 0),
        },
    };
    if (values.json) {
...
//# sourceMappingURL=provider-research.js.map