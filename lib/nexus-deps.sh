...
#
# ARCHITECTURE:
#   - Extends lib/graph-cache.sh for O(1) lookups
...
if [[ -f "$_LIB_DIR/nexus-registry.sh" ]]; then
    # shellcheck source=lib/nexus-registry.sh
    source "$_LIB_DIR/nexus-registry.sh"
else
...
if [[ -f "$_LIB_DIR/nexus-query.sh" ]]; then
    # shellcheck source=lib/nexus-query.sh
    source "$_LIB_DIR/nexus-query.sh"
else
...
if [[ -f "$_LIB_DIR/nexus-permissions.sh" ]]; then
    # shellcheck source=lib/nexus-permissions.sh
    source "$_LIB_DIR/nexus-permissions.sh"
else
