...
import { checkTaskCountMismatch, formatValidationResult, validateSourceFiles, } from '../validate.js';
let tempDir;
let cleoDir;
describe('validateSourceFiles', () => {
    ...
    describe('valid JSON files', () => {
        it('validates all files successfully with data', async () => {
            ...
            await writeFile(join(cleoDir, 'sessions.json'), JSON.stringify({
                version: '1.0.0',
        ...
        it('accepts archive.json with "tasks" key instead of "archivedTasks"', async () => {
            await writeFile(join(cleoDir, 'todo.json'), JSON.stringify({ tasks: [], _meta: { schemaVersion: '2.10.0' } }));
            await writeFile(join(cleoDir, 'todo-archive.json'), JSON.stringify({
                _meta: { schemaVersion: '2.4.0' },
                tasks: [
    ...
    describe('corrupted JSON detection', () => {
        it('detects corrupted todo.json with syntax error', async () => {
            await writeFile(join(cleoDir, 'todo.json'), '{ "tasks": [ { "id": "T001", "title": "Test", "status": "pending" }, ] }');
            const result = validateSourceFiles(cleoDir);
    ...
    describe('empty JSON warnings', () => {
        ...
        it('warns about empty sessions array', async () => {
            await writeFile(join(cleoDir, 'todo.json'), JSON.stringify({ tasks: [{ id: 'T001', title: 'Test', status: 'pending' }] }));
            await writeFile(join(cleoDir, 'sessions.json'), JSON.stringify({ sessions: [], _meta: { schemaVersion: '1.0.0' } }));
            const result = validateSourceFiles(cleoDir);
            expect(result.valid).toBe(true);
            expect(result.warnings).toHaveLength(1);
        ...
        it('warns about empty archive', async () => {
            await writeFile(join(cleoDir, 'todo.json'), JSON.stringify({ tasks: [{ id: 'T001', title: 'Test', status: 'pending' }] }));
            await writeFile(join(cleoDir, 'todo-archive.json'), JSON.stringify({ archivedTasks: [], _meta: { schemaVersion: '2.4.0' } }));
            const result = validateSourceFiles(cleoDir);
            expect(result.valid).toBe(true);
            expect(result.warnings).toHaveLength(1);
    ...
    describe('multiple file validation', () => {
        it('fails if any file is corrupted', async () => {
            await writeFile(join(cleoDir, 'todo.json'), JSON.stringify({ tasks: [{ id: 'T001', title: 'Valid', status: 'pending' }] }));
            await writeFile(join(cleoDir, 'sessions.json'), '{ invalid }');
            await writeFile(join(cleoDir, 'todo-archive.json'), JSON.stringify({ archivedTasks: [{ id: 'T100', title: 'Archived', status: 'done' }] }));
            const result = validateSourceFiles(cleoDir);
            expect(result.valid).toBe(false);
            expect(result.todoJson.valid).toBe(true);
            expect(result.sessionsJson.valid).toBe(false);
