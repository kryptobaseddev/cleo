...
description: "Coding standards and style guide for CLEO development"
icon: "code"
---
...
This document defines coding conventions for the cleo project. For normative requirements, see the relevant specification documents in `developer/specifications/`.
...
|---------|------------|---------|
| Scripts | kebab-case | `add.sh`, `archive.sh` |
...
For LLM-Agent-First compliance, see `developer/specifications/LLM-AGENT-FIRST`.
...
See `developer/specifications/CONFIG-SYSTEM` for full details.
...
@test "add.sh rejects duplicate IDs with E_DUPLICATE_ID" {
    # Test implementation
}
```
...
- **Work-item notes**: Prefer tracked task IDs over inline note markers
...
- `developer/specifications/LLM-AGENT-FIRST` - CLI output requirements
- `developer/specifications/LLM-TASK-ID-SYSTEM-DESIGN` - Task ID format
- `developer/specifications/CONFIG-SYSTEM` - Configuration system
