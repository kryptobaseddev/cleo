...
describe("core/paths/standard - uncovered lines", () => {
    ...
    it("resolveProvidersRegistryPath throws when registry not found", async () => {
        ...
        expect(() => resolveProvidersRegistryPath("/nonexistent/path/nowhere")).toThrow("Cannot find providers/registry.json");
    });
...
describe("core/skills/catalog - uncovered delegate functions", () => {
    ...
    beforeEach(async () => {
        ...
        writeFileSync(join(fixtureRoot, "profiles", "default.json"), JSON.stringify({ name: "default", skills: ["test-skill"] }));
    });
    afterEach(async () => {
        const catalog = await import("../../src/core/skills/catalog.js");
        catalog.clearRegisteredLibrary();
...
describe("core/formats/index - unsupported format branches", () => {
    it("readConfig throws for unsupported format", async () => {
        ...
        await expect(readConfig("/tmp/test.xml", "xml")).rejects.toThrow("Unsupported config format: xml");
    });
    it("writeConfig throws for unsupported format", async () => {
        const { writeConfig } = await import("../../src/core/formats/index.js");
        await expect(writeConfig("/tmp/test.xml", "xml", "key", "server", {})).rejects.toThrow("Unsupported config format: xml");
    });
    it("removeConfig throws for unsupported format", async () => {
        const { removeConfig } = await import("../../src/core/formats/index.js");
        await expect(removeConfig("/tmp/test.xml", "xml", "key", "server")).rejects.toThrow("Unsupported config format: xml");
    });
});
// ════════════════════════════════════════════════════════════════════
...
describe("core/formats/utils - setNestedValue intermediate null handling", () => {
    ...
    it("creates intermediate objects when path has null values", async () => {
        const { setNestedValue } = await import("../../src/core/formats/utils.js");
        const result = setNestedValue({ context: null }, "context.servers", "test", { url: "http://x" });
        const context = result.context;
        const servers = context.servers;
        expect(servers.test).toEqual({ url: "http://x" });
    });
...
describe("core/skills/recommendation - uncovered branches", () => {
    ...
    it("validates conflicting criteria", async () => {
        ...
        expect(result.valid).toBe(false);
        expect(result.issues.some((i) => i.code === "E_SKILLS_CRITERIA_CONFLICT")).toBe(true);
    });
    it("scoreSkillRecommendation covers all scoring paths", async () => {
        ...
        expect(result.reasons.some((r) => r.code === "MATCH_TOPIC_GITBOOK")).toBe(true);
        expect(result.reasons.some((r) => r.code === "HAS_GIT_SYNC")).toBe(true);
        expect(result.reasons.some((r) => r.code === "HAS_API_WORKFLOW")).toBe(true);
    ...
    it("scoreSkillRecommendation detects legacy GitBook CLI markers", async () => {
        ...
        expect(result.reasons.some((r) => r.code === "PENALTY_LEGACY_CLI")).toBe(true);
    ...
    it("recommendSkills sorts by stars when scores are equal", async () => {
        const { recommendSkills } = await import("../../src/core/skills/recommendation.js");
        const skills = [
            { name: "a", scopedName: "@t/a", description: "test a", author: "a", stars: 10, githubUrl: "", repoFullName: "", path: "", source: "agentskills.in" },
            { name: "b", scopedName: "@t/b", description: "test b", author: "b", stars: 20, githubUrl: "", repoFullName: "", path: "", source: "agentskills.in" },
        ];
...
describe("core/marketplace/client - all adapters fail for getSkill", () => {
    it("getSkill throws MarketplaceUnavailableError when all adapters fail", async () => {
        ...
        const client = new MarketplaceClient([failingAdapter]);
    ...
    it("getSkill returns null when adapter returns null", async () => {
        ...
        const client = new MarketplaceClient([nullAdapter]);
...
describe("core/skills/recommendation-api - branches", () => {
    it("formatSkillRecommendations handles empty ranking in human mode", async () => {
        ...
        const result = formatSkillRecommendations({ criteria: { query: "test", queryTokens: ["test"], mustHave: [], prefer: [], exclude: [] }, ranking: [] }, { mode: "human" });
        expect(result).toBe("No recommendations found.");
    });
    it("formatSkillRecommendations json mode with details", async () => {
        const { formatSkillRecommendations } = await import("../../src/core/skills/recommendation-api.js");
        const ranking = [{
                skill: { name: "a", scopedName: "@t/a", description: "desc", author: "a", stars: 10, githubUrl: "", repoFullName: "", path: "", source: "agentskills.in" },
                score: 10,
                reasons: [{ code: "QUERY_MATCH" }],
                tradeoffs: [],
                excluded: false,
                breakdown: { mustHave: 0, prefer: 0, query: 3, stars: 2, metadata: 2, modernity: 0, exclusionPenalty: 0, total: 10 },
            }];
        const result = formatSkillRecommendations({ criteria: { query: "test", queryTokens: ["test"], mustHave: [], prefer: [], exclude: [] }, ranking }, { mode: "json", details: true });
    ...
    it("formatSkillRecommendations json mode without details", async () => {
        ...
        const result = formatSkillRecommendations({ criteria: { query: "test", queryTokens: ["test"], mustHave: [], prefer: [], exclude: [] }, ranking }, { mode: "json", details: false });
        expect(result.options[0].description).toBeUndefined();
    });
    it("formatSkillRecommendations json mode with empty ranking", async () => {
        const { formatSkillRecommendations } = await import("../../src/core/skills/recommendation-api.js");
        const result = formatSkillRecommendations({ criteria: { query: "test", queryTokens: ["test"], mustHave: [], prefer: [], exclude: [] }, ranking: [] }, { mode: "json" });
...
describe("core/marketplace/client - adapter index fallback", () => {
    it("search handles mixed success/failure across adapters", async () => {
        ...
        const client = new MarketplaceClient([okAdapter, failAdapter]);
...
//# sourceMappingURL=core-coverage-gaps.test.js.map