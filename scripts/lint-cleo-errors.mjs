...
function countCleoErrorArgs(source, callStart) {
  ...
  while (i < source.length) {
    ...
    switch (state) {
      ...
      default:
        // Enter comments
        if (c === '/' && next === '/') { state = S_LINE_COMMENT; i++; continue; }
        if (c === '/' && next === '*') { state = S_BLOCK_COMMENT; i += 2; continue; }

        // Enter strings
        if (c === "'") { state = S_STRING_SQ; i++; continue; }
        if (c === '"') { state = S_STRING_DQ; i++; continue; }
        if (c === '`') {
...
for (const file of allFiles) {
  ...
  while ((match = pattern.exec(source)) !== null) {
    ...
    if (line.includes(INTERNAL_INVARIANT_MARKER) || prevLine.includes(INTERNAL_INVARIANT_MARKER)) {
      continue;
    }
