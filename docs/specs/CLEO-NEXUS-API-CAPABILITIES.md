...
│ (Per-project)│ │ (Per-project)│ │ (Per-project)│ │ (Global XDG) │
│              │ │              │ │              │ │              │
│ • Tasks      │ │ • Patterns   │ │ • Agents     │ │ • Project    │
│ • sessions   │ │ • Learnings  │ │ • Messages   │ │   Registry   │
│   (table)    │ │ • Decisions  │ │ • Presence   │ │ • Audit Log  │
│ • audit_log  │ │ • Memory     │ │              │ │ • Metadata   │
│ • pipeline   │ │              │ │              │ │              │
│   Manifest   │ │              │ │              │ │              │
└──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘
```
...
| Access Point | Protocol | Status | Best For |
|--------------|----------|--------|----------|
| **CLI** | citty stdin/stdout | Implemented | Agents, scripts, humans (LAFS via `--json`, human via `--human`) |
| **SDK** | TypeScript/JS (`@cleocode/core`) | Implemented | Type-safe programmatic integration |
| **HTTP API** | HTTP/1.1, JSON (Fastify) | PLANNED (CLEO-WEB-API.md) | Web apps, external tools |
...
CLEO storage is four databases: `tasks.db`, `brain.db`, and `signaldock.db` live per-project under `.cleo/`; `nexus.db` is the global cross-project registry at the XDG path.
...
Agent A (Backend):
  ...
  - Makes progress via `cleo` CLI commands
...
Coordination:
  - Both agents drive CLEO through the same `cleo` CLI
...
### 7.4 Agent Integration via CLI

Agents spawn `cleo` as a subprocess and parse the LAFS envelope from stdout:

```bash
# Agent queries NEXUS status
cleo nexus status --json
```

Response (LAFS envelope on stdout):
```json
{
  "_meta": {
...
> **Note:** For in-process integration without spawning a subprocess, use the `@cleocode/core` SDK directly. It exposes the same engine layer that the CLI dispatch calls.
...
├── CLEO-WEB-API.md                # HTTP/Web adapter (PLANNED)
