...
| --- | --- | --- | --- | --- | --- |
| `I1` | Storage uniformity | `info` | _none_ | `packages/core/src/tasks/__tests__/id-generator.test.ts` | All task IDs are stored as T#### and the type column is the canonical tier discriminator, including type="saga". There is no separate ID space for Sagas, Epics, Tasks, or Subtasks; labels never define hierarchy. |
...
| `I5` | Parent containment only; legacy groups hierarchy deprecated | `error` | `assertSagaInvariantI5` (`packages/core/src/sagas/enforcement.ts`) | `packages/core/src/sagas/__tests__/enforcement.test.ts` | tasks.parent_id is the ONLY containment edge. Saga children are Epics whose parent_id points at the Saga; Epic children are Tasks; Task children are Subtasks. task_relations.type="groups" is legacy non-containment provenance/cross-reference only and MUST NOT drive child listing, rollups, completion, or depth. |
...
| `I7` | Maximum parent depth is 3 | `error` | `assertSagaInvariantI7` (`packages/core/src/sagas/enforcement.ts`) | `packages/core/src/sagas/__tests__/enforcement.test.ts` | hierarchy.maxDepth=3 means at most three parent_id edges from root to leaf: Saga(depth 0) → Epic(depth 1) → Task(depth 2) → Subtask(depth 3). Standalone Epics may be roots at depth 0. Secondary relations cannot extend or bypass this budget. |
