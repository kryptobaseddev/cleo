...
description: "Behavioral protocol for orchestrating multi-agent research workflows with manifest-based coordination"
icon: "file-code"
---
...
- [SUBAGENT_PROTOCOL.md](../.cleo/agent-outputs/SUBAGENT_PROTOCOL.md) for research output format
...
OUTPUT REQUIREMENTS:
1. MUST write findings to: .cleo/agent-outputs/YYYY-MM-DD_{topic-slug}.md
...
2. MUST start task: `cleo start <task-id>`
...
└───────────────────────────────────────┘
        ...
        ▼
┌───────────────────────────────────────┐
│ 2. Start task: cleo start <task-id>│
└───────────────────────────────────────┘
        │
        ▼
┌───────────────────────────────────────┐
...
cat .cleo/agent-outputs/MANIFEST.jsonl | jq -s '[.[] | select(.needs_followup | length > 0)]'

# Step 3: Check active task
cleo current
...
| Condition | Action |
|-----------|--------|
| Active session exists with active task | Resume session; continue from active task |
| Active session exists, no active task | Query manifest for needs_followup; spawn next agent |
...
└───────────────────────────────────────┘
        │
        ├──── Active task exists ────▶ CONTINUE ACTIVE TASK
        │
        ▼ No active task
┌───────────────────────────────────────┐
...
| Event | Link Action |
|-------|-------------|
| Subagent starts task | Start task; no link yet |
...
for f in .cleo/agent-outputs/*.md; do
...
| `cleo start <id>` | Mark active work | Subagent start |
...
|------|---------|------------|
| `.cleo/agent-outputs/` | Research output directory | `cleo research init` |
...
- [ ] Starts task before work (`cleo start <id>`)
