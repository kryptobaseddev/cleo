...
describe("skills commands - format resolution error branches", () => {
  ...
  describe("check command - format error", () => {
    it("exits with error when resolveFormat throws", async () => {
      ...
      await expect(
        program.parseAsync(["node", "test", "check"]),
      ).rejects.toThrow("process-exit");

      expect(lafsFormatMocks.emitJsonError).toHaveBeenCalledWith(
        "skills.check",
        "standard",
        "E_FORMAT_CONFLICT",
  ...
  describe("init command - format error", () => {
    it("exits with error when resolveFormat throws", async () => {
      ...
      await expect(
        program.parseAsync(["node", "test", "init", "my-skill"]),
      ).rejects.toThrow("process-exit");
  ...
  describe("install command - format error", () => {
    it("exits with error when resolveFormat throws", async () => {
      ...
      await expect(
        program.parseAsync(["node", "test", "install", "some-skill", "--all"]),
      ).rejects.toThrow("process-exit");
  ...
  describe("remove command - format error", () => {
    it("exits with error when resolveFormat throws", async () => {
      ...
      await expect(
        program.parseAsync(["node", "test", "remove", "my-skill"]),
      ).rejects.toThrow("process-exit");
  ...
  describe("audit command - format error (non-sarif path)", () => {
    it("exits with error when resolveFormat throws", async () => {
      ...
      await expect(
        program.parseAsync(["node", "test", "audit", "/some/path"]),
      ).rejects.toThrow("process-exit");
