...
function mockFetchError(status, body = 'Internal Server Error') {
    return {
        ok: false,
        status,
...
function mockFetchEnvelopeError(code, message) {
    const envelope = {
        success: false,
        error: { code, message },
    };
...
describe('SignalDockTransport', () => {
    ...
    describe('register', () => {
        ...
        it('throws on non-200 response', async () => {
            ...
            await expect(transport.register('orchestrator', 'code_dev', 'private')).rejects.toThrow(/SignalDock API error.*409.*Agent already exists/);
        });
        it('throws on envelope error with success:false', async () => {
            fetchMock.mockResolvedValueOnce(mockFetchEnvelopeError('DUPLICATE', 'Agent name already taken'));
            await expect(transport.register('orchestrator', 'code_dev', 'private')).rejects.toThrow(/SignalDock error \[DUPLICATE\]: Agent name already taken/);
        });
    });
    describe('deregister', () => {
        ...
        it('throws on non-200 DELETE response', async () => {
            ...
            await expect(transport.deregister('nonexistent')).rejects.toThrow(/SignalDock API error.*404/);
        });
    });
    describe('send', () => {
    ...
    describe('error handling edge cases', () => {
        it('handles network failure (fetch throws)', async () => {
            ...
            await expect(transport.register('test', 'code_dev', 'private')).rejects.toThrow('fetch failed');
        });
        it('handles response.text() failure gracefully', async () => {
            ...
            await expect(transport.register('test', 'code_dev', 'private')).rejects.toThrow(/SignalDock API error.*500/);
        });
...
//# sourceMappingURL=signaldock-transport.test.js.map