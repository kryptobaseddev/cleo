...
describe("integration: skills find recommend contract", () => {
    beforeEach(() => {
        ...
        mocks.formatSkillRecommendations.mockImplementation((result, opts) => {
            if (opts.mode === "human") {
    ...
    it("returns stable validation and conflict codes in JSON mode", async () => {
        ...
        await expect(programValidation.parseAsync(["node", "test", "find", "docs", "--recommend", "--json", "--top", "0"])).rejects.toThrow("process-exit");
        const validationOutput = JSON.parse(String(errorSpy.mock.calls[0]?.[0] ?? "{}"));
        ...
        await expect(programConflict.parseAsync([
            "node",
            "test",
            "find",
            "docs",
        ...
        const conflictOutput = JSON.parse(String(errorSpy.mock.calls[0]?.[0] ?? "{}"));
...
//# sourceMappingURL=skills-find-recommendation.test.js.map