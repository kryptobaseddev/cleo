...
vi.mock('../../lib/engine.js', () => ({
    ...
    // Memory engine (brain.db backed after T5241 cutover)
    memoryShow: vi.fn(() => ({ success: true, data: {} })),
    memoryFind: vi.fn(() => ({ success: true, data: [] })),
    memoryTimeline: vi.fn(() => ({ success: true, data: {} })),
    memoryFetch: vi.fn(() => ({ success: true, data: [] })),
    memoryObserve: vi.fn(() => ({ success: true, data: {} })),
    memoryBrainStats: vi.fn(() => ({ success: true, data: {} })),
    memoryDecisionFind: vi.fn(() => ({ success: true, data: [] })),
    memoryDecisionStore: vi.fn(() => ({ success: true, data: {} })),
    memoryPatternFind: vi.fn(() => ({ success: true, data: [] })),
    memoryPatternStats: vi.fn(() => ({ success: true, data: {} })),
    memoryPatternStore: vi.fn(() => ({ success: true, data: {} })),
    memoryLearningFind: vi.fn(() => ({ success: true, data: [] })),
    memoryLearningStats: vi.fn(() => ({ success: true, data: {} })),
    memoryLearningStore: vi.fn(() => ({ success: true, data: {} })),
    // Pipeline manifest (moved from memory domain in T5241)
    pipelineManifestShow: vi.fn(() => ({ success: true, data: {} })),
    pipelineManifestList: vi.fn(() => ({ success: true, data: [] })),
    pipelineManifestFind: vi.fn(() => ({ success: true, data: [] })),
    pipelineManifestPending: vi.fn(() => ({ success: true, data: [] })),
    pipelineManifestStats: vi.fn(() => ({ success: true, data: {} })),
    pipelineManifestRead: vi.fn(() => ({ success: true, data: [] })),
    pipelineManifestAppend: vi.fn(() => ({ success: true, data: {} })),
    pipelineManifestArchive: vi.fn(() => ({ success: true, data: {} })),
    pipelineManifestLink: vi.fn(() => ({ success: true, data: {} })),
    pipelineManifestContradictions: vi.fn(() => ({ success: true, data: [] })),
    pipelineManifestSuperseded: vi.fn(() => ({ success: true, data: [] })),
    pipelineManifestCompact: vi.fn(() => ({ success: true, data: {} })),
    pipelineManifestValidate: vi.fn(() => ({ success: true, data: {} })),
    readManifestEntries: vi.fn(() => []),
    filterManifestEntries: vi.fn(() => []),
    // Session context injection (moved from memory domain in T5241)
    sessionContextInject: vi.fn(() => ({ success: true, data: {} })),
    // Validate engine
...
describe('CLI Adapter', () => {
    ...
    describe('dispatchFromCli', () => {
        it('dispatches a successful query and calls cliOutput', async () => {
            ...
            expect(cliOutput).toHaveBeenCalledWith({ id: 'T001', title: 'Test' }, expect.objectContaining({
                command: 'show',
        ...
        it('sets default operation from domain.operation when not provided', async () => {
            ...
            expect(cliOutput).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
                command: 'show',
                operation: 'tasks.show',
            }));
        });
