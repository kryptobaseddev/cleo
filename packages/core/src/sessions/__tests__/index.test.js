...
describe('sessions index memory bridge wiring', { timeout: 60_000 }, () => {
    ...
    it('calls bridgeSessionToMemory with derived end-session payload', async () => {
        ...
        expect(mocks.bridgeSessionToMemory).toHaveBeenCalledWith(tempDir, expect.objectContaining({
    ...
    it('keeps session end successful when bridge rejects', async () => {
        ...
        await expect(endSession({}, tempDir)).resolves.toEqual(expect.objectContaining({
            id: started.id,
            status: 'ended',
        }));
        expect(mocks.bridgeSessionToMemory).toHaveBeenCalledTimes(1);
