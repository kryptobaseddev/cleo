...
// ---------------------------------------------------------------------------
// T695-1 flakiness class — ENV-gated skip guard
//
// All three tests in this file spawn `cleo` via `execFile` against a real
// SQLite brain.db. They fail deterministically when the cleo binary is
// unavailable — either because:
//   (a) the monorepo dist has not been built yet (fresh clone, pre-build CI), or
//   (b) no global `cleo` is installed on PATH and CLEO_BIN is unset.
//
// This is the same T695-1 flakiness class that prompted T1093/d44c7669c to
// skip the wave3 perf canary. The correct response is a skipIf guard, not a
// build requirement injected into the test runner. Behavioral coverage of STDP
// plasticity is provided by the wave3 unit tests (T690/T694/T695-2/T695-3/T695-4)
// which require no CLI binary.
//
// The guard is deterministic: existsSync for .js paths, spawnSync for PATH
// resolution. Both branches are evaluated at module-load time so the skip
// decision is visible in the test reporter before any test body runs.
// ---------------------------------------------------------------------------

/**
 * Returns true if the resolved cleo binary is actually executable.
 *
 * For a `.js` ESM bundle: checks existsSync on the file path.
 * For a bare `"cleo"` command: probes via `spawnSync("cleo", ["--version"])`.
 */
function isCleoBinAvailable(): boolean {
  if (CLEO_BIN_IS_SCRIPT) {
    return existsSync(CLEO_BIN);
  }
  // PATH-based binary — probe with --version to avoid side effects
  try {
    const probe = spawnSync(CLEO_BIN, ['--version'], { timeout: 5_000 });
    return probe.status === 0 || probe.status === 1; // non-null status ⇒ process launched
  } catch {
    return false;
  }
}

/** Module-level availability flag evaluated once at import time. */
const CLEO_BIN_AVAILABLE = isCleoBinAvailable();
...
describe.skipIf(!CLEO_BIN_AVAILABLE)(
  ...
  () => {
    ...
    it('T682-1: cleo memory dream writes brain_plasticity_events with kind=ltp to real brain.db', async () => {
      ...
      const rowCount = nativeDb
        .prepare('SELECT COUNT(*) AS cnt FROM brain_retrieval_log')
        .get() as {
        cnt: number;
      };
      ...
      expect(
        ltpCount.cnt,
        'At least one plasticity event must have kind=ltp',
      ).toBeGreaterThanOrEqual(1);
    ...
    it('T682-2: cleo brain plasticity stats --json reports totalEvents > 0 after dream cycle', async () => {
      ...
      const statsResult = await runCleo(
        ['brain', 'plasticity', 'stats', '--json'],
        cleoDirAbsolute,
      );
      expect(
    ...
    it('T682-3: LTP plasticity events have non-zero weight delta (delta_w > 0)', async () => {
      ...
      for (const evt of ltpEvents) {
        expect(evt.delta_w, `LTP event delta_w must be > 0 (was ${evt.delta_w})`).toBeGreaterThan(
          0,
        );
      }
    });
