    ...
    describe('RCASD-IVTR+C Pipeline Gates', () => {
        describe('Research → Consensus Gate', () => {
            it('should pass when research is completed', async () => {
                const { epicId, targetStage, currentManifest, expectedResult } = lifecycleScenarios.successes.researchToConsensus;
            ...
            it('should block when research is pending', async () => {
                const { epicId, targetStage, currentManifest, expectedResult } = lifecycleScenarios.failures.consensusWithoutResearch;
                const result = await enforcer.checkLifecycleGate(epicId, targetStage, currentManifest || undefined);
        ...
        describe('Consensus → Specification Gate', () => {
            ...
            it('should accept skipped consensus stage', async () => {
                const { epicId, targetStage, currentManifest, expectedResult } = lifecycleScenarios.successes.withSkippedStage;
                const result = await enforcer.checkLifecycleGate(epicId, targetStage, currentManifest || undefined);
        ...
        describe('Specification → Decomposition Gate', () => {
            ...
            it('should block without specification', async () => {
                const { epicId, targetStage, currentManifest, expectedResult } = lifecycleScenarios.failures.decompositionWithoutSpec;
                const result = await enforcer.checkLifecycleGate(epicId, targetStage, currentManifest || undefined);
                expect(result.passed).toBe(false);
        ...
        describe('Decomposition → Implementation Gate', () => {
            it('should pass with complete RCASD-IVTR+C', async () => {
                const { epicId, targetStage, currentManifest, expectedResult } = lifecycleScenarios.successes.completeRCASDToImplementation;
                const result = await enforcer.checkLifecycleGate(epicId, targetStage, currentManifest || undefined);
                expect(result.passed).toBe(true);
                expect(result.missingPrerequisites).toEqual([]);
            });
            it('should allow without manifest (legacy behavior)', async () => {
                const { epicId, targetStage, currentManifest } = lifecycleScenarios.failures.skipToImplementation;
                const result = await enforcer.checkLifecycleGate(epicId, targetStage, currentManifest || undefined);
    ...
    describe('IVTR Pipeline Gates', () => {
        describe('Implementation → Validation Gate', () => {
            ...
            it('should block without implementation', async () => {
                const { epicId, targetStage, currentManifest, expectedResult } = lifecycleScenarios.failures.validationWithoutImplementation;
                const result = await enforcer.checkLifecycleGate(epicId, targetStage, currentManifest || undefined);
                expect(result.passed).toBe(false);
        ...
        describe('Testing → Release Gate', () => {
            it('should pass when all stages complete', async () => {
                const { epicId, targetStage, currentManifest, expectedResult } = lifecycleScenarios.successes.readyForRelease;
                const result = await enforcer.checkLifecycleGate(epicId, targetStage, currentManifest || undefined);
                expect(result.passed).toBe(true);
                expect(result.missingPrerequisites).toEqual([]);
            });
            it('should block without testing', async () => {
                const { epicId, targetStage, currentManifest, expectedResult } = lifecycleScenarios.failures.releaseWithoutTesting;
                const result = await enforcer.checkLifecycleGate(epicId, targetStage, currentManifest || undefined);
                expect(result.passed).toBe(false);
...
//# sourceMappingURL=lifecycle-gates.test.js.map