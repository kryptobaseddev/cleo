...
function scanFile(absPath) {
  ...
  for (const call of stdoutCallIdentities(src, relPath)) {
    if (!lines[call.line - 1].includes(ALLOW_INLINE)) violations.push(call);
  }
}
...
// Updating remains explicit; normal CI reads the complete-call multiset only.
if (UPDATE_BASELINE) {
  writeFileSync(BASELINE_PATH, `${JSON.stringify(createStdoutBaseline(violations), null, 2)}\n`);
  console.info(
...
if (!existsSync(BASELINE_PATH)) {
  console.error(`lint-stdout-write-allowlist: ERROR — missing baseline at ${BASELINE_PATH}`);
  process.exit(1);
}
let baseline;
let comparison;
try {
  ...
  comparison = compareStdoutBaseline(violations, baseline);
} catch (error) {
  console.error(`lint-stdout-write-allowlist: ERROR — ${error.message}`);
  process.exit(1);
}
const newViolations = comparison.added;
if (newViolations.length === 0) {
  console.info(
    `lint-stdout-write-allowlist: OK — ${totalViolations} violation(s) (baseline: ${baseline.total}); ${comparison.removed} prior violation(s) removed`,
  );
  process.exit(0);
}
