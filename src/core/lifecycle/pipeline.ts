...
export async function initializePipeline(
  ...
  if (existing.length > 0) {
    throw new CleoError(ExitCode.ALREADY_EXISTS, `Pipeline already exists for task ${taskId}`);
  }
 ...
 * @task T4912 - Implemented SQLite wiring
 */
export async function advanceStage(taskId: string, options: AdvanceStageOptions): Promise<void> {
  const db = await getDb();
  const now = new Date();

  // Validate required parameters
  if (!options.toStage) {
    throw new CleoError(ExitCode.INVALID_INPUT, 'advanceStage() requires a target stage (toStage)');
  }

  if (!options.initiatedBy) {
    throw new CleoError(ExitCode.INVALID_INPUT, 'advanceStage() requires initiatedBy agent/user');
  }
  ...
  if (pipelineResult.length === 0) {
    throw new CleoError(ExitCode.NOT_FOUND, `No pipeline found for task ${taskId}`);
  }
 ...
 * @task T4912 - Implemented SQLite wiring
 */
export async function listPipelines(options: PipelineQueryOptions = {}): Promise<Pipeline[]> {
  const db = await getDb();
  ...
  if (options.status) {
    conditions.push(
      eq(
        schema.lifecyclePipelines.status,
        options.status as (typeof schema.LIFECYCLE_PIPELINE_STATUSES)[number],
      ),
    );
  }
    ...
    switch (options.orderBy) {
      case 'createdAt':
        query = db
          ...
          .orderBy(order(schema.lifecyclePipelines.startedAt)) as typeof query;
        break;
      case 'currentStage':
        query = db
          .select()
          .from(schema.lifecyclePipelines)
          .where(conditions.length > 0 ? and(...conditions) : undefined)
          .orderBy(order(schema.lifecyclePipelines.currentStageId)) as typeof query;
        break;
    }
  } else {
    query = db
      ...
      .from(schema.lifecyclePipelines)
      .where(conditions.length > 0 ? and(...conditions) : undefined)
      .orderBy(desc(schema.lifecyclePipelines.startedAt)) as typeof query;
  }
  ...
  if (options.limit) {
    query = db
      ...
      .from(schema.lifecyclePipelines)
      .where(conditions.length > 0 ? and(...conditions) : undefined)
      .orderBy(desc(schema.lifecyclePipelines.startedAt))
      .limit(options.limit) as typeof query;
  }
  ...
  if (options.offset) {
    query = db
      ...
      .offset(options.offset) as typeof query;
  }
 ...
 * @task T4912 - Implemented SQLite wiring
 */
export async function cancelPipeline(taskId: string, reason: string): Promise<void> {
  const db = await getDb();
  const now = new Date();
 ...
 * @task T4912
 */
export async function getPipelineStages(taskId: string): Promise<PipelineStageRecord[]> {
  const db = await getDb();
