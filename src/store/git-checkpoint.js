 ...
 * @task T4872
 */
export function makeCleoGitEnv(cleoDir) {
 ...
 * @task T4872
 */
export async function cleoGitCommand(args, cleoDir) {
    try {
 ...
 * @task T4872
 */
export function isCleoGitInitialized(cleoDir) {
 ...
 */
export async function loadStateFileAllowlist(cwd) {
    try {
        ...
        return allowlist.filter((entry) => typeof entry === 'string');
    }
    catch {
        return [];
    }
 ...
 */
async function getAllStateFiles(cwd) {
 ...
 * @task T4552
 */
export async function loadCheckpointConfig(cwd) {
    try {
        const configPath = getConfigPath(cwd);
        const config = await readJson(configPath);
        const gc = config?.gitCheckpoint;
        return {
 ...
 * @task T4872
 */
async function isCleoGitRepo(cleoDir) {
 ...
 * @task T4872
 */
async function getChangedStateFiles(cleoDir, cwd) {
    const changed = [];
    ...
    for (const stateFile of allStateFiles) {
        ...
        const untrackedResult = await cleoGitCommand(['ls-files', '--others', '--exclude-standard', '--', stateFile], cleoDir);
...
export async function shouldCheckpoint(options) {
    ...
    if (!isCleoGitInitialized(cleoDir))
        return false;
    if (!(await isCleoGitRepo(cleoDir)))
        return false;
    if (isMergeInProgress(cleoDir))
        return false;
    if (await isDetachedHead(cleoDir))
        return false;
    if (isRebaseInProgress(cleoDir))
        return false;
 ...
 * @task T4872
 */
export async function gitCheckpoint(trigger = 'auto', context, cwd) {
    // Suppression check
    if (process.env['GIT_CHECKPOINT_SUPPRESS'] === 'true') {
        return;
    }
 ...
 * @task T4872
 */
export async function gitCheckpointStatus(cwd) {
    const config = await loadCheckpointConfig(cwd);
    const cleoDir = getCleoDir(cwd);
    const lastCheckpoint = await getLastCheckpointTime(cleoDir);
 ...
 * @task T4872
 */
export async function gitCheckpointDryRun(cwd) {
    const cleoDir = getCleoDir(cwd);
    const isRepo = isCleoGitInitialized(cleoDir) && (await isCleoGitRepo(cleoDir));
    if (!isRepo)
        return [];
