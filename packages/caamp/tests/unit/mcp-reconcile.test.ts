...
describe("inferCleoLockData", () => {
  it("detects npx + cleo package as package source", () => {
    const result = inferCleoLockData(
      { command: "npx", args: ["-y", "@cleocode/cleo@latest", "mcp"] },
      "stable",
    );
  ...
  it("detects beta package spec", () => {
    const result = inferCleoLockData(
      { command: "npx", args: ["-y", "@cleocode/cleo@beta", "mcp"] },
      "beta",
    );
  ...
  it("detects dev channel as command source", () => {
    const result = inferCleoLockData(
      { command: "/home/user/cleo/dist/cli.js", args: ["mcp"] },
      "dev",
    );
  ...
  it("detects path-based command as command source", () => {
    const result = inferCleoLockData(
      { command: "./local/bin/cleo", args: ["serve"] },
      "stable",
    );
...
describe("reconcileCleoLock", () => {
  ...
  it("merges agents from multiple providers into a single backfill", async () => {
    ...
    const backfillEntry = result.backfilled.find(
      (b) => b.serverName === "cleo" && b.scope === "project",
    );
