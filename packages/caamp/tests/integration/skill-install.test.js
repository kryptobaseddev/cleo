...
describe("integration: skills install command", () => {
    ...
    it("exits when profile not found", async () => {
        ...
        const program = new Command();
        registerSkillsInstall(program);
        await expect(program.parseAsync(["node", "test", "install", "--profile", "unknown", "--all"])).rejects.toThrow("process-exit");
        expect(exitSpy).toHaveBeenCalledWith(1);
    });
    it("exits when catalog not available for --profile", async () => {
        ...
        const program = new Command();
        registerSkillsInstall(program);
        await expect(program.parseAsync(["node", "test", "install", "--profile", "core", "--all"])).rejects.toThrow("process-exit");
        expect(exitSpy).toHaveBeenCalledWith(1);
    });
    it("exits when source is missing and no profile", async () => {
        ...
        const program = new Command();
        registerSkillsInstall(program);
        await expect(program.parseAsync(["node", "test", "install", "--all"])).rejects.toThrow("process-exit");
        expect(exitSpy).toHaveBeenCalledWith(1);
    });
    ...
    it("installs from registered library (package/library type)", async () => {
        ...
        expect(mocks.recordSkillInstall).toHaveBeenCalledWith("ct-test", "library:ct-test", "library:ct-test", "library", ["claude-code"], "/tmp/canonical/demo", true);
    });
...
//# sourceMappingURL=skill-install.test.js.map