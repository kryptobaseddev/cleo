...
**Registry Operations**: `nexus.init`, `nexus.status`, `nexus.register`, `nexus.unregister`, `nexus.list`, `nexus.show`, `nexus.sync`, `nexus.sync.all`, `nexus.permission.set`, `nexus.reconcile`

**Query Operations**: `nexus.query`, `nexus.search`, `nexus.discover`, `nexus.deps`, `nexus.graph`, `nexus.path.show`/`nexus.critical-path`, `nexus.blockers.show`/`nexus.blocking`, `nexus.orphans.list`/`nexus.orphans`

**Sharing Operations**: `nexus.share.status`, `nexus.share.remotes`, `nexus.share.sync.status`, `nexus.share.snapshot.export`, `nexus.share.snapshot.import`, `nexus.share.sync.gitignore`, `nexus.share.remote.add`, `nexus.share.remote.remove`, `nexus.share.push`, `nexus.share.pull`

> **Note**: The NEXUS domain is fully A2A-compliant and uses the LAFS protocol for all agent-to-agent communication. See [CLEO-NEXUS-API.md](./CLEO-NEXUS-API.md) for detailed operation schemas, examples, and integration patterns.
