...
import { pipelineManifestAppend, pipelineManifestArchive, pipelineManifestCompact, pipelineManifestContradictions, pipelineManifestFind, pipelineManifestLink, pipelineManifestList, pipelineManifestPending, pipelineManifestRead, pipelineManifestStats, pipelineManifestSuperseded, pipelineManifestValidate, } from '../pipeline-manifest-sqlite.js';
...
describe('Pipeline Manifest SQLite (moved from memory domain)', () => {
    ...
    async function seedEntries() {
        for (const entry of SAMPLE_ENTRIES) {
            await pipelineManifestAppend(entry, testRoot);
        }
    }
    beforeEach(() => {
    ...
    describe('pipelineManifestAppend', () => {
        ...
        it('should reject invalid entry', async () => {
            const result = await pipelineManifestAppend({}, testRoot);
            expect(result.success).toBe(false);
    ...
    describe('pipelineManifestSuperseded', () => {
        it('should return empty array when no superseded entries', async () => {
            ...
            expect(result.success).toBe(true);
            expect(result.data.superseded).toBeDefined();
        });
    });
    ...
    describe('pipelineManifestValidate', () => {
        it('should validate entries for task', async () => {
            ...
            expect(result.success).toBe(true);
            expect(result.data.taskId).toBe('T001');
            expect(result.data.entriesFound).toBeGreaterThan(0);
        });
...
//# sourceMappingURL=engine-compat.test.js.map