...
import { formatSuccess as _formatSuccess, formatError, } from '../../../core/src/output.js';
/**
...
function assertShape(obj, shape, path = '') {
    ...
    for (const [key, expectedType] of Object.entries(shape)) {
        ...
        if (typeof expectedType === 'object' && expectedType !== null) {
            assertShape(record[key], expectedType, fullPath);
            continue;
        }
...
describe('Golden Parity: LAFS Envelope Structure (T4697)', () => {
    describe('Success envelope shape', () => {
        it('tasks.show envelope matches golden shape', () => {
            const json = formatSuccess({ task: { id: 'T001', title: 'Test', status: 'pending' } }, undefined, 'tasks.show');
        ...
        it('tasks.add envelope matches golden shape', () => {
            const json = formatSuccess({ task: { id: 'T002', title: 'New task' } }, 'Task created', 'tasks.add');
            const parsed = JSON.parse(json);
            assertShape(parsed, GOLDEN_SUCCESS_SHAPE);
            expect(parsed.message).toBe('Task created');
        });
        it('system.dash envelope matches golden shape', () => {
            const json = formatSuccess({ project: { name: 'cleo' }, stats: { total: 50 } }, undefined, 'system.dash');
            const parsed = JSON.parse(json);
            assertShape(parsed, GOLDEN_SUCCESS_SHAPE);
        });
        it('session.status envelope matches golden shape', () => {
            const json = formatSuccess({ session: { id: 'sess-1', status: 'active' } }, undefined, 'session.status');
            const parsed = JSON.parse(json);
            assertShape(parsed, GOLDEN_SUCCESS_SHAPE);
        });
...
//# sourceMappingURL=golden-parity.test.js.map