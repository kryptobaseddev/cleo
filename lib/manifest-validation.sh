...
source "${_MV_LIB_DIR}/exit-codes.sh"

# Try to source protocol validation (may not be available)
if [[ -f "${_MV_LIB_DIR}/protocol-validation.sh" ]]; then
    # shellcheck source=lib/protocol-validation.sh
    source "${_MV_LIB_DIR}/protocol-validation.sh"
fi

# Try to source compliance check (may not be available)
if [[ -f "${_MV_LIB_DIR}/compliance-check.sh" ]]; then
    # shellcheck source=lib/compliance-check.sh
    source "${_MV_LIB_DIR}/compliance-check.sh"
fi
