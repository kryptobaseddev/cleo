...
> **Note**: As of 2026.3, the global registry has migrated from `~/.cleo/projects-registry.json` to `~/.cleo/nexus.db` (SQLite). Legacy JSON files are automatically migrated on first use. See the CLEO NEXUS Specification for current architecture.
...
~/.cleo/nexus.db (SQLite)           .cleo/project-info.json
┌─────────────────────────┐         ┌─────────────────────────┐
...
**Location**: `~/.cleo/nexus.db` (SQLite)

> **Legacy path**: `~/.cleo/projects-registry.json` was used prior to 2026.3. Existing JSON files are automatically migrated to `nexus.db` on first use.
