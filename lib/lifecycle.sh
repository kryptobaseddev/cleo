...
if [[ -f "$_LIFECYCLE_LIB_DIR/exit-codes.sh" ]]; then
    source "$_LIFECYCLE_LIB_DIR/exit-codes.sh"
fi

# Source config (required)
if [[ -f "$_LIFECYCLE_LIB_DIR/config.sh" ]]; then
    source "$_LIFECYCLE_LIB_DIR/config.sh"
fi

# Source verification (required for gate checking)
if [[ -f "$_LIFECYCLE_LIB_DIR/verification.sh" ]]; then
    source "$_LIFECYCLE_LIB_DIR/verification.sh"
fi
