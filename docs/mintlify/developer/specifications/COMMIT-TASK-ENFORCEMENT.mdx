...
| CMSG-007 | Hook MUST provide auto-suggestion from active task context | Developer UX |
...
| SCOPE-004 | Hook SHOULD use `cleo current` for current active task |
| SCOPE-006 | Hook MUST NOT fail if no session is active |

### 4.2 Auto-suggestion Logic

**Priority**:
1. Current active task (`cleo current`)
...
if [[ -z "$TASK_ID" ]]; then
    ...
    if [[ -n "$FOCUSED_TASK" ]]; then
        ...
        echo "Active task: $FOCUSED_TASK"
        echo "Suggested format: $COMMIT_MSG ($FOCUSED_TASK)"
    else
        echo "WARNING: No task ID and no active task"
    fi
    exit 1
fi
