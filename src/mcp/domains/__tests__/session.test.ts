...
describe('SessionHandler', () => {
  ...
  describe('Handler Without Executor', () => {
    it('should handle query when executor is not provided', async () => {
      const handlerNoExecutor = new SessionHandler();

      // With dual-mode routing, status runs natively
      // In the test environment (no CLEO project), it may succeed or fail depending on context
      const result = await handlerNoExecutor.query('status', {});

      // Result depends on whether native mode finds a CLEO project
