 ...
 *
 * @remarks
 * Relocated from `packages/cleo/src/dispatch/nexus-decorator.ts` into
 * `@cleocode/runtime/gateway` (R3-K1 · T11455 · SG-RUNTIME-UNIFICATION) so the
 * nexus domain handler can import it from the runtime layer rather than a
 * cleo-internal path. The module is pure (`@cleocode/contracts` only). A thin
 * re-export shim remains at the old path for the renderer / CLI import sites.
 *
 * @task T9146
 * @task T11455
 * @module runtime/gateway/nexus-decorator
 */
