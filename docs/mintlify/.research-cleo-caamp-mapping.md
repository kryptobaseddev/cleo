...
**Note**: CAAMP is now integrated — `@cleocode/caamp` is a direct dependency in `package.json` and is used by `src/cli/commands/mcp-install.ts` for provider detection and MCP config writing.
...
- These operate on CLEO's task system, session protocol, and RCASD-IVTR+C lifecycle
