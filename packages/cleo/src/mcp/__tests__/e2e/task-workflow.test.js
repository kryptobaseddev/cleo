...
describe('E2E: Task Management Workflow', () => {
    ...
    it('should complete full task lifecycle', async () => {
        ...
        expect(showResult.data.title).toBe('Updated E2E Test Task');
        // Notes include timestamps, so check that at least one note contains the text
        const notes = showResult.data.notes;
        expect(Array.isArray(notes)
            ? notes.some((n) => n.includes('Updated during workflow test'))
            : false).toBe(true);
    ...
    it('should handle task search efficiently', async () => {
        ...
        const results = findResult.data;
        expect(Array.isArray(results)).toBe(true);
        const found = results.find((t) => t.id === taskId);
    ...
    it('should handle task dependencies correctly', async () => {
        ...
        const upstreamIds = upstream.map((d) => (typeof d === 'string' ? d : d.id || d.taskId));
        ...
        const dependentTask = showDependent.data;
