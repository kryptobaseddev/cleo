...
import { distillManifestEntry, migrateManifestJsonlToSqlite, pipelineManifestAppend, pipelineManifestArchive, pipelineManifestCompact, pipelineManifestContradictions, pipelineManifestFind, pipelineManifestLink, pipelineManifestList, pipelineManifestPending, pipelineManifestRead, pipelineManifestShow, pipelineManifestStats, pipelineManifestSuperseded, pipelineManifestValidate, readManifestEntries, } from '../pipeline-manifest-sqlite.js';
// ---------------------------------------------------------------------------
// Fixtures
// ---------------------------------------------------------------------------
const ENTRY_A = {
...
// Helpers
// ---------------------------------------------------------------------------
async function seedEntries(root, entries) {
...
describe('pipeline-manifest-sqlite', () => {
    ...
    describe('pipelineManifestAppend', () => {
        it('should append a valid entry', async () => {
            ...
            expect(result.data.entryId).toBe('T001-research');
        });
        it('should return error when entry is null', async () => {
            const result = await pipelineManifestAppend(null, testRoot);
            expect(result.success).toBe(false);
            expect(result.error?.code).toBe('E_INVALID_INPUT');
        });
        it('should return error for missing required fields', async () => {
            const incomplete = { id: 'T999', file: 'out/T999.md' };
        ...
        it('should update on duplicate id (upsert)', async () => {
            ...
            expect(show.data.title).toBe('Updated Title');
        });
    });
    // =========================================================================
    ...
    describe('pipelineManifestShow', () => {
        it('should show an existing entry', async () => {
            ...
            expect(result.success).toBe(true);
            expect(result.data.id).toBe('T001-research');
            expect(result.data.title).toBe('First Research');
        });
        ...
        it('should include fileExists: false when file does not exist', async () => {
            ...
            const result = await pipelineManifestShow('T001-research', testRoot);
            expect(result.success).toBe(true);
            expect(result.data.fileExists).toBe(false);
        });
    });
    // =========================================================================
    ...
    describe('pipelineManifestList', () => {
        ...
        it('should apply offset after filtering', async () => {
            ...
            expect(result.data.entries[0].id).toBe('T001-research');
            expect(result.data.total).toBe(3);
            expect(result.data.filtered).toBe(2);
    ...
    describe('pipelineManifestStats', () => {
        it('should return aggregate stats', async () => {
            ...
            expect(result.success).toBe(true);
            expect(result.data.total).toBe(3);
            expect(result.data.byType).toHaveProperty('research');
            expect(result.data.byStatus).toBeDefined();
        });
    ...
    describe('pipelineManifestCompact', () => {
        ...
        it('should remove duplicate contentHash entries (keeping newest)', async () => {
            ...
            const countBefore = resultBefore.data.total;
            const compact = await pipelineManifestCompact(testRoot);
            expect(compact.success).toBe(true);
            expect(compact.data.remainingEntries).toBeLessThanOrEqual(countBefore);
        });
    });
    // =========================================================================
    ...
    describe('pipelineManifestContradictions', () => {
        it('should detect contradictory findings on shared topic', async () => {
            ...
            expect(result.success).toBe(true);
            expect(result.data.contradictions).toBeDefined();
            // These two entries share 'engine' topic with deprecated vs recommended contradiction
            expect(result.data.contradictions.length).toBeGreaterThanOrEqual(1);
        });
        ...
        it('should filter by topic param', async () => {
            ...
            expect(result.data.contradictions.length).toBe(0);
        });
    });
    // =========================================================================
    ...
    describe('pipelineManifestSuperseded', () => {
        it('should find superseded entries on same topic+type', async () => {
            ...
            expect(result.success).toBe(true);
            expect(result.data.superseded.length).toBeGreaterThanOrEqual(1);
        });
        it('should return empty superseded when entries have different types', async () => {
            ...
            expect(result.data.superseded.length).toBe(0);
        });
    });
    // =========================================================================
    ...
    describe('pipelineManifestLink', () => {
        it('should link an entry to a new task', async () => {
            ...
            const show = await pipelineManifestShow('T001-research', testRoot);
            expect(show.data.linked_tasks).toContain('T999');
        });
    ...
    describe('distillManifestEntry', () => {
        it('should return skipped=true (phase 3 pending)', async () => {
            ...
            expect(result.data.reason).toBe('distillation_pending_phase3');
        });
    });
    // =========================================================================
...
//# sourceMappingURL=pipeline-manifest-sqlite.test.js.map