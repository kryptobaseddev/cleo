...
When `--write-changelog` flag is used with `cleo release ship`:

```bash
# Call unified changelog system
./scripts/generate-changelog.sh

# Generates CHANGELOG.md in Mintlify/Keep a Changelog format
# Sources:
#   - Git commit history
#   - Task metadata from release
#   - Labels (feature/enhancement/bug)
```

**Unified System**: Use `scripts/generate-changelog.sh` exclusively. Deprecate `lib/changelog.sh` (per T2539 consensus).

**Flags**:
- `--write-changelog`: Generate changelog (default: prompt user)
- `--changelog-format <markdown|json>`: Output format (default: markdown)
