...
export async function checkPrerequisites(
  ...
  const canForce = blockers.every(
    (b) => b.severity !== 'error' || b.reason !== 'Stage not initialized',
  );
...
export function setStageStatus(
  ...
  if (!currentState) {
    throw new CleoError(ExitCode.NOT_FOUND, `Stage ${stage} not found in context`);
  }
 ...
 * @task T4800
 */
export function getStageStatus(stage: Stage, context: StateMachineContext): StageStatus {
 ...
 * @task T4800
 */
export function getCurrentStageState(context: StateMachineContext): StageState {
...
export function isTerminalState(context: StateMachineContext): boolean {
  return context.currentStage === 'release' && context.stages['release'].status === 'completed';
}
...
export async function validateTransitions(
  ...
  return Promise.all(transitions.map((t) => validateTransition(t, context)));
}
