<script lang="ts">
   ...
   */
  let shouldUseGpu = $derived(useGpuRenderer || filteredGraph.nodes.length > 2000);
...
<div class="lb-page">
  ...
  <div class="lb-header">
    ...
    <div class="header-controls">
      ...
      <button
        class="renderer-btn"
        class:active={shouldUseGpu}
        onclick={() => (useGpuRenderer = !useGpuRenderer)}
        title={shouldUseGpu
          ? 'GPU mode active (cosmos.gl) — click to switch to Standard'
          : 'Standard mode active (sigma) — click to switch to GPU'}
      >
        {shouldUseGpu ? 'GPU mode' : 'Standard'}
      </button>
    </div>
  </div>
    ...
    <div class="lb-canvas">
      {#if shouldUseGpu}
        <LivingBrainCosmograph
          ...
          {pulsingNodes}
          {pulsingEdges}
          onInitFailed={() => { useGpuRenderer = false; }}
        />
      {:else}
        <LivingBrainGraph
          nodes={filteredGraph.nodes}
