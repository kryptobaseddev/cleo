...
|------|---------|------------|
| `query` | Read-only operations | Find tasks, check status, analyze dependencies |
| `mutate` | Write operations | Create tasks, start sessions, complete work |

This 2-tool design uses ~1,800 tokens of context -- a **94% reduction** compared to exposing each operation as a separate tool.

<Info>
The MCP Server covers **256 operations** (145 query + 111 mutate) across 10 canonical domains: tasks, session, memory, check, pipeline, orchestrate, tools, admin, nexus, and sticky. Cross-project share operations live under `nexus.share.*`.
</Info>
...
- **CLEO** installed (`npm install -g @cleocode/cleo`) and initialized (`cleo init`)
- **Node.js** >= 24 (for the MCP server runtime)
