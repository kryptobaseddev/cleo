...
import { RECOMMENDATION_ERROR_CODES, normalizeRecommendationCriteria, rankSkills, scoreSkillRecommendation, validateRecommendationCriteria, } from "../../src/core/skills/recommendation.js";
...
describe("skills recommendation criteria parsing", () => {
    ...
    it("returns stable validation error code for invalid field type", () => {
        const result = validateRecommendationCriteria({ mustHave: 42 });
        expect(result.valid).toBe(false);
        expect(result.issues[0]?.code).toBe(RECOMMENDATION_ERROR_CODES.QUERY_INVALID);
    });
});
describe("skills recommendation scoring and ranking", () => {
    ...
    it("includes reason codes and exclusion penalties", () => {
        ...
        const result = rankSkills([legacy, modern], {
            query: "typed docs",
            prefer: "typed",
            exclude: "jquery",
        }, { includeDetails: true });
...
//# sourceMappingURL=recommendation.test.js.map