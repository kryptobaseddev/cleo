/**
 * Operations Registry — canonical OPERATIONS data for the CLEO dispatch layer.
 *
 * Relocated from `packages/cleo/src/dispatch/registry.ts` in T10061 /
 * T9833b (E-CLI-BOUNDARY · Saga T9831 SG-ARCH-SOLID). The data array and
 * builder helpers live here so that contracts owns the registry shape while
 * `packages/cleo/src/dispatch/registry.ts` becomes a thin re-export shim.
 *
...
/**
 * Type-constraining builder for a single {@link OperationDef}.
 *
 * Acts as an identity function: accepts an OperationDef literal and returns
 * it unchanged. The purpose is to enforce the type at the call site so that
 * editors provide auto-complete and TypeScript catches structural errors at
 * authoring time rather than at array-level `satisfies` checks.
 */
export function defineOp(def: OperationDef): OperationDef {
  return def;
}

/**
 * Groups N {@link OperationDef} entries that share a {@link CanonicalDomain}.
 *
 * Returns the flat array of definitions — the grouping is purely a
 * documentation/authoring convenience so large domains can be broken up
 * into domain-scoped sections without changing the flat {@link OPERATIONS}
 * shape that the dispatcher consumes.
 */
export function defineDomain(_domain: CanonicalDomain, ops: OperationDef[]): OperationDef[] {
  return ops;
}
...
export const OPERATIONS: OperationDef[] = [
  ...
  },
  {
    ...
    domain: 'tasks',
    operation: 'traverse',
    description: 'tasks.traverse (query) — WorkGraph traversal from a task root',
    tier: 1,
    idempotent: true,
    sessionRequired: false,
    requiredParams: ['rootId', 'direction'],
    params: [
      {
        name: 'rootId',
        type: 'string',
        required: true,
        description: 'Root task ID for the traversal',
        cli: { positional: true },
      },
      {
        name: 'direction',
        type: 'string',
        required: true,
        description: 'Traversal direction to project from the root task',
        enum: ['ancestors', 'descendants', 'upstream', 'downstream'],
      },
      {
        name: 'maxDepth',
        type: 'number',
        required: false,
        description: 'Maximum traversal depth to include',
      },
      {
        name: 'includeRelations',
        type: 'boolean',
        required: false,
        description: 'Include non-containment relation edges',
      },
      { name: 'cursor', type: 'string', required: false, description: 'Opaque pagination cursor' },
      { name: 'limit', type: 'number', required: false, description: 'Maximum nodes to return' },
    ] satisfies ParamDef[],
  },
  {
    gateway: 'query',
    domain: 'tasks',
    operation: 'tree',
    description: 'tasks.tree (query) — WorkGraph containment tree projection',
    tier: 1,
    idempotent: true,
    sessionRequired: false,
    ...
    params: [
      {
        ...
        type: 'string',
        required: true,
        description: 'Root task ID for the tree projection',
        cli: { positional: true },
      },
      {
        name: 'maxDepth',
        type: 'number',
        required: false,
        description: 'Maximum containment depth to include',
      },
      { name: 'cursor', type: 'string', required: false, description: 'Opaque pagination cursor' },
      { name: 'limit', type: 'number', required: false, description: 'Maximum nodes to return' },
    ] satisfies ParamDef[],
  },
  {
    ...
    domain: 'tasks',
    operation: 'rollup',
    description: 'tasks.rollup (query) — WorkGraph direct and subtree status/type summary',
    tier: 1,
    idempotent: true,
    sessionRequired: false,
    requiredParams: ['rootId'],
    params: [
      {
        ...
        type: 'string',
        required: true,
        description: 'Root task ID for the rollup',
        cli: { positional: true },
      },
      {
        name: 'expectedDirectRollup',
        type: 'string',
        required: false,
        description: 'Optional serialized expected direct rollup for stale-projection diagnostics',
      },
    ] satisfies ParamDef[],
  },
  {
    ...
    domain: 'tasks',
    operation: 'frontier',
    description: 'tasks.frontier (query) — WorkGraph ready and blocked frontier under a root',
    tier: 1,
    idempotent: true,
    sessionRequired: false,
    requiredParams: ['rootId'],
    params: [
      {
        ...
        type: 'string',
        required: true,
        description: 'Root task ID for the frontier',
        cli: { positional: true },
      },
      {
        name: 'role',
        type: 'string',
        required: false,
        description: 'Optional assignee/role lane filter',
      },
    ] satisfies ParamDef[],
  },
  {
    ...
    params: [
      {
        ...
        type: 'string',
        required: true,
        description: 'Root task ID for the WorkGraph audit',
        cli: { positional: true },
      },
      {
        name: 'maxDepth',
        type: 'number',
        required: false,
        description: 'Maximum descendant depth to include in audit projections',
      },
      {
        name: 'includeRelations',
        type: 'boolean',
        required: false,
        description: 'Include relation-edge diagnostics in the audit result',
      },
      { name: 'cursor', type: 'string', required: false, description: 'Opaque pagination cursor' },
      {
        name: 'limit',
  ...
  },
  {
    ...
    description: 'tasks.completion.list (query) — list typed completion criteria results',
    tier: 1,
    idempotent: true,
    sessionRequired: false,
    requiredParams: ['taskId'],
    params: [
      {
        ...
        type: 'string',
        required: true,
        description: 'Task ID to list completion criteria for',
        cli: { positional: true },
      },
      {
        name: 'status',
        type: 'string',
        required: false,
        description: 'Optional criterion status filter',
        enum: ['satisfied', 'unsatisfied', 'waived', 'replaced'],
      },
      {
        name: 'kind',
        type: 'string',
        required: false,
        description: 'Optional criterion kind filter',
        enum: ['text', 'evidence_bound', 'child_task'],
      },
    ] satisfies ParamDef[],
  },
  {
    ...
    params: [
      {
        ...
        type: 'string',
        required: true,
        description: 'Task ID to evaluate completion readiness for',
        cli: { positional: true },
      },
      {
        name: 'includeContext',
        type: 'boolean',
        required: false,
        description: 'Include bounded audit context pack',
      },
      {
        name: 'limit',
        type: 'number',
        required: false,
        description: 'Maximum audit context events when context is included',
      },
      {
        name: 'since',
        type: 'string',
        required: false,
        description: 'Lower ISO timestamp bound for audit context',
      },
      {
        name: 'relationDepth',
        type: 'number',
        required: false,
        description: 'Related-task audit context depth',
      },
    ] satisfies ParamDef[],
  },
  {
    gateway: 'query',
    domain: 'tasks',
    operation: 'completion.explain',
    description: 'tasks.completion.explain (query) — explain completion blockers and stale state',
    tier: 1,
    idempotent: true,
    sessionRequired: false,
    requiredParams: ['taskId'],
    params: [
      {
        ...
        type: 'string',
        required: true,
        description: 'Task ID to explain completion readiness for',
        cli: { positional: true },
      },
      {
        name: 'includeContext',
    ...
    params: [
      {
        ...
        type: 'string',
        required: true,
        description: 'Task ID whose completion projection should be repaired',
        cli: { positional: true },
      },
      {
        name: 'dryRun',
        type: 'boolean',
        required: false,
        description: 'Preview repair actions without writing projection state',
      },
    ] satisfies ParamDef[],
  },
  {
  ...
  },
  {
    ...
    params: [
      ...
      { name: 'limit', type: 'number' as const, required: false, description: 'Maximum matches' },
      {
        name: 'type',
        type: 'string' as const,
        required: false,
  ...
  },
  {
    ...
    requiredParams: ['ownerId'],
    params: [
      { name: 'ownerId', type: 'string' as const, required: true, description: 'Document owner' },
      {
        name: 'name',
        type: 'string' as const,
        required: false,
