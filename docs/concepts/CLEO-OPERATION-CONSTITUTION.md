...
Additional workshop vocabulary (`Thread`, `Loom` as epic-frame, `Tapestry`, `Tessera`, `Cogs`, `Cascade`, `Tome`, `Sticky Notes`) is also conceptual only. These terms MAY be used in documentation and UX language, but they MUST map onto the existing 10 domains and MUST NOT be introduced as new domains without constitutional change.
