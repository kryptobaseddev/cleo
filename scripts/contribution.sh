...
usage() {
    cat << 'EOF'
Usage: cleo contribution <subcommand> [OPTIONS]

Validate contribution protocol compliance for shared codebase work.

Subcommands:
  validate <TASK_ID>     Validate contribution protocol compliance for task
...
Validation Checks (CONT-*):
  CONT-002: Provenance tags @task T#### in new code (MUST)
  CONT-003: Tests must pass (delegated to CI) (MUST)
  CONT-007: agent_type = "implementation" (MUST)

Exit Codes:
  0:  Valid or non-strict mode
  65: Protocol violations (EXIT_PROTOCOL_CONTRIBUTION) in strict mode

Examples:
  # Validate task's contribution output
  cleo contribution validate T1234

  # Check manifest entry directly
  cleo contribution check manifest.json

  # Strict mode (exit on violations)
  cleo contribution validate T1234 --strict
...
Note:
  Contribution protocol validates work from multiple agents in shared repos.
  Requires @task/@contribution provenance tags for attribution tracking.
EOF
}
...
# validate_task - Validate contribution protocol for a task
# Args: $1 = task_id
validate_task() {
    local task_id="$1"
