...
if [[ -f "$_RELEASE_CONFIG_LIB_DIR/config.sh" ]]; then
    source "$_RELEASE_CONFIG_LIB_DIR/config.sh"
fi

# Source exit codes if available
if [[ -f "$_RELEASE_CONFIG_LIB_DIR/exit-codes.sh" ]]; then
    source "$_RELEASE_CONFIG_LIB_DIR/exit-codes.sh"
fi
