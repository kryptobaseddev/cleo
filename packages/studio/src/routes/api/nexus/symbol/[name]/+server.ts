...
export const GET: RequestHandler = ({ params }) => {
  ...
  if (!centerRow) {
    return json({ error: `Symbol "${name}" not found` }, { status: 404 });
  }
  ...
  const subgraphEdges =
    allIds.length > 0
      ? (db
          ...
          .all(...allIds, ...allIds) as { source_id: string; target_id: string; type: string }[])
      : [];
  ...
  const edges: EgoEdge[] = subgraphEdges.map((e) => ({
    source: e.source_id,
    target: e.target_id,
    type: e.type,
  }));

  return json({ center: centerId, nodes, edges } satisfies EgoNetwork);
};
