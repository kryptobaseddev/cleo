...
<Tip>
  If you want the agent to just plan without implementing, say so: "Create a
  task for this but don't start coding yet."
</Tip>
...
<Info>
  **Tips for good breakdowns:** - Each sub-task should be completable in one
  focused session - Keep it to 5-7 sub-tasks per parent — if you need more, add
  grouping - Tell the agent which tasks depend on others so it can suggest the
  right order
</Info>
...
<Tip>
  Start each conversation with "Show me the project dashboard" or "What's the
  status?" to give the agent (and yourself) context on where things stand.
</Tip>
...
| --------------- | -------------------- | ------------------------- |
| **query**  | Read-only operations | Never modifies state      |
...
```
1. query   → domain: session,  operation: status
2. mutate  → domain: tasks,    operation: create    → params: { title, description, priority }
3. mutate  → domain: session,  operation: start     → params: { scope: "task:T####", name, autoFocus: true }
4. mutate  → domain: session,  operation: tasks.start → params: { taskId: "T####" }
5.   ... do the work ...
6. mutate  → domain: tasks,    operation: update    → params: { taskId, notes: "progress..." }
7. mutate  → domain: tasks,    operation: complete  → params: { taskId, notes: "what was done" }
8. mutate  → domain: session,  operation: end       → params: { note: "summary" }
```

## Workflow: Bug Investigation

```
1. query   → domain: system,   operation: dash
2. query   → domain: tasks,    operation: find      → params: { query: "search terms" }
3. mutate  → domain: tasks,    operation: create    → params: { title, description, priority: "high" }
4. mutate  → domain: session,  operation: tasks.start → params: { taskId: "T####" }
5.   ... investigate ...
6. mutate  → domain: tasks,    operation: complete  → params: { taskId, notes: "root cause and fix" }
```

## Workflow: Task Decomposition

```
1. mutate  → domain: tasks,    operation: create    → params: { title, description, type: "epic" }
2. mutate  → domain: session,  operation: start     → params: { scope: "epic:T####", autoFocus: true }
3. mutate  → domain: tasks,    operation: create    → params: { title, description, parent: "T####" }
   ... repeat for each sub-task ...
4. mutate  → domain: tasks,    operation: update    → params: { taskId: "T###", depends: ["T###"] }
5. query   → domain: tasks,    operation: tree      → params: { taskId: "T####" }
6. query   → domain: tasks,    operation: next
```

## Workflow: GitHub Issue Filing

```
1. mutate  → domain: issues,   operation: create_bug → params: { title, body, severity, dryRun: true }
2.   ... user confirms preview ...
3. mutate  → domain: issues,   operation: create_bug → params: { title, body, severity }
```
...
| **tasks**   | `depends`     | Dependency graph            | `taskId`                   |
| **tasks**   | `blockers`    | What's blocking             | `taskId`                   |
| **session** | `status`      | Current session             | -                          |
| **session** | `list`        | All sessions                | -                          |
| **tasks**   | `current`     | Current focus               | -                          |
| **admin**   | `dash`        | Project dashboard           | -                          |
| **admin**   | `version`     | CLEO version                | -                          |
| **admin**   | `doctor`      | Health check                | -                          |
| **tools**   | `issue.diagnostics` | System info for bug reports | -                    |
...
| **tasks**   | `archive`        | Archive task       | `taskId`               |
| **session** | `start`          | Begin session      | `scope`, `name`        |
| **session** | `end`            | End session        | `notes?`               |
| **tasks**   | `start`          | Set active task    | `taskId`               |
| **tasks**   | `stop`           | Clear focus        | `taskId`               |
| **tools**   | `issue.add.bug`     | File bug report    | `title`, `body`     |
| **tools**   | `issue.add.feature` | Request feature    | `title`, `body`     |
| **tools**   | `issue.add.help`    | Ask question       | `title`, `body`     |

<Note>
  For the complete list of all 10 domains and 256 operations (145 query + 111
  mutate), see the [MCP Server Specification](../specs/MCP-SERVER-SPECIFICATION)
  or inspect the source of truth: `src/mcp/gateways/query.ts` and
...
| `E_SIBLING_LIMIT`     | 12   | Configured child limit exceeded |
