---
title: "Multi-Session Binding"
description: "Automatic session context binding with agent detection, resolution priority, and actionable error handling"
icon: "file-code"
---
...
|---------|-----------------|----------|
| `start/clear/show` | Yes | Uses resolved session |
...
cmd_focus_set() {
    ...
    if ! session_id=$(resolve_current_session_id "$provided_session"); then
        ...
        output_error_actionable "E_SESSION_REQUIRED" \
            ...
            36 true \
            "Session required for task operations" \
...
| 35 | `E_TASK_CLAIMED` | Task active in another session | Yes |
| 36 | `E_SESSION_REQUIRED` | Session context required | Yes |
| 38 | `E_ACTIVE_TASK_REQUIRED` | Active task required for session start | Yes |
...
| `E_ACTIVE_TASK_REQUIRED` | `cleo session start --scope <scope> --focus <task>` |
