...
import { cleanupIntegrationTest, createTestTask, getAuditLogEntries, setupIntegrationTest, startTestSession, taskExists, } from '../../__tests__/integration-setup.js';
describe('mutate Gateway Integration', () => {
    ...
    describe('Tasks Domain', () => {
        ...
        beforeAll(async () => {
            for (const label of POOL_LABELS) {
                const id = await createTestTask(context, `Mutate Pool [${label}] ${Date.now()}`, `Task for testing ${label} operations`, { parent: testEpicId });
                taskPool.push(id);
            }
        }, 120000);
        ...
        it('should unarchive task', async () => {
            ...
            expect(result.success || result.error?.code?.includes('NOT_FOUND') || result.exitCode === 4).toBe(true);
        });
    ...
    describe('Session Domain', () => {
        ...
        it('should set focused task', async () => {
            ...
            const taskId = context.createdTaskIds[0] ||
    ...
    describe('Admin Domain', () => {
        ...
        it('should create backup', async () => {
            ...
            expect(result.exitCode === 0 ||
                ...
                result.exitCode !== undefined).toBe(true);
        });
    ...
    describe('Validation Gates', () => {
        it('should enforce title/description difference', async () => {
            ...
            if (result.success) {
                ...
                if (id)
                    context.createdTaskIds.push(id);
            }
            else {
        ...
        it('should enforce sibling limit', async () => {
            ...
            const siblingParent = await createTestTask(context, 'Sibling Test Parent', 'Parent for sibling limit test');
    ...
    describe('Idempotency', () => {
        it('should handle duplicate task completion', async () => {
            const taskId = await createTestTask(context, 'Idempotent Task', 'Test idempotency for completion');
            // Complete first time
    ...
    describe('Response Format', () => {
        it('should return consistent success format', async () => {
            ...
            if (createdId)
                context.createdTaskIds.push(createdId);
        });
...
//# sourceMappingURL=mutate.integration.test.js.map