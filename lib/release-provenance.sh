...
if [[ -f "$_RELEASE_PROV_LIB_DIR/file-ops.sh" ]]; then
    source "$_RELEASE_PROV_LIB_DIR/file-ops.sh"
fi

# Source config for unified config access
if [[ -f "$_RELEASE_PROV_LIB_DIR/config.sh" ]]; then
    source "$_RELEASE_PROV_LIB_DIR/config.sh"
fi

# Source release config for security settings
if [[ -f "$_RELEASE_PROV_LIB_DIR/release-config.sh" ]]; then
    source "$_RELEASE_PROV_LIB_DIR/release-config.sh"
fi

# Source exit codes if available
if [[ -f "$_RELEASE_PROV_LIB_DIR/exit-codes.sh" ]]; then
    source "$_RELEASE_PROV_LIB_DIR/exit-codes.sh"
fi
