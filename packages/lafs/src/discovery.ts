 ...
 * Reference: specs/external/agent-discovery.md (RFC 8615 well-known URI)
 */
...
export interface DiscoveryConfig {
  /** Agent information (A2A v1.0 Agent Card, required). */
 ...
 *
 * @remarks
 * Controls path routing and caching behavior for the discovery middleware.
 *
 * @example
 * ```typescript
 ...
 *
 * @remarks
 * When `autoIncludeLafsExtension` is set, the LAFS extension is appended to
...
/**
 * Create Express middleware for serving the A2A Agent Card.
 *
 ...
 *
 * @remarks
 * Serves an A2A-compliant Agent Card at the well-known URI defined by
 * RFC 8615 (default `/.well-known/agent-card.json`). Supports ETag-based
 * conditional requests, HEAD requests, and configurable cache headers.
 *
 * @example
 * ```typescript
...
export function discoveryMiddleware(
  ...
  if (!config.agent) {
    throw new Error("Discovery config requires 'agent' (A2A v1.0 Agent Card)");
  }

  // Cache serialized document so GET and HEAD return identical ETags
  let cachedJson: string | null = null;

  function getSerializedDoc(req: Request): string {
    if (!cachedJson) {
      cachedJson = JSON.stringify(buildAgentCard(config, req), null, 2);
    }
    return cachedJson;
  }
  ...
  return function discoveryHandler(req: Request, res: Response, next: NextFunction): void {
    ...
    if (req.method === 'HEAD') {
      ...
      const json = getSerializedDoc(req);
