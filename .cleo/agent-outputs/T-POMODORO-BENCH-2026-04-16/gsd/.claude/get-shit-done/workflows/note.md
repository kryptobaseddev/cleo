...
- **Global scope**: `/tmp/pomodoro-bench/gsd/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/tmp/pomodoro-bench/gsd/.claude/notes/`)
...
2. Glob `/tmp/pomodoro-bench/gsd/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/tmp/pomodoro-bench/gsd/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/tmp/pomodoro-bench/gsd/.claude/notes/` — works in any directory
