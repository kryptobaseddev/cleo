...
describe('label operations', () => {
    ...
    describe('showLabelTasks', () => {
        it('returns tasks with matching label', async () => {
            ...
            const result = (await showLabelTasks('bug', env.tempDir, accessor));
            expect(result.label).toBe('bug');
            expect(result.count).toBe(2);
            expect(result.tasks.map((t) => t.id).sort()).toEqual(['T001', 'T003']);
        });
    ...
    describe('getLabelStats', () => {
        it('returns label statistics', async () => {
            ...
            const stats = (await getLabelStats(env.tempDir, accessor));
