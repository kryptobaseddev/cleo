...
@test "get_config_file returns cleo-config.json" {
    unset CLEO_DIR
    result=$(get_config_file)
    assert_equal "$result" ".cleo/cleo-config.json"
}

@test "get_log_file returns cleo-log.json" {
    unset CLEO_DIR
    result=$(get_log_file)
    assert_equal "$result" ".cleo/cleo-log.json"
}
