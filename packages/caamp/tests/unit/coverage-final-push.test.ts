...
describe("coverage: catalog.ts discovery", () => {
  ...
  it("isCatalogAvailable returns false when no library is registered", async () => {
    ...
    if (saved !== undefined) process.env["CAAMP_SKILL_LIBRARY"] = saved;
  });
  ...
  it("registerSkillLibrary sets a library directly", async () => {
    ...
    const mockLibrary = {
      ...
      listSkills: () => [],
      getSkill: () => undefined,
      getSkillPath: (n: string) => `/tmp/${n}`,
      getSkillDir: (n: string) => `/tmp/${n}`,
      readSkillContent: () => "",
      getCoreSkills: () => [],
      getSkillsByCategory: () => [],
      getSkillDependencies: () => [],
      resolveDependencyTree: (n: string[]) => n,
      listProfiles: () => [],
    ...
    catalog.registerSkillLibrary(mockLibrary as any);
...
describe("coverage: recommendation.ts branch gaps", () => {
  it("exercises all recommendation scoring branches", async () => {
    ...
    } = await import("../../src/core/skills/recommendation.js");

    // validateRecommendationCriteria with non-string query
    const invalid1 = validateRecommendationCriteria({ query: 123 as any });
    expect(invalid1.valid).toBe(false);
    expect(invalid1.issues[0]!.field).toBe("query");

    // non-string/non-array mustHave
    const invalid2 = validateRecommendationCriteria({ query: "test", mustHave: 123 as any });
    expect(invalid2.valid).toBe(false);

    // non-string/non-array prefer
    const invalid3 = validateRecommendationCriteria({ query: "test", prefer: 123 as any });
    expect(invalid3.valid).toBe(false);

    // non-string/non-array exclude
    const invalid4 = validateRecommendationCriteria({ query: "test", exclude: 123 as any });
...
describe("coverage: recommendation-api.ts", () => {
  ...
  it("formatSkillRecommendations handles ranking with entries (json, with details)", async () => {
    ...
    const mockRanking = [
      {
        ...
        score: 15,
        reasons: [{ code: "QUERY_MATCH" as const, detail: "1" }],
    ...
    const obj = jsonResult as Record<string, unknown>;
    ...
    const obj2 = jsonNoDetails as Record<string, unknown>;
    ...
    expect((humanResult as string).includes("Recommended")).toBe(true);
  });
...
describe("coverage: marketplace client.ts", () => {
  it("search deduplicates by scopedName keeping higher stars", async () => {
    ...
    const client = new MarketplaceClient([mockAdapter1, mockAdapter2] as any);
  ...
  it("search throws MarketplaceUnavailableError when all adapters fail", async () => {
    ...
    const client = new MarketplaceClient([failAdapter] as any);
  ...
  it("search handles partial adapter failure", async () => {
    ...
    const client = new MarketplaceClient([failAdapter, okAdapter] as any);
    const results = await client.search("test");
    expect(results.length).toBe(1);
  });
  ...
  it("getSkill returns first match", async () => {
    ...
    const client = new MarketplaceClient([adapter1, adapter2] as any);
  ...
  it("getSkill returns null when no adapter has it", async () => {
    ...
    const client = new MarketplaceClient([adapter] as any);
    const result = await client.getSkill("@a/test");
...
describe("coverage: detection.ts branch gaps", () => {
  ...
  it("detectProjectProvider returns false for provider without pathProject", async () => {
    ...
    const provider = { pathProject: "", id: "test" } as any;
...
describe("coverage: providers.ts branch gaps", () => {
  it("exercises all provider query functions", async () => {
    const {
      ...
      getProviderCount,
      getRegistryVersion,
    } = await import("../../src/core/registry/providers.js");
...
describe("coverage: mcp installer.ts", () => {
  it("buildServerConfig handles different source types", async () => {
    ...
    const remote = buildServerConfig(
      { type: "remote", value: "https://example.com/sse" },
      "sse",
    ...
    const remoteNoHeaders = buildServerConfig(
      { type: "remote", value: "https://example.com" },
    );
    expect(remoteNoHeaders.type).toBe("http");
    expect(remoteNoHeaders.headers).toBeUndefined();

    // Remote with empty headers
    const remoteEmptyHeaders = buildServerConfig(
      { type: "remote", value: "https://example.com" },
      undefined,
      {},
    );
...
describe("coverage: orchestration stableStringify (internal)", () => {
  it("exercises detectMcpConfigConflicts which uses stableStringify", async () => {
    ...
    const conflicts = await detectMcpConfigConflicts(
      [provider],
      [{ serverName: "test", config: { type: "http", url: "https://example.com" }, scope: "project" as const }],
      "/tmp",
    );
    expect(conflicts.some(c => c.code === "unsupported-transport")).toBe(true);

    // Operation with unsupported headers
    const headerConflicts = await detectMcpConfigConflicts(
      [provider],
      [{ serverName: "test", config: { command: "npx", headers: { Authorization: "test" } }, scope: "project" as const }],
      "/tmp",
    );
...
describe("coverage: additional branch exercises", () => {
  ...
  it("getAgentsMcpDir returns correct paths", async () => {
    const {
      ...
      getLockFilePath,
      resolveProjectPath,
    } = await import("../../src/core/paths/standard.js");
...
describe("coverage: catalog.ts CAAMP_SKILL_LIBRARY location discovery", () => {
  it("env var CAAMP_SKILL_LIBRARY with skills.json (line 85-86)", async () => {
    ...
    try {
      ...
      if (saved !== undefined) process.env["CAAMP_SKILL_LIBRARY"] = saved;
      else delete process.env["CAAMP_SKILL_LIBRARY"];
      await rm(tmpDir, { recursive: true, force: true });
    }
  });
});
