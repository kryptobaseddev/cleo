...
} from './operation-gate-validators.js';
...
export class VerificationGate {
  ...
  private buildSuccessResult(layers: Record<GateLayer, LayerResult>): VerificationResult {
    return {
      ...
      layers,
      totalViolations: 0,
      exitCode: ProtocolExitCode.SUCCESS,
  ...
  private buildFailureResult(
    ...
    let exitCode = ProtocolExitCode.E_VALIDATION_ERROR;
    let category = ErrorCategory.GENERAL;

    if (blockedAt === GateLayer.SCHEMA) {
      exitCode = ProtocolExitCode.E_VALIDATION_ERROR;
      category = ErrorCategory.GENERAL;
   ...
   */
  private determineSemanticExitCode(violations: GateViolation[]): ProtocolExitCode {
    for (const v of violations) {
      if (v.code.includes('PARENT')) return ProtocolExitCode.E_PARENT_NOT_FOUND;
      if (v.code.includes('DEPTH')) return ProtocolExitCode.E_DEPTH_EXCEEDED;
      if (v.code.includes('SIBLING')) return ProtocolExitCode.E_SIBLING_LIMIT;
      if (v.code.includes('CIRCULAR')) return ProtocolExitCode.E_CIRCULAR_REFERENCE;
      if (v.code.includes('SESSION')) return ProtocolExitCode.E_SESSION_REQUIRED;
    }
    return ProtocolExitCode.E_VALIDATION_ERROR;
  }

  /**
   * Determine referential layer exit code from violations
   */
  private determineReferentialExitCode(violations: GateViolation[]): ProtocolExitCode {
    for (const v of violations) {
      if (v.code.includes('NOT_FOUND')) return ProtocolExitCode.E_NOT_FOUND;
      if (v.code.includes('PARENT')) return ProtocolExitCode.E_PARENT_NOT_FOUND;
      if (v.code.includes('DEPENDENCY')) return ProtocolExitCode.E_DEPENDENCY_ERROR;
    }
    return ProtocolExitCode.E_NOT_FOUND;
  }

  /**
   * Determine protocol layer exit code from violations
   */
  private determineProtocolExitCode(violations: GateViolation[]): ProtocolExitCode {
    for (const v of violations) {
      if (v.code.includes('RESEARCH')) return ProtocolExitCode.E_PROTOCOL_RESEARCH;
      if (v.code.includes('CONSENSUS')) return ProtocolExitCode.E_PROTOCOL_CONSENSUS;
      if (v.code.includes('SPECIFICATION')) return ProtocolExitCode.E_PROTOCOL_SPECIFICATION;
      if (v.code.includes('DECOMPOSITION')) return ProtocolExitCode.E_PROTOCOL_DECOMPOSITION;
      if (v.code.includes('IMPLEMENTATION')) return ProtocolExitCode.E_PROTOCOL_IMPLEMENTATION;
      if (v.code.includes('CONTRIBUTION')) return ProtocolExitCode.E_PROTOCOL_CONTRIBUTION;
      if (v.code.includes('RELEASE')) return ProtocolExitCode.E_PROTOCOL_RELEASE;
      if (v.code.includes('VALIDATION')) return ProtocolExitCode.E_PROTOCOL_VALIDATION;
      if (v.code.includes('TESTING')) return ProtocolExitCode.E_TESTS_SKIPPED;
      if (v.code.includes('LIFECYCLE')) return ProtocolExitCode.E_LIFECYCLE_GATE_FAILED;
    }
    return ProtocolExitCode.E_PROTOCOL_GENERIC;
  }
