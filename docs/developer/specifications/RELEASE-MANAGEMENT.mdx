...
cleo release ship <version> [OPTIONS]  # active → released
  ...
  --write-changelog         # Generate changelog via generate-changelog.sh
...
Tasks are included if they meet ANY of:
- Explicitly assigned via `cleo release plan --tasks T001,T002`
- Have label matching version (e.g., `v0.74.0` or `0.74.0`)
- Have `changelog` or `release` label AND completed between release dates
- Completed between previous release date and current release
...
# Changelog is AUTOMATICALLY generated and appended
# No need for --write-changelog flag (deprecated)
