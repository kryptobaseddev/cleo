...
function makeTempDir() {
    const dir = join(tmpdir(), `cleo-adapter-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    mkdirSync(dir, { recursive: true });
    return dir;
}
function createManifest(dir, manifest) {
    const adaptersDir = join(dir, 'packages', 'adapters', manifest.provider);
...
const MOCK_MANIFEST = {
    ...
    detectionPatterns: [
        { type: 'env', pattern: 'CLEO_TEST_ADAPTER_DETECT', description: 'Test env var' },
    ],
};
// --- Discovery tests ---
...
describe('AdapterManager', () => {
    ...
    it('activate throws for unknown adapter', async () => {
        const manager = AdapterManager.getInstance(tempDir);
        await expect(manager.activate('nonexistent')).rejects.toThrow('Adapter manifest not found: nonexistent');
    });
