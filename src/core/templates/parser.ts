...
import { getDataPath, writeJsonFileAtomic } from '../platform.js';
 ...
 */
export function parseIssueTemplates(projectRoot: string): TemplateResult<TemplateConfig> {
...
export function getTemplateForSubcommand(
  ...
  if (!result.success || !result.data) {
    return result.error
      ...
      : {
          success: false,
          error: { code: 'E_PARSE_ERROR', message: 'Failed to parse issue templates' },
        };
  }
...
export async function generateTemplateConfig(
  projectRoot: string,
): Promise<TemplateResult<TemplateConfig>> {
  const result = parseIssueTemplates(projectRoot);
...
export function validateLabels(
  labels: string[],
  repoLabels: string[],
): TemplateResult<{ existing: string[]; missing: string[] }> {
