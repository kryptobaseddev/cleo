 ...
 * - Query (8): stages, status, validate, report, export, history, gates, prerequisites
 * - Mutate (8): record, enforce, skip, unskip, import, reset, gate.pass, gate.fail
 *
...
export class LifecycleHandler implements DomainHandler {
  ...
  private wrapNativeResult(
    ...
    if (result.success) {
      return {
        _meta: createGatewayMeta(gateway, 'lifecycle', operation, startTime),
        success: true,
        data: result.data,
      };
    }
