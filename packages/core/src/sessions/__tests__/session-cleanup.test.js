...
describe('cleanupSessions', () => {
    beforeEach(() => {
        ...
        getRawConfigValue.mockResolvedValue(undefined);
    });
    ...
    describe('configurable timeout', () => {
        it('uses retention.autoEndActiveAfterDays from config', async () => {
            // Config says 3 days
            getRawConfigValue.mockResolvedValue(3);
        ...
        it('does NOT auto-end session within custom threshold', async () => {
            // Config says 10 days
            getRawConfigValue.mockResolvedValue(10);
            const session = makeSession({
                id: 'session-8days',
        ...
        it('falls back to default 7 days when config value is invalid', async () => {
            getRawConfigValue.mockResolvedValue('not-a-number');
            const session = makeSession({
                id: 'session-8days',
                startedAt: daysAgo(8),
            });
