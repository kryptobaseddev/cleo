...
vi.mock("../../src/core/mcp/cleo.js", async (importOriginal) => {
  const original = await importOriginal<typeof import("../../src/core/mcp/cleo.js")>();
  return {
    ...original,
...
describe("commands/mcp/cleo", () => {
  beforeEach(() => {
    ...
    mocks.getProvider.mockImplementation((id: string) =>
      id === "claude-code" ? provider : id === "cursor" ? provider2 : undefined,
    );
  ...
  describe("executeCleoInstall", () => {
    it("installs stable channel in json mode", async () => {
      ...
      await executeCleoInstall(
        "install",
        { channel: "stable", provider: ["claude-code"], arg: [], env: [], json: true },
        "mcp.cleo.install",
      );
    ...
    it("installs in human mode with successful validation", async () => {
      ...
      await executeCleoInstall(
        "install",
        { channel: "stable", provider: ["claude-code"], arg: [], env: [], human: true },
        "mcp.cleo.install",
      );
    ...
    it("dry run in human mode shows env when present", async () => {
      ...
      const envLog = logSpy.mock.calls.find((call) =>
    ...
    it("exits when no providers found in json mode", async () => {
      ...
      await expect(
        executeCleoInstall(
          "install",
          { channel: "stable", provider: [], arg: [], env: [], json: true },
          "mcp.cleo.install",
        ),
    ...
    it("exits when no providers found in human mode", async () => {
      ...
      await expect(
        executeCleoInstall(
          "install",
          { channel: "stable", provider: [], arg: [], env: [], human: true },
          "mcp.cleo.install",
        ),
      ).rejects.toThrow("process-exit");
    ...
    it("exits on format conflict error", async () => {
      ...
      await expect(
        executeCleoInstall(
          "install",
          { channel: "stable", provider: ["claude-code"], arg: [], env: [], json: true, human: true },
          "mcp.cleo.install",
        ),
      ).rejects.toThrow("process-exit");

      expect(mocks.emitJsonError).toHaveBeenCalledWith(
        "mcp.cleo.install",
        "standard",
        "E_FORMAT_CONFLICT",
        "Cannot specify both --json and --human",
        "VALIDATION",
      );
    });
    ...
    it("handles format conflict with non-Error object", async () => {
      ...
      await expect(
        executeCleoInstall(
          "install",
          { channel: "stable", provider: ["claude-code"], arg: [], env: [] },
          "mcp.cleo.install",
        ),
      ).rejects.toThrow("process-exit");
    ...
    it("uses --all flag to collect all providers", async () => {
      ...
      await executeCleoInstall(
        "install",
        { channel: "stable", provider: [], all: true, arg: [], env: [], json: true },
        "mcp.cleo.install",
      );
    ...
    it("uses global scope when --global is set", async () => {
      ...
      await executeCleoInstall(
        "install",
        { channel: "stable", provider: ["claude-code"], global: true, arg: [], env: [], json: true },
        "mcp.cleo.install",
      );
    ...
    it("detects server name conflicts in human mode", async () => {
      ...
      const warningLog = logSpy.mock.calls.find((call) =>
        String(call[0]).includes("conflict"),
      );
    ...
    it("validates profile - server missing after write", async () => {
      ...
      const result = call[2];
      const validations = result.providers?.map((p: { validation: unknown }) => p.validation);
  ...
  describe("executeCleoUninstall", () => {
    it("uninstalls in json mode", async () => {
      await executeCleoUninstall(
        { channel: "stable", provider: ["claude-code"], json: true },
        "mcp.cleo.uninstall",
      );
    ...
    it("uninstalls in human mode with successful removal", async () => {
      ...
      await executeCleoUninstall(
        { channel: "stable", provider: ["claude-code"], human: true },
        "mcp.cleo.uninstall",
      );
    ...
    it("uninstalls in human mode with no matching profile", async () => {
      ...
      await executeCleoUninstall(
        { channel: "dev", provider: ["claude-code"], human: true },
        "mcp.cleo.uninstall",
      );
    ...
    it("handles dry run in json mode", async () => {
      await executeCleoUninstall(
        { channel: "stable", provider: ["claude-code"], json: true, dryRun: true },
        "mcp.cleo.uninstall",
      );
    ...
    it("handles dry run in human mode", async () => {
      ...
      await executeCleoUninstall(
        { channel: "beta", provider: ["claude-code"], human: true, dryRun: true },
        "mcp.cleo.uninstall",
      );
    ...
    it("uses global scope", async () => {
      await executeCleoUninstall(
        { channel: "stable", provider: ["claude-code"], global: true, json: true },
        "mcp.cleo.uninstall",
      );
    ...
    it("exits when no providers found in json mode", async () => {
      ...
      await expect(
        executeCleoUninstall(
          { channel: "stable", provider: [], json: true },
          "mcp.cleo.uninstall",
        ),
      ).rejects.toThrow("process-exit");
    ...
    it("exits when no providers found in human mode", async () => {
      ...
      await expect(
        executeCleoUninstall(
          { channel: "stable", provider: [], human: true },
          "mcp.cleo.uninstall",
        ),
      ).rejects.toThrow("process-exit");
    ...
    it("exits on format conflict error", async () => {
      ...
      await expect(
        executeCleoUninstall(
          { channel: "stable", provider: ["claude-code"] },
          "mcp.cleo.uninstall",
        ),
      ).rejects.toThrow("process-exit");
  ...
  describe("executeCleoShow", () => {
    ...
    it("--global scans only global scope", async () => {
      ...
      await executeCleoShow(
        { provider: ["claude-code"], global: true, json: true },
        "mcp.cleo.show",
      );
  ...
  describe("interactive install", () => {
    ...
    it("throws when no providers selected in interactive mode", async () => {
      ...
      ).rejects.toThrow("No providers selected");
    });
    ...
    it("throws when no installed providers detected", async () => {
      ...
      ).rejects.toThrow("No installed providers");
    });
  ...
  describe("registerMcpCleoCompatibilityCommands", () => {
    it("rejects update for non-cleo name", async () => {
      ...
      await expect(
        program.parseAsync(["node", "test", "update", "not-cleo", "--channel", "stable", "--json"]),
      ).rejects.toThrow("process-exit");
      ...
      expect(mocks.emitJsonError).toHaveBeenCalledWith(
        ...
        "standard",
        "E_INVALID_INPUT",
        expect.stringContaining("Only managed profile 'cleo'"),
        "VALIDATION",
        { name: "not-cleo" },
      );
    });
    ...
    it("rejects uninstall for non-cleo name", async () => {
      ...
      await expect(
        program.parseAsync(["node", "test", "uninstall", "not-cleo", "--channel", "stable", "--json"]),
      ).rejects.toThrow("process-exit");
    ...
    it("rejects show for non-cleo name", async () => {
      ...
      await expect(
        program.parseAsync(["node", "test", "show", "not-cleo", "--json"]),
      ).rejects.toThrow("process-exit");
