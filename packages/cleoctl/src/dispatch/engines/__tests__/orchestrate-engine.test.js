...
import { orchestrateAnalyze, orchestrateContext, orchestrateHandoff, orchestrateNext, orchestrateReady, orchestrateSpawn, orchestrateStartup, orchestrateStatus, orchestrateValidate, orchestrateWaves, } from '../orchestrate-engine.js';
 ...
 */
async function seedTasks(testRoot, tasks) {
    ...
    for (const task of tasks) {
        await createTask(task, testRoot);
    }
}
const SAMPLE_TASKS = [
    {
...
describe('Orchestrate Engine', () => {
    ...
    describe('orchestrateReady', () => {
        it('should find tasks with met dependencies', async () => {
            ...
            expect(data.readyTasks.map((t) => t.id)).toContain('T102');
            expect(data.readyTasks.map((t) => t.id)).toContain('T103');
        });
    });
    ...
    describe('orchestrateSpawn', () => {
        it('should generate spawn context for ready task', async () => {
            ...
            expect(result.success).toBe(true);
            expect(result.data.taskId).toBe('T102');
            expect(result.data.tokenResolution.fullyResolved).toBe(true);
        });
...
//# sourceMappingURL=orchestrate-engine.test.js.map