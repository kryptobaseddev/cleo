...
describe("integration: advanced command wrappers", () => {
    ...
    it("fails providers wrapper on invalid tier and exits", async () => {
        ...
        const program = createProgram();
        await expect(program.parseAsync(["node", "test", "advanced", "providers", "--agent", "alpha", "--min-tier", "urgent"])).rejects.toThrow("process-exit:1");
        expect(exitSpy).toHaveBeenCalledWith(1);
        const envelope = parseEnvelope(errorSpy);
        expect(envelope["success"]).toBe(false);
        expect(envelope["error"].message).toContain("Invalid tier");
    });
    it("runs conflicts wrapper and emits detail payload", async () => {
        ...
        expect(mocks.detectMcpConfigConflicts).toHaveBeenCalledWith([alpha], [{ serverName: "filesystem", config: { command: "npx" } }], undefined);
        const envelope = parseEnvelope(logSpy);
        expect(envelope["success"]).toBe(true);
    });
    it("fails conflicts wrapper when no providers are selected", async () => {
        ...
        const program = createProgram();
        await expect(program.parseAsync(["node", "test", "advanced", "conflicts", "--agent", "alpha", "--mcp-file", "/tmp/mcp.json"])).rejects.toThrow("process-exit:1");
        expect(exitSpy).toHaveBeenCalledWith(1);
        const envelope = parseEnvelope(errorSpy);
    ...
    it("runs apply wrapper with policy", async () => {
        ...
        expect(mocks.applyMcpInstallWithPolicy).toHaveBeenCalledWith([alpha], [{ serverName: "filesystem", config: { command: "npx" } }], "skip", undefined);
        const envelope = parseEnvelope(logSpy);
        expect(envelope["success"]).toBe(true);
    });
    it("fails apply wrapper on invalid policy", async () => {
        ...
        const envelope = parseEnvelope(errorSpy);
        expect(envelope["error"].message).toContain("Invalid policy");
    });
    ...
    it("fails batch wrapper when no operation inputs are provided", async () => {
        ...
        const program = createProgram();
        await expect(program.parseAsync(["node", "test", "advanced", "batch", "--agent", "alpha"])).rejects.toThrow("process-exit:1");
        expect(exitSpy).toHaveBeenCalledWith(1);
        const envelope = parseEnvelope(errorSpy);
    ...
    it("runs instructions wrapper with file content", async () => {
        ...
        expect(mocks.updateInstructionsSingleOperation).toHaveBeenCalledWith([alpha], "instruction content from file", "global", undefined);
        const envelope = parseEnvelope(logSpy);
        expect(envelope["success"]).toBe(true);
    });
    ...
    it("fails configure wrapper for unknown provider", async () => {
        ...
        const envelope = parseEnvelope(errorSpy);
        expect(envelope["error"].message).toContain("Unknown provider: ghost");
    });
    ...
    it("apply with fail policy and conflicts", async () => {
        ...
        const envelope = parseEnvelope(errorSpy);
        expect(envelope["success"]).toBe(false);
        expect(envelope["error"].message).toContain("Conflicts detected and policy is set to fail");
    });
    it("apply with failed writes", async () => {
        ...
        const envelope = parseEnvelope(errorSpy);
        expect(envelope["success"]).toBe(false);
        expect(envelope["error"].message).toContain("One or more MCP writes failed");
    });
    it("apply with no providers exits", async () => {
        ...
        const envelope = parseEnvelope(errorSpy);
        expect(envelope["success"]).toBe(false);
        expect(envelope["error"].message).toContain("No target providers resolved");
    });
    ...
    it("instructions with inline --content", async () => {
        ...
        expect(mocks.updateInstructionsSingleOperation).toHaveBeenCalledWith([alpha], "direct text", "global", undefined);
        const envelope = parseEnvelope(logSpy);
        expect(envelope["success"]).toBe(true);
    });
    ...
    it("instructions with --scope project --project-dir", async () => {
        ...
        expect(mocks.updateInstructionsSingleOperation).toHaveBeenCalledWith([alpha], "some instructions", "project", "/my/project");
        const envelope = parseEnvelope(logSpy);
        expect(envelope["success"]).toBe(true);
    });
    ...
    it("batch reports rollback when it occurs", async () => {
        ...
        const envelope = parseEnvelope(errorSpy);
        expect(envelope["success"]).toBe(false);
        expect(envelope["error"].message).toBe("MCP write failed mid-batch");
    });
...
//# sourceMappingURL=advanced-commands.test.js.map