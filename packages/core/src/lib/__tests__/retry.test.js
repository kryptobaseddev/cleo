...
describe('withRetry — failure', () => {
    it('throws after exhausting all attempts', async () => {
        ...
        const settled = promise.then((v) => ({ ok: true, value: v }), (e) => ({ ok: false, error: e }));
        await vi.runAllTimersAsync();
    ...
    it('makes exactly 1 attempt when maxAttempts is 1', async () => {
        ...
        }, { maxAttempts: 1 })).rejects.toThrow('nope');
        expect(calls).toBe(1);
    });
});
// ============================================================================
...
describe('withRetry — retryableErrors', () => {
    ...
    it('retries when error matches a predicate function', async () => {
        vi.useFakeTimers();
        let calls = 0;
        const isRateLimit = (err) => err instanceof Error && err.message.includes('rate limit');
        const promise = withRetry(async () => {
            calls++;
            if (calls < 2)
    ...
    it('treats all errors as retryable when retryableErrors is omitted', async () => {
        ...
        const promise = withRetry(async () => {
            calls++;
            if (calls < 2)
                throw new Error('whatever obscure error');
            return 'ok';
        }, { maxAttempts: 3, baseDelayMs: 10, maxDelayMs: 100 });
        await vi.runAllTimersAsync();
        const result = await promise;
