...
import { PIPELINE_STAGES as BARREL_PIPELINE_STAGES, CANONICAL_PREREQUISITES, CANONICAL_STAGE_DEFINITIONS, } from '../lifecycle/index.js';
// RCASD-INDEX population
import { buildIndex, findByStage, getIndexTotals, getTaskAnchor, readIndex, rebuildIndex, writeIndex, } from '../lifecycle/rcasd-index.js';
// Canonical imports from stages.ts
import { CONTRIBUTION_STAGE, checkTransition, getNextStage, getPrerequisites, getPreviousStage, PIPELINE_STAGES, validateStage, } from '../lifecycle/stages.js';
describe('RCASD-IVTR+C Pipeline E2E', () => {
    ...
    describe('RCASD-INDEX', () => {
        ...
        it('builds an index from on-disk manifests', async () => {
            ...
            // Create a spec file
            await writeFile(join(taskDir, 'TEST-SPEC.md'), '# Test Spec\n\nThis is a test specification.');
            const index = buildIndex(testDir);
        ...
        it('queries task anchors', async () => {
            ...
            await mkdir(taskDir, { recursive: true });
            await writeFile(join(taskDir, '_manifest.json'), JSON.stringify({
                epicId: 'T5678',
                stages: {
        ...
        it('finds tasks by stage', async () => {
            ...
            await writeFile(join(taskDir1, '_manifest.json'), JSON.stringify({
                epicId: 'T1001',
                stages: { research: { status: 'completed', completedAt: '2026-01-01T00:00:00Z' } },
            }));
            await writeFile(join(taskDir2, '_manifest.json'), JSON.stringify({
                epicId: 'T1002',
                stages: {
                    research: { status: 'completed', completedAt: '2026-01-01T00:00:00Z' },
                    consensus: { status: 'completed', completedAt: '2026-01-02T00:00:00Z' },
...
//# sourceMappingURL=rcsd-pipeline-e2e.test.js.map