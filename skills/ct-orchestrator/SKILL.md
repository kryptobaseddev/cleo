...
The orchestrator has explicit tool usage constraints to protect context and enforce delegation patterns.

| ID | Tool | Rule | Rationale |
|----|------|------|-----------|
| TOOL-001 | TaskOutput | **MUST NEVER** use | Violates manifest-mediated handoff; pulls full content into orchestrator context |
| TOOL-002 | Task | **MUST** use for all delegation | Single spawn mechanism for subagents |
| TOOL-003 | Read/Write/Edit (code) | **MUST NOT** use for implementation | Code changes MUST be delegated to subagents |
| TOOL-004 | Bash (implementation) | **MUST NOT** use for coding | Implementation commands MUST be delegated |

### Why TaskOutput is Prohibited

TaskOutput retrieves full subagent output into orchestrator context. This:

1. **Violates ORC-003**: Pulls full file content, not manifest summaries
2. **Defeats context protection**: Content flows THROUGH orchestrator instead of files
3. **Creates bottleneck**: Orchestrator becomes content processor instead of coordinator
4. **Breaks handoff chain**: Prevents proper Agent A → manifest → Agent B pattern

### Correct Pattern

```
Orchestrator spawns A → A completes, writes manifest →
Orchestrator reads MANIFEST.jsonl (key_findings only) →
Orchestrator spawns B with: "A found [key_findings]. Read A's file if needed."
```
