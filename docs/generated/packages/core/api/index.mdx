...
| [`_resetPlatformPathsCache()`](/packages/core/api/functions#resetplatformpathscache) | function | Invalidate the path and system info caches. Use in tests after mutating CLEO_... |
...
| [`getCleoHome()`](/packages/core/api/functions#getcleohome) | function | Get the global CLEO home directory. Respects CLEO_HOME env var; otherwise use... |
...
| [`getCleoDir()`](/packages/core/api/functions#getcleodir) | function | Get the project CLEO data directory (relative). Respects CLEO_DIR env var, de... |
...
| [`getProjectRoot()`](/packages/core/api/functions#getprojectroot) | function | Get the project root from the CLEO directory. Respects CLEO_ROOT env var, the... |
...
| [`getAgentOutputsAbsolute()`](/packages/core/api/functions#getagentoutputsabsolute) | function | Get the absolute path to the agent outputs directory.  T4700 |
| [`getManifestPath()`](/packages/core/api/functions#getmanifestpath) | function | Get the absolute path to the MANIFEST.jsonl file.  Checks config.agentOutputs... |
| [`getManifestArchivePath()`](/packages/core/api/functions#getmanifestarchivepath) | function | Get the absolute path to the MANIFEST.archive.jsonl file.  T4700 |
...
| [`getAgentsHome()`](/packages/core/api/functions#getagentshome) | function | Get the global agents hub directory. Respects AGENTS_HOME env var, defaults t... |
...
| [`setMetaValue()`](/packages/core/api/functions#setmetavalue) | function | Write a JSON blob to the schema_meta table by key. |
...
| [`isProviderHookEvent()`](/packages/core/api/functions#isproviderhookevent) | function | Type guard for CAAMP/provider-discoverable hook events. |
...
| [`loadStateFileAllowlist()`](/packages/core/api/functions#loadstatefileallowlist) | function | Load additional state file paths from config.json `checkpoint.stateFileAllowl... |
...
| [`toCompact()`](/packages/core/api/functions#tocompact) | function | Convert a full Task to compact representation. |
...
| [`linkPipelineAdr()`](/packages/core/api/functions#linkpipelineadr) | function | Link ADRs to a pipeline task when the architecture_decision stage completes. |
| [`syncAdrsToDb()`](/packages/core/api/functions#syncadrstodb) | function | Sync all ADR markdown files into the architecture_decisions table AND regener... |
...
| [`normalizeEpicId()`](/packages/core/api/functions#normalizeepicid) | function | Strip suffixes from epic directory names. E.g. `T4881_install-channels` - `T4... |
...
| [`getStagePath()`](/packages/core/api/functions#getstagepath) | function | Get the stage subdirectory path for an epic. Uses STAGE_SUBDIRS mapping, fall... |
...
| [`makeTaskFile()`](/packages/core/api/functions#maketaskfile) | function | Build a TaskFile structure from a list of task partials. Useful for seeding t... |
...
| [`pruneAuditLog()`](/packages/core/api/functions#pruneauditlog) | function | Prune old audit_log rows from tasks.db.  1. If auditRetentionDays is 0 or und... |
| [`queryAudit()`](/packages/core/api/functions#queryaudit) | function | Query audit entries from SQLite audit_log table. Used by session-grade.ts for... |
...
| [`stripCLEOBlocks()`](/packages/core/api/functions#stripcleoblocks) | function | Strip legacy !-- CLEO:START --...!-- CLEO:END -- blocks from a file. Called b... |
...
| [`createDefaultConfig()`](/packages/core/api/functions#createdefaultconfig) | function |  |
...
| [`ensureContributorMcp()`](/packages/core/api/functions#ensurecontributormcp) | function | Ensure .mcp.json contains a cleo-dev server entry pointing to the local build... |
...
| [`ensureGlobalScaffold()`](/packages/core/api/functions#ensureglobalscaffold) | function | Perform a complete global scaffold operation: ensure home, schemas, and templ... |
...
| [`getMcpServerName()`](/packages/core/api/functions#getmcpservername) | function | Resolve MCP server name by channel. |
| [`detectEnvMode()`](/packages/core/api/functions#detectenvmode) | function | Detect the current CLEO environment mode by reading ~/.cleo/VERSION.  The VER... |
| [`generateMcpServerEntry()`](/packages/core/api/functions#generatemcpserverentry) | function | Generate the MCP server entry for the cleo server based on env mode.  Returns... |
...
| [`toTaskFileExt()`](/packages/core/api/functions#totaskfileext) | function | Convert a TaskFile (from contracts) to the looser TaskFileExt shape. Accepts ... |
| [`recordDecision()`](/packages/core/api/functions#recorddecision) | function | Record a decision to the audit trail. Appends a JSON line to `.cleo/audit/dec... |
...
| [`initMcpServer()`](/packages/core/api/functions#initmcpserver) | function | Install MCP server config to all detected providers via CAAMP.  T4706 |
...
| [`installMcpToProviders()`](/packages/core/api/functions#installmcptoproviders) | function | Install the CLEO MCP server config to all detected providers. |
...
| [`getSharingStatus()`](/packages/core/api/functions#getsharingstatus) | function | Get the sharing status: which .cleo/ files are tracked vs ignored.  T4883 |
...
| [`countManifestEntries()`](/packages/core/api/functions#countmanifestentries) | function | Count manifest entries from MANIFEST.jsonl. |
...
| [`readSyncState()`](/packages/core/api/functions#readsyncstate) | function | Read sync session state for a provider. Returns null if no state file exists. |
| [`writeSyncState()`](/packages/core/api/functions#writesyncstate) | function | Write sync session state for a provider. |
| [`clearSyncState()`](/packages/core/api/functions#clearsyncstate) | function | Clear (delete) sync session state for a provider. |
...
| [`handleSessionStart()`](/packages/core/api/functions#handlesessionstart) | function | Handle onSessionStart - capture initial session context |
| [`handleSessionEnd()`](/packages/core/api/functions#handlesessionend) | function | Handle onSessionEnd - capture session summary |
| [`handleToolStart()`](/packages/core/api/functions#handletoolstart) | function | Handle onToolStart (maps to task.start in CLEO) |
| [`handleToolComplete()`](/packages/core/api/functions#handletoolcomplete) | function | Handle onToolComplete (maps to task.complete in CLEO) |
| [`handleError()`](/packages/core/api/functions#handleerror) | function | Handle onError - capture operation errors to BRAIN  Includes infinite-loop gu... |
| [`handleFileChange()`](/packages/core/api/functions#handlefilechange) | function | Handle onFileChange - capture file changes to BRAIN  Gated behind CLEO_BRAIN_... |
| [`handlePromptSubmit()`](/packages/core/api/functions#handlepromptsubmit) | function | Handle onPromptSubmit - optionally capture prompt events to BRAIN  No-op by d... |
| [`handleResponseComplete()`](/packages/core/api/functions#handleresponsecomplete) | function | Handle onResponseComplete - optionally capture response events to BRAIN  No-o... |
...
| [`importTasksPackage()`](/packages/core/api/functions#importtaskspackage) | function | Import tasks from a cross-project export package with ID remapping. |
...
| [`mcpList()`](/packages/core/api/functions#mcplist) | function | List MCP servers for a specific provider.  T4332 |
| [`mcpListAll()`](/packages/core/api/functions#mcplistall) | function | List MCP servers across all installed providers.  T4332 |
| [`mcpInstall()`](/packages/core/api/functions#mcpinstall) | function | Install an MCP server to a provider's config.  T4332 |
| [`mcpRemove()`](/packages/core/api/functions#mcpremove) | function | Remove an MCP server from a provider's config.  T4332 |
| [`mcpConfigPath()`](/packages/core/api/functions#mcpconfigpath) | function | Resolve the config file path for a provider.  T4332 |
...
| [`batchInstallWithRollback()`](/packages/core/api/functions#batchinstallwithrollback) | function | Install multiple MCP servers atomically with rollback on failure. Supports Wa... |
| [`dualScopeConfigure()`](/packages/core/api/functions#dualscopeconfigure) | function | Configure a provider at both global and project scope simultaneously. Used du... |
...
| [`injectTasks()`](/packages/core/api/functions#injecttasks) | function | Inject tasks into TodoWrite format. |
...
| [`validateLabelsExist()`](/packages/core/api/functions#validatelabelsexist) | function | Validate that required labels exist (informational). |
...
| [`applyTemporalDecay()`](/packages/core/api/functions#applytemporaldecay) | function | Apply temporal decay to brain_learnings confidence values.  Entries older tha... |
...
| [`addResearch()`](/packages/core/api/functions#addresearch) | function | Add a research entry.  T4465 |
| [`showResearch()`](/packages/core/api/functions#showresearch) | function | Show a specific research entry.  T4465 |
| [`listResearch()`](/packages/core/api/functions#listresearch) | function | List research entries with optional filtering.  T4465 |
| [`pendingResearch()`](/packages/core/api/functions#pendingresearch) | function | List pending research entries.  T4465 |
| [`linkResearch()`](/packages/core/api/functions#linkresearch) | function | Link a research entry to a task.  T4465 |
| [`updateResearch()`](/packages/core/api/functions#updateresearch) | function | Update research findings.  T4465 |
| [`statsResearch()`](/packages/core/api/functions#statsresearch) | function | Get research statistics.  T4474 |
| [`linksResearch()`](/packages/core/api/functions#linksresearch) | function | Get research entries linked to a specific task.  T4474 |
| [`archiveResearch()`](/packages/core/api/functions#archiveresearch) | function | Archive old research entries by status. Moves 'complete' entries older than a... |
| [`readManifest()`](/packages/core/api/functions#readmanifest) | function | Read manifest entries from MANIFEST.jsonl.  T4465 |
| [`appendManifest()`](/packages/core/api/functions#appendmanifest) | function | Append a manifest entry.  T4465 |
| [`queryManifest()`](/packages/core/api/functions#querymanifest) | function | Query manifest entries.  T4465 |
| [`readExtendedManifest()`](/packages/core/api/functions#readextendedmanifest) | function | Read all manifest entries as extended entries.  T4787 |
| [`filterManifestEntries()`](/packages/core/api/functions#filtermanifestentries) | function | Filter manifest entries by criteria.  T4787 |
| [`showManifestEntry()`](/packages/core/api/functions#showmanifestentry) | function | Show a manifest entry by ID with optional file content.  T4787 |
| [`searchManifest()`](/packages/core/api/functions#searchmanifest) | function | Search manifest entries by text with relevance scoring.  T4787 |
| [`pendingManifestEntries()`](/packages/core/api/functions#pendingmanifestentries) | function | Get pending manifest entries (partial, blocked, or needing followup).  T4787 |
| [`manifestStats()`](/packages/core/api/functions#manifeststats) | function | Get manifest-based research statistics.  T4787 |
| [`linkManifestEntry()`](/packages/core/api/functions#linkmanifestentry) | function | Link a manifest entry to a task (adds taskId to linked_tasks array).  T4787 |
...
| [`archiveManifestEntries()`](/packages/core/api/functions#archivemanifestentries) | function | Archive manifest entries older than a date.  T4787 |
| [`findContradictions()`](/packages/core/api/functions#findcontradictions) | function | Find manifest entries with overlapping topics but conflicting key_findings.  ... |
| [`findSuperseded()`](/packages/core/api/functions#findsuperseded) | function | Identify research entries replaced by newer work on same topic.  T4787 |
| [`readProtocolInjection()`](/packages/core/api/functions#readprotocolinjection) | function | Read protocol injection content for a given protocol type.  T4787 |
| [`compactManifest()`](/packages/core/api/functions#compactmanifest) | function | Compact MANIFEST.jsonl by removing duplicate/stale entries.  T4787 |
| [`validateManifestEntries()`](/packages/core/api/functions#validatemanifestentries) | function | Validate research entries for a task.  T4787 |
...
| [`ensureArray()`](/packages/core/api/functions#ensurearray) | function | Normalize a value to an array of strings. Handles MCP clients sending comma-s... |
| [`sanitizeParams()`](/packages/core/api/functions#sanitizeparams) | function | Sanitize all params in a request before routing |
| [`ClaudeCodeTransport`](/packages/core/api/functions#claudecodetransport) | class | Claude Code transport — wraps the current provider-specific messaging.  Regis... |
| [`SignalDockTransport`](/packages/core/api/functions#signaldocktransport) | class | SignalDock HTTP transport implementation.  Communicates with a SignalDock ser... |
| [`createTransport()`](/packages/core/api/functions#createtransport) | function | Create an AgentTransport instance based on configuration.  Returns SignalDock... |
...
| [`injectTokens()`](/packages/core/api/functions#injecttokens) | function | Inject token values into a template string. Replaces all TOKEN_NAME patterns ... |
...
| [`spawnBatch()`](/packages/core/api/functions#spawnbatch) | function | Spawn prompts for multiple tasks in a batch. Ports orchestrator_spawn_batch f... |
...
| [`getSkillSearchPaths()`](/packages/core/api/functions#getskillsearchpaths) | function | Get ordered skill search paths based on configuration.  Priority: 1. CLEO_SKI... |
...
| [`resolveProtocolPath()`](/packages/core/api/functions#resolveprotocolpath) | function | Resolve a protocol .md file.  Search order per base path: 1. base/_ct-skills-... |
| [`resolveSharedPath()`](/packages/core/api/functions#resolvesharedpath) | function | Resolve a shared resource .md file.  Search order per base path: 1. base/_ct-... |
...
| [`resolveProjectRoot()`](/packages/core/api/functions#resolveprojectroot) | function | Resolve the project root directory. Checks CLEO_ROOT env, then falls back to ... |
...
| [`checkCoreFilesNotIgnored()`](/packages/core/api/functions#checkcorefilesnotignored) | function | Check that core CLEO files are not being ignored by .gitignore. Uses `git che... |
...
| [`startupHealthCheck()`](/packages/core/api/functions#startuphealthcheck) | function | Unified startup health check for MCP server and CLI entry points.  This is th... |
...
| [`analyzeChanges()`](/packages/core/api/functions#analyzechanges) | function | Analyze TodoWrite state and detect changes against injected task IDs. |
| [`mergeTodoWriteState()`](/packages/core/api/functions#mergetodowritestate) | function | Merge TodoWrite state back to CLEO tasks. |
...
| [`parseIssueTemplates()`](/packages/core/api/functions#parseissuetemplates) | function | Parse all templates from the repo's .github/ISSUE_TEMPLATE/ directory.  Reads... |
...
| [`getValidationKey()`](/packages/core/api/functions#getvalidationkey) | function | Get the validation key for a target filename. |
| [`extractMarkerVersion()`](/packages/core/api/functions#extractmarkerversion) | function | Extract the CLEO version from an injection marker string. Returns null if no ... |
...
| [`parseManifest()`](/packages/core/api/functions#parsemanifest) | function | Parse a MANIFEST.jsonl file into entries. Skips invalid JSON lines gracefully... |
...
| [`checkKeyFindingsCount()`](/packages/core/api/functions#checkkeyfindingscount) | function | Check if key_findings array has valid count (3-7).  T4527 |
| [`checkStatusValid()`](/packages/core/api/functions#checkstatusvalid) | function | Check if status is valid enum value.  T4527 |
| [`checkAgentType()`](/packages/core/api/functions#checkagenttype) | function | Check if agent_type matches expected value.  T4527 |
| [`checkLinkedTasksPresent()`](/packages/core/api/functions#checklinkedtaskspresent) | function | Check if linked_tasks array contains required task IDs.  T4527 |
| [`checkProvenanceTags()`](/packages/core/api/functions#checkprovenancetags) | function | Check if file contains  provenance tag.  T4527 |
| [`validateCommonManifestRequirements()`](/packages/core/api/functions#validatecommonmanifestrequirements) | function | Validate common manifest requirements across all protocols.  T4527 |
...
| [`getSyncStatus()`](/packages/core/api/functions#getsyncstatus) | function | Get current sync status.  T5326 |
| [`clearSyncState()`](/packages/core/api/functions#clearsyncstate) | function | Clear sync state.  T5326 |
...
| [`buildDefaultChain()`](/packages/core/api/functions#builddefaultchain) | function | Build the canonical 9-stage RCASD-IVTR+C WarpChain.  - Each PIPELINE_STAGE be... |
...
| [`reasonWhy()`](/packages/core/api/functions#reasonwhy) | function | Build a causal trace for why a task is blocked.  Walks upstream through `depe... |
...
| [`memoryShow()`](/packages/core/api/functions#memoryshow) | function | memory.show - Look up a brain.db entry by ID |
| [`memoryBrainStats()`](/packages/core/api/functions#memorybrainstats) | function | memory.stats - Aggregate stats from brain.db across all tables |
| [`memoryDecisionFind()`](/packages/core/api/functions#memorydecisionfind) | function | memory.decision.find - Search decisions in brain.db |
| [`memoryDecisionStore()`](/packages/core/api/functions#memorydecisionstore) | function | memory.decision.store - Store a decision to brain.db |
| [`memoryFind()`](/packages/core/api/functions#memoryfind) | function | memory.find - Token-efficient brain search |
| [`memoryTimeline()`](/packages/core/api/functions#memorytimeline) | function | memory.timeline - Chronological context around anchor |
| [`memoryFetch()`](/packages/core/api/functions#memoryfetch) | function | memory.fetch - Batch fetch brain entries by IDs |
| [`memoryObserve()`](/packages/core/api/functions#memoryobserve) | function | memory.observe - Save observation to brain |
| [`memoryPatternStore()`](/packages/core/api/functions#memorypatternstore) | function | memory.pattern.store - Store a pattern to BRAIN memory |
| [`memoryPatternFind()`](/packages/core/api/functions#memorypatternfind) | function | memory.pattern.find - Search patterns in BRAIN memory |
| [`memoryPatternStats()`](/packages/core/api/functions#memorypatternstats) | function | memory.pattern.stats - Get pattern memory statistics |
| [`memoryLearningStore()`](/packages/core/api/functions#memorylearningstore) | function | memory.learning.store - Store a learning to BRAIN memory |
| [`memoryLearningFind()`](/packages/core/api/functions#memorylearningfind) | function | memory.learning.find - Search learnings in BRAIN memory |
| [`memoryLearningStats()`](/packages/core/api/functions#memorylearningstats) | function | memory.learning.stats - Get learning memory statistics |
| [`memoryContradictions()`](/packages/core/api/functions#memorycontradictions) | function | memory.contradictions - Find contradictory entries in brain.db |
| [`memorySuperseded()`](/packages/core/api/functions#memorysuperseded) | function | memory.superseded - Find superseded entries in brain.db  Identifies entries t... |
| [`memoryLink()`](/packages/core/api/functions#memorylink) | function | memory.link - Link a brain entry to a task |
| [`memoryUnlink()`](/packages/core/api/functions#memoryunlink) | function | memory.unlink - Remove a link between a brain entry and a task |
| [`memoryGraphAdd()`](/packages/core/api/functions#memorygraphadd) | function | memory.graph.add - Add a node or edge to the PageIndex graph |
| [`memoryGraphShow()`](/packages/core/api/functions#memorygraphshow) | function | memory.graph.show - Get a node and its edges from the PageIndex graph |
| [`memoryGraphNeighbors()`](/packages/core/api/functions#memorygraphneighbors) | function | memory.graph.neighbors - Get neighbor nodes from the PageIndex graph |
| [`memoryReasonWhy()`](/packages/core/api/functions#memoryreasonwhy) | function | memory.reason.why - Causal trace through task dependency chains |
| [`memoryReasonSimilar()`](/packages/core/api/functions#memoryreasonsimilar) | function | memory.reason.similar - Find semantically similar entries |
| [`memorySearchHybrid()`](/packages/core/api/functions#memorysearchhybrid) | function | memory.search.hybrid - Hybrid search across FTS5, vector, and graph |
| [`memoryGraphRemove()`](/packages/core/api/functions#memorygraphremove) | function | memory.graph.remove - Remove a node or edge from the PageIndex graph |
...
| [`pipelineManifestAppend()`](/packages/core/api/functions#pipelinemanifestappend) | function | pipeline.manifest.append - Append entry to pipeline_manifest table |
...
| [`readManifestEntries()`](/packages/core/api/functions#readmanifestentries) | function | Read all manifest entries from the pipeline_manifest table. Replaces readMani... |
...
| [`migrateManifestJsonlToSqlite()`](/packages/core/api/functions#migratemanifestjsonltosqlite) | function | Migrate existing .cleo/MANIFEST.jsonl entries into the pipeline_manifest tabl... |
...
| [`listSkills()`](/packages/core/api/functions#listskills) | function | List available skills. |
| [`getSkillContent()`](/packages/core/api/functions#getskillcontent) | function | Read skill content for injection into agent context. |
...
| [`generateSessionId()`](/packages/core/api/functions#generatesessionid) | function | Generate a canonical session ID.  Format: ses_YYYYMMDDHHmmss_6hex Example: se... |
...
| [`coreTaskNext()`](/packages/core/api/functions#coretasknext) | function | Suggest next task to work on based on priority, phase, age, and deps.  T4790 |
| [`coreTaskBlockers()`](/packages/core/api/functions#coretaskblockers) | function | Show blocked tasks and analyze blocking chains.  T4790 |
| [`coreTaskTree()`](/packages/core/api/functions#coretasktree) | function | Build hierarchy tree.  T4790 |
| [`coreTaskDeps()`](/packages/core/api/functions#coretaskdeps) | function | Show dependencies for a task.  T4790 |
| [`coreTaskRelates()`](/packages/core/api/functions#coretaskrelates) | function | Show task relations.  T4790 |
| [`coreTaskRelatesAdd()`](/packages/core/api/functions#coretaskrelatesadd) | function | Add a relation between two tasks.  T4790 |
| [`coreTaskAnalyze()`](/packages/core/api/functions#coretaskanalyze) | function | Analyze tasks for priority and leverage.  T4790 |
| [`coreTaskRestore()`](/packages/core/api/functions#coretaskrestore) | function | Restore a cancelled task back to pending.  T4790 |
...
| [`coreTaskUnarchive()`](/packages/core/api/functions#coretaskunarchive) | function | Move an archived task back to active tasks.  T4790 |
| [`coreTaskReorder()`](/packages/core/api/functions#coretaskreorder) | function | Change task position within its sibling group.  T4790 |
| [`coreTaskReparent()`](/packages/core/api/functions#coretaskreparent) | function | Move task under a different parent.  T4790 |
| [`coreTaskPromote()`](/packages/core/api/functions#coretaskpromote) | function | Promote a subtask to task or task to root.  T4790 |
| [`coreTaskReopen()`](/packages/core/api/functions#coretaskreopen) | function | Reopen a completed task.  T4790 |
| [`coreTaskComplexityEstimate()`](/packages/core/api/functions#coretaskcomplexityestimate) | function | Deterministic complexity scoring from task metadata.  T4790 |
| [`coreTaskDepsOverview()`](/packages/core/api/functions#coretaskdepsoverview) | function | Overview of all dependencies across the project.  T5157 |
| [`coreTaskDepsCycles()`](/packages/core/api/functions#coretaskdepscycles) | function | Detect circular dependencies across the project.  T5157 |
| [`coreTaskDepends()`](/packages/core/api/functions#coretaskdepends) | function | List dependencies for a task in a given direction.  T4790 |
| [`coreTaskStats()`](/packages/core/api/functions#coretaskstats) | function | Compute task statistics.  T4790 |
| [`coreTaskExport()`](/packages/core/api/functions#coretaskexport) | function | Export tasks as JSON or CSV.  T4790 |
| [`coreTaskHistory()`](/packages/core/api/functions#coretaskhistory) | function | Get task history from the log file.  T4790 |
| [`coreTaskLint()`](/packages/core/api/functions#coretasklint) | function | Lint tasks for common issues.  T4790 |
| [`coreTaskBatchValidate()`](/packages/core/api/functions#coretaskbatchvalidate) | function | Validate multiple tasks at once.  T4790 |
| [`coreTaskImport()`](/packages/core/api/functions#coretaskimport) | function | Import tasks from a JSON source string.  T4790 |
...
| [`checkConsensusManifest()`](/packages/core/api/functions#checkconsensusmanifest) | function | Validate consensus protocol from manifest file. |
...
| [`checkContributionManifest()`](/packages/core/api/functions#checkcontributionmanifest) | function | Validate contribution protocol from manifest file. |
...
| [`checkDecompositionManifest()`](/packages/core/api/functions#checkdecompositionmanifest) | function | Validate decomposition protocol from manifest file. |
...
| [`checkImplementationManifest()`](/packages/core/api/functions#checkimplementationmanifest) | function | Validate implementation protocol from manifest file. |
...
| [`checkSpecificationManifest()`](/packages/core/api/functions#checkspecificationmanifest) | function | Validate specification protocol from manifest file. |
...
| [`resolveEpicFromContent()`](/packages/core/api/functions#resolveepicfromcontent) | function | Extract an epic/task ID from file content by searching for:   1. `@task T####... |
...
| [`migrateLooseFiles()`](/packages/core/api/functions#migrateloosefiles) | function | Migrate loose `T####_*.md` files from `.cleo/rcasd/` root into `rcasd/{epicId... |
...
| [`findResumablePipelines()`](/packages/core/api/functions#findresumablepipelines) | function | Query active pipelines that can be resumed.  Searches the lifecycle_pipelines... |
...
| [`isValidStatusTransition()`](/packages/core/api/functions#isvalidstatustransition) | function | Check if a status transition is valid.  State transitions:   not_started → in... |
...
| [`getPreferredChannel()`](/packages/core/api/functions#getpreferredchannel) | function | Look up the preferred channel for a given domain + operation. |
| [`getRoutingForDomain()`](/packages/core/api/functions#getroutingfordomain) | function | Get routing entries for a specific domain. |
| [`getOperationsByChannel()`](/packages/core/api/functions#getoperationsbychannel) | function | Get all operations that prefer a specific channel. |
...
| [`activateStage()`](/packages/core/api/functions#activatestage) | function | Update a stage's status to 'in_progress' and clear block fields. |
...
| [`reparentTask()`](/packages/core/api/functions#reparenttask) | function | Reparent a task within a TaskFile.  Mutates the task in-place within `data.ta... |
...
| [`checkReleaseManifest()`](/packages/core/api/functions#checkreleasemanifest) | function | Validate release protocol from manifest file. |
| [`validateResearchTask()`](/packages/core/api/functions#validateresearchtask) | function | Validate research protocol for a task. |
| [`checkResearchManifest()`](/packages/core/api/functions#checkresearchmanifest) | function | Validate research protocol from manifest file. |
| [`validateTestingTask()`](/packages/core/api/functions#validatetestingtask) | function | Validate testing protocol for a task. |
| [`checkTestingManifest()`](/packages/core/api/functions#checktestingmanifest) | function | Validate testing protocol from manifest file. |
| [`validateValidationTask()`](/packages/core/api/functions#validatevalidationtask) | function | Validate verification/validation protocol for a task. |
| [`checkValidationManifest()`](/packages/core/api/functions#checkvalidationmanifest) | function | Validate validation protocol from manifest file. |
...
| [`ProblemDetails`](/packages/core/api/types#problemdetails) | interface | RFC 9457 Problem Details object. Structured error representation for API/MCP ... |
...
| [`ProviderHookEvent`](/packages/core/api/types#providerhookevent) | type | CAAMP-defined hook events supported by provider capability discovery. |
| [`InternalHookEvent`](/packages/core/api/types#internalhookevent) | type |  |
| [`HookEvent`](/packages/core/api/types#hookevent) | type | Full CLEO hook event union.  CAAMP defines provider-facing events; CLEO exten... |
...
| [`OnSessionStartPayload`](/packages/core/api/types#onsessionstartpayload) | interface | Payload for onSessionStart hook Fired when a CLEO session begins |
| [`OnSessionEndPayload`](/packages/core/api/types#onsessionendpayload) | interface | Payload for onSessionEnd hook Fired when a CLEO session ends |
| [`OnToolStartPayload`](/packages/core/api/types#ontoolstartpayload) | interface | Payload for onToolStart hook Fired when a task/tool operation begins |
| [`OnToolCompletePayload`](/packages/core/api/types#ontoolcompletepayload) | interface | Payload for onToolComplete hook Fired when a task/tool operation completes |
...
| [`OnFileChangePayload`](/packages/core/api/types#onfilechangepayload) | interface | Payload for onFileChange hook Fired when a tracked file is written, created, ... |
| [`OnErrorPayload`](/packages/core/api/types#onerrorpayload) | interface | Payload for onError hook Fired when an operation fails with a structured error |
| [`OnPromptSubmitPayload`](/packages/core/api/types#onpromptsubmitpayload) | interface | Payload for onPromptSubmit hook Fired when an agent submits a prompt through ... |
| [`OnResponseCompletePayload`](/packages/core/api/types#onresponsecompletepayload) | interface | Payload for onResponseComplete hook Fired when a gateway operation finishes (... |
...
| [`CompactTask`](/packages/core/api/types#compacttask) | interface | Compact task representation — minimal fields for MCP list responses. |
...
| [`AlertCheckResult`](/packages/core/api/types#alertcheckresult) | interface | Alert result from check_context_alert. |
...
| [`BrainFtsRow`](/packages/core/api/types#brainftsrow) | interface | Row returned by FTS content_hash duplicate check. |
| [`BrainNarrativeRow`](/packages/core/api/types#brainnarrativerow) | interface | Row returned by narrative backfill query (missing embeddings). |
| [`BrainSearchHit`](/packages/core/api/types#brainsearchhit) | interface | Flattened FTS hit used in hybrid search scoring. |
| [`BrainKnnRow`](/packages/core/api/types#brainknnrow) | interface | Row returned by KNN vector similarity query. |
| [`BrainDecisionNode`](/packages/core/api/types#braindecisionnode) | interface | Decision node attached to a blocker in causal traces. |
| [`BrainAnchor`](/packages/core/api/types#brainanchor) | interface | Anchor entry in a timeline result. |
...
| [`McpEnvMode`](/packages/core/api/types#mcpenvmode) | interface | Resolved environment mode for MCP server config. |
| [`ScaffoldResult`](/packages/core/api/types#scaffoldresult) | interface |  |
...
| [`TaskWorkStateExt`](/packages/core/api/types#taskworkstateext) | interface | Task work state from the task store. |
| [`TaskFileExt`](/packages/core/api/types#taskfileext) | interface | Task file structure (subset for session operations). |
...
| [`TasksAPI`](/packages/core/api/types#tasksapi) | interface |  |
| [`SessionsAPI`](/packages/core/api/types#sessionsapi) | interface |  |
| [`MemoryAPI`](/packages/core/api/types#memoryapi) | interface |  |
| [`OrchestrationAPI`](/packages/core/api/types#orchestrationapi) | interface |  |
| [`LifecycleAPI`](/packages/core/api/types#lifecycleapi) | interface |  |
| [`ReleaseAPI`](/packages/core/api/types#releaseapi) | interface |  |
| [`AdminAPI`](/packages/core/api/types#adminapi) | interface |  |
| [`StickyAPI`](/packages/core/api/types#stickyapi) | interface |  |
| [`NexusAPI`](/packages/core/api/types#nexusapi) | interface |  |
| [`SyncAPI`](/packages/core/api/types#syncapi) | interface |  |
| [`CleoInitOptions`](/packages/core/api/types#cleoinitoptions) | interface |  |
...
| [`PreferredChannel`](/packages/core/api/types#preferredchannel) | type | Preferred communication channel for token efficiency. Added for provider-agno... |
...
| [`AgentClass`](/packages/core/api/types#agentclass) | type | Functional classification of an agent. |
| [`PrivacyTier`](/packages/core/api/types#privacytier) | type | Visibility tier controlling agent discoverability. |
| [`AgentStatus`](/packages/core/api/types#agentstatus) | type | Current online status of an agent. |
| [`MessageStatus`](/packages/core/api/types#messagestatus) | type | Delivery status of a message. |
| [`ContentType`](/packages/core/api/types#contenttype) | type | Content type for message payloads. |
| [`ConversationVisibility`](/packages/core/api/types#conversationvisibility) | type | Visibility setting for a conversation. |
| [`Agent`](/packages/core/api/types#agent) | interface | A registered agent. |
| [`NewAgent`](/packages/core/api/types#newagent) | interface | Payload for registering a new agent. |
| [`Message`](/packages/core/api/types#message) | interface | A message exchanged between two agents. |
| [`NewMessage`](/packages/core/api/types#newmessage) | interface | Payload for creating a new message. |
| [`Conversation`](/packages/core/api/types#conversation) | interface | A conversation between agents. |
| [`NewConversation`](/packages/core/api/types#newconversation) | interface | Payload for creating a new conversation. |
| [`ApiResponse`](/packages/core/api/types#apiresponse) | interface | Standard API response envelope. |
| [`AgentRegistration`](/packages/core/api/types#agentregistration) | interface | Result of agent registration. |
| [`MessageResult`](/packages/core/api/types#messageresult) | interface | Result of sending a message. |
| [`AgentTransport`](/packages/core/api/types#agenttransport) | interface | Provider-neutral interface for inter-agent communication.  Implementations: -... |
| [`SignalDockTransportConfig`](/packages/core/api/types#signaldocktransportconfig) | interface | Configuration for SignalDockTransport. |
| [`TransportFactoryConfig`](/packages/core/api/types#transportfactoryconfig) | interface | Configuration for transport selection. |
| [`SkillFrontmatter`](/packages/core/api/types#skillfrontmatter) | interface | Skill frontmatter parsed from SKILL.md YAML header. |
| [`Skill`](/packages/core/api/types#skill) | interface | Skill definition loaded from disk. |
| [`SkillSummary`](/packages/core/api/types#skillsummary) | interface | Lightweight skill summary for manifest/listing. |
...
| [`DispatchResult`](/packages/core/api/types#dispatchresult) | interface | Dispatch result from skill_auto_dispatch. |
...
| [`ManifestEntry`](/packages/core/api/types#manifestentry) | interface | Research manifest entry (MANIFEST.jsonl). |
...
| [`TokenValues`](/packages/core/api/types#tokenvalues) | type | Token values map: TOKEN_NAME - value. |
...
| [`TodoWriteItem`](/packages/core/api/types#todowriteitem) | interface |  |
| [`TodoWriteState`](/packages/core/api/types#todowritestate) | interface |  |
| [`SyncSessionState`](/packages/core/api/types#syncsessionstate) | interface |  |
| [`ChangeSet`](/packages/core/api/types#changeset) | interface |  |
| [`TodoWriteMergeOptions`](/packages/core/api/types#todowritemergeoptions) | interface |  |
| [`TodoWriteMergeResult`](/packages/core/api/types#todowritemergeresult) | interface |  |
...
| [`TaskFile`](/packages/core/api/types#taskfile) | interface |  |
| [`ArchiveFile`](/packages/core/api/types#archivefile) | interface |  |
...
| [`SyncStatusResult`](/packages/core/api/types#syncstatusresult) | interface | Result for sync status operation. |
| [`SyncClearResult`](/packages/core/api/types#syncclearresult) | interface | Result for sync clear operation. |
...
| [`ComplexityFactor`](/packages/core/api/types#complexityfactor) | interface | Complexity factor. |
...
| [`WorkflowGateName`](/packages/core/api/types#workflowgatename) | enum | Workflow gate names per MCP-SERVER-SPECIFICATION.md Section 7.1   T3141 |
...
| [`SkillsPrecedenceConfig`](/packages/core/api/types#skillsprecedenceconfig) | interface |  |
| [`ResolvedSkillPath`](/packages/core/api/types#resolvedskillpath) | interface |  |
| [`SkillInstallationContext`](/packages/core/api/types#skillinstallationcontext) | interface |  |
...
| [`RoutingEntry`](/packages/core/api/types#routingentry) | interface | Routing entry describing the preferred channel for an operation. |
...
| [`ReparentOptions`](/packages/core/api/types#reparentoptions) | interface | Options for reparenting a task. |
| [`ReparentResult`](/packages/core/api/types#reparentresult) | interface | Result of a reparent operation. |
...
| [`LIFECYCLE_STAGE_NAMES`](/packages/core/api/functions#lifecyclestagenames) | variable | Canonical lifecycle stage names matching DB CHECK constraint on lifecycle_sta... |
| [`LIFECYCLE_GATE_RESULTS`](/packages/core/api/functions#lifecyclegateresults) | variable | Gate result values matching DB CHECK constraint on lifecycle_gate_results.res... |
| [`LIFECYCLE_EVIDENCE_TYPES`](/packages/core/api/functions#lifecycleevidencetypes) | variable | Evidence type values matching DB CHECK constraint on lifecycle_evidence.type. |
...
| [`tokenUsage`](/packages/core/api/functions#tokenusage) | variable | Central provider-aware token telemetry for CLI, MCP, and external adapters. S... |
...
| [`CLEO_TO_CAAMP_HOOK_MAP`](/packages/core/api/functions#cleotocaamphookmap) | variable | Mapping from CLEO internal lifecycle events to CAAMP hook events This is wher... |
...
| [`EMBEDDING_DIMENSIONS`](/packages/core/api/functions#embeddingdimensions) | variable | Matches the brain_embeddings vec0 table: FLOAT[384]. |
...
| [`INJECTION_VALIDATION_KEYS`](/packages/core/api/functions#injectionvalidationkeys) | variable | Validation key names for JSON output. Maps target filenames to JSON-safe key ... |
...
| [`PROTOCOL_TYPES`](/packages/core/api/functions#protocoltypes) | variable | All supported protocol types. |
| [`PROTOCOL_EXIT_CODES`](/packages/core/api/functions#protocolexitcodes) | variable | Map protocol types to exit codes. |
| [`DEFAULT_CHAIN_ID`](/packages/core/api/functions#defaultchainid) | variable |  |
| [`DEFAULT_PROTOCOL_STAGE_MAP`](/packages/core/api/functions#defaultprotocolstagemap) | variable | Stage mapping for protocol validation gates in the default chain.  `contribut... |
...
| [`ROUTING_TABLE`](/packages/core/api/functions#routingtable) | variable | Static routing table for all canonical operations.  Operations are grouped by... |
