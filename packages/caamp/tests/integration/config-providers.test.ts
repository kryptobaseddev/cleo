...
const mockProvider = (overrides: MockProviderOverrides = {}) => {
  ...
  const mcp = {
    ...
    configPathProject:
      configPathProject === undefined ? ".claude/settings.json" : configPathProject,
...
describe("integration: config and providers commands", () => {
  ...
  describe("config show", () => {
    ...
    it("shows global config with --global flag", async () => {
      ...
      expect(mocks.resolveProviderConfigPath).toHaveBeenCalledWith(
        expect.anything(),
        "global"
      );
    });
    ...
    it("exits with error when provider not found", async () => {
      ...
      await expect(
        program.parseAsync(["node", "test", "config", "show", "unknown"])
    ...
    it("exits with error when config read fails", async () => {
      ...
      await expect(
        program.parseAsync(["node", "test", "config", "show", "claude-code"])
      ).rejects.toThrow("process-exit");
  ...
  describe("config path", () => {
    ...
    it("exits with error when provider not found", async () => {
      ...
      await expect(
        program.parseAsync(["node", "test", "config", "path", "unknown"])
      ).rejects.toThrow("process-exit");
      expect(exitSpy).toHaveBeenCalledWith(1);
    });
  });
  ...
  describe("providers show", () => {
    ...
    it("exits with error when provider not found", async () => {
      ...
      await expect(
        program.parseAsync(["node", "test", "providers", "show", "unknown"])
      ).rejects.toThrow("process-exit");
      expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining("Provider not found"));
      expect(exitSpy).toHaveBeenCalledWith(1);
    });
