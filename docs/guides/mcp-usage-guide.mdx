...
<Note>
For the complete list of all 8 domains and 111 operations, see the [MCP Server Specification](../specs/MCP-SERVER-SPECIFICATION) or inspect the source of truth: `mcp-server/src/gateways/query.ts` and `mcp-server/src/gateways/mutate.ts`.
</Note>
