...
PRIMARY_SOURCE: docs/concepts/CLEO-VISION.md
IDENTITY_LOCK: TRUE
...
| **sticky** | Scratch Pad | Ephemeral capture before formal encoding. The sticky domain provides lightweight quick-capture notes (SN-xxx IDs) that exist before formal task or memory creation. |
...
CLEO also uses a non-normative workshop language to make work more intuitive to humans without changing the runtime contract:
