...
Define the authoritative scope for zero-TODO hygiene checks so report artifacts and archived development material do not create false failures.
...
- `docs/**` (narrative/spec content may legitimately reference the token `TODO`)
...
Use this command for policy-scoped zero-TODO checks:
...
- Zero-TODO claims for RB-13 are therefore evaluated against in-scope paths only, using the enforcement command above.
