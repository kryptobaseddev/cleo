# T1075 PSYCHE Memory Integration — Comprehensive Integration Plan

**Owner**: kryptobaseddev
**Filed**: 2026-04-21 (session ses_20260421173407_ac5c20)
**Parent epic**: T1075 The PSYCHE Memory Integration
...
The existing T1075 → T1076/T1081/T1082/T1083 tree with 12 atomic tasks captures **~30% of the PSYCHE integration surface**. PSYCHE's actual architecture has three additional subsystems (`deriver/`, `dreamer/`, `reconciler/`) plus a sigil identity layer plus vector reconciliation that the original plan missed. This document:

1. Audits the **existing T1075 tree** (17 tasks, structural parentage verified correct).
2. Adds **Wave 0** (pre-requisite PSYCHE source study + packages/agents/ peer_id SSoT cleanup).
...
6. Adds **Wave 8** (sigil identity layer + CANT DSL integration).
7. Cross-references **every** PSYCHE source file to the target CLEO file.
8. Captures the **SDK-first** mapping (D023) — every new capability ships as core SDK primitive first, dispatch surface second.

---

## Part 1: PSYCHE Source Inventory

PSYCHE is locally available at `/mnt/projects/honcho`. Full source tree:
...
```
composer.ts                     ← Wave 8 — compose sigil from CANT agent
...
### `packages/agents/` — ship surface (per ADR-055, v2026.4.110+)
```
package.json, README.md, .npmignore
cleo-subagent.cant              ← universal protocol base (every agent extends)
seed-agents/                    ← 4 generic role templates with {{placeholder}} variables
    orchestrator-generic.cant, dev-lead-generic.cant,
    code-worker-generic.cant, docs-worker-generic.cant, README.md
meta/                           ← meta-agents that synthesize other agents
    agent-architect.cant, README.md
harness-adapters/claude-code/   ← Claude Code harness bridge
    cleo-subagent.AGENT.md
```

**NOT shipped** — CLEO dogfood personas (`cleo-prime`, `cleo-dev`,
`cleo-db-lead`, `cleo-historian`, `cleo-rust-lead`,
`cleoos-opus-orchestrator`) live in this repo's `.cleo/cant/agents/`
only. They are owner-specific development personas, not seed templates.
...
```
T1075 (epic) The PSYCHE Memory Integration
...
└── T1083 (epic) Wave 4: Multi-Pass Context Engine
    ...
    └── T1092 End-to-End Integration Test for PSYCHE Memory Engine
```
...
#### 0.1 PSYCHE source deep-audit (information task, small)
...
- Output: `.cleo/agent-outputs/T1075-psyche-integration-plan/PSYCHE-SOURCE-NOTES.md` with a per-file summary and direct mapping to CLEO equivalents.

#### 0.2 packages/agents/ cleanup — establish peer_id SSoT (SHIPPED v2026.4.110 per ADR-055)
- Package structure finalised to **three ship surfaces**: `cleo-subagent.cant` (universal protocol base), `seed-agents/` (4 generic role templates), `meta/` (meta-agents). CLEO dogfood personas relocated to this repo's `.cleo/cant/agents/` and explicitly excluded from the package.
- `PeerIdentity` type lives at `packages/contracts/src/peer.ts`:
  ```ts
  export interface PeerIdentity {
    peerId: string;        // "orchestrator-generic", "cleo-subagent", "agent-architect"
    peerKind: "orchestrator" | "lead" | "worker" | "subagent";
    cantFile: string;      // absolute path to .cant source
    displayName: string;   // falls back to peerId
...
#### 0.3 PSYCHE↔CLEO terminology glossary (small)
- PSYCHE `workspace` ≡ CLEO project (`.cleo/` scope)
- PSYCHE `peer` ≡ CLEO CANT agent
- PSYCHE `session` ≡ CLEO session (close match)
- PSYCHE `representation` ≡ theory-of-mind model (NEW in CLEO; belongs in BRAIN as `brain_representations` table OR joins to `user_profile` with `peer_id`)
- PSYCHE `collection` ≡ CLEO `brain_observations` grouping
- PSYCHE `document` ≡ CLEO `brain_learnings`
- PSYCHE `message` ≡ CLEO session turn (not currently persisted at turn-granularity; requires new `session_messages` table for Wave 5)

**Acceptance**:
- PSYCHE-SOURCE-NOTES.md written with per-file mapping.
...
- Schema (Drizzle):
  ...
  ```
- PSYCHE reference: `/mnt/projects/honcho/src/models.py` (User + Peer models; extract `metadata`/`attributes` pattern)
...
- PSYCHE reference: `/mnt/projects/honcho/src/crud/peer.py` (getOrCreate pattern, metadata merge).
...
- PSYCHE reference: `/mnt/projects/honcho/src/crud/workspace.py` (workspace export pattern).
...
- PSYCHE reference: `/mnt/projects/honcho/src/models.py` (Peer model + relationships).
...
- PSYCHE reference: `/mnt/projects/honcho/src/dependencies.py` (request-scoped peer context injection pattern).
...
- PSYCHE reference: `/mnt/projects/honcho/src/dialectic/core.py` + `/mnt/projects/honcho/src/dialectic/prompts.py`
- Core API:
  ```ts
  ...
  ```
- Uses `packages/core/src/memory/llm-backend-resolver.ts` (CLEO's existing LLM abstraction — do NOT import psyche's `llm/` directly).
...
- PSYCHE reference: `/mnt/projects/honcho/src/deriver/deriver.py` (session state derivation)
...
- PSYCHE reference: `/mnt/projects/honcho/src/dialectic/chat.py` (context-building before LLM call).
...
- File: `packages/core/src/memory/__tests__/psyche-integration.test.ts`
- Fixtures:
...
**PSYCHE equivalent**: `/mnt/projects/honcho/src/deriver/` — background consumer/producer queue for derivation.

**Why it's missing from T1075**: The original T1087 dialectic-evaluator is a *synchronous-ish fire-and-forget*. PSYCHE's deriver is a real **queue** with durable state, retries, and dead-letter handling. For a production-grade integration, we need the queue.
...
**PSYCHE equivalent**: `/mnt/projects/honcho/src/dreamer/` — consolidation with Bayesian surprisal, topic specialists, hierarchical trees.

**Why it's missing from T1075**: We already have `brain-lifecycle.ts` with dream-cycle + `sleep-consolidation.ts` + `brain-consolidator.ts`. But PSYCHE's dreamer has THREE capabilities ours lacks:
...
- PSYCHE reference: `/mnt/projects/honcho/src/dreamer/surprisal.py`
...
- PSYCHE reference: `/mnt/projects/honcho/src/dreamer/specialists.py`
...
- PSYCHE reference: `/mnt/projects/honcho/src/dreamer/trees/`

---

### Wave 7 — Reconciler with Vector Sync (FILED — T1147, MERGES with T1139)

**PSYCHE equivalent**: `/mnt/projects/honcho/src/reconciler/` — conflict resolution + vector index sync + dead-letter cleanup.

**Already have T1139 filed** — but T1139's scope was *decision supersession only*. PSYCHE's reconciler handles three distinct concerns:
...
- PSYCHE reference: `/mnt/projects/honcho/src/reconciler/sync_vectors.py`
...
- PSYCHE reference: `/mnt/projects/honcho/src/reconciler/queue_cleanup.py`
...
- PSYCHE reference: `/mnt/projects/honcho/src/reconciler/scheduler.py`

---

### Wave 8 — Sigil Identity Layer + CANT Integration (FILED — T1148)

**PSYCHE equivalent**: `/mnt/projects/honcho/src/crud/peer_card.py` + `/mnt/projects/honcho/src/crud/representation.py`

**Naming**: CLEO calls this record a **sigil** (owner decision 2026-04-23). A sigil is the structured identity record attached to a CANT agent — carrying capability state + theory-of-mind — analogous to the external `peer_card` concept but with a name that slots into CLEO's symbolic vocabulary.

**Why it's missing from T1075**: The original plan tags memory with `peer_id` (string) but doesn't model the *sigil* — the structured identity of a CANT agent as seen by BRAIN. This matters for:
- Rendering agent "profiles" in UI (Studio)
- Storing representation (theory-of-mind for each peer)
- Cross-project portability (export sigil with agent)

#### 8.1 Sigil schema (small)
- File: `packages/core/src/store/memory-schema.ts`
- New table: `sigils`
  ```ts
...
- File: `packages/cant/src/native-loader.ts` — on agent load, upsert sigil into BRAIN.
- File: `packages/core/src/memory/sigil.ts` (new) — CRUD + `getSigilRepresentation(peerId)`.

#### 8.3 Representation updates via dialectic
- Extend T1087 `DialecticInsights` with `peerRepresentationDelta: string`
- On insight apply, merge delta into `sigils.representationJson`.
- PSYCHE reference: `/mnt/projects/honcho/src/crud/representation.py`

#### 8.4 CLI surface
- `cleo agents list` — list sigils with status
- `cleo agents show <peerId>` — full sigil + representation
- `cleo agents export <peerId>` — bundle .cant + sigil + representation for cross-project use
- `cleo agents import <path>` — inverse

#### 8.5 packages/agents/ structural cleanup (SHIPPED v2026.4.110 per ADR-055 — superseded)
Package layout is already canonical: `cleo-subagent.cant` at root, `seed-agents/` with 4 generic templates, `meta/agent-architect.cant`, `harness-adapters/claude-code/`. The historical duplicate `packages/agents/cleo-subagent/` directory was removed; AGENT.md content folded into the universal `cleo-subagent.cant`. This item is closed and kept for historical reference.
...
PSYCHE has its own `src/llm/` layer with backend registry, caching, tool-loop, structured output. CLEO has `packages/core/src/memory/llm-backend-resolver.ts` which is thinner. For dialectic + dreamer + reconciler to work at scale:

- Evaluate caching (PSYCHE has `llm/caching.py`) — CLEO doesn't cache LLM calls today; dialectic evaluations are high-volume and deterministic-ish → caching is a 10× cost reduction.
- Evaluate conversation adapter (`llm/history_adapters.py`) — CLEO re-serializes context every call; PSYCHE reuses.
...
PSYCHE has `src/telemetry/` with emission for every memory operation. We have `packages/cleo/src/system/telemetry/` (grep to verify). Cross-reference and align.

### 5.3 Vector Store Adapter (TBD — consider T1149)

PSYCHE has `src/vector_store/` with pluggable adapters. We have `brain_embeddings` (SQLite FTS5 + separate embedding column). For Wave 7 vector sync to be efficient, consider if we should adopt PSYCHE's pluggable pattern OR stick with SQLite-native (per current architecture).

### 5.4 Webhook Ingestion (TBD — consider T1150)

PSYCHE has `src/webhooks/` for inbound event ingestion. CLEO has no webhook surface today. Relevant if CLEO wants to auto-ingest from Granola/Gmail/etc (per PSYCHE examples).
...
- `.cleo/agent-outputs/T1075-psyche-integration-plan/RCASD-W0-prerequisites.md`
- `.cleo/agent-outputs/T1075-psyche-integration-plan/RCASD-W1-nexus-user-identity.md`
- `.cleo/agent-outputs/T1075-psyche-integration-plan/RCASD-W2-cant-peer-isolation.md`
- `.cleo/agent-outputs/T1075-psyche-integration-plan/RCASD-W3-dialectic-evaluator.md`
- `.cleo/agent-outputs/T1075-psyche-integration-plan/RCASD-W4-multipass-context.md`
- `.cleo/agent-outputs/T1075-psyche-integration-plan/RCASD-W5-deriver-queue.md`
- `.cleo/agent-outputs/T1075-psyche-integration-plan/RCASD-W6-dreamer-surprisal.md`
- `.cleo/agent-outputs/T1075-psyche-integration-plan/RCASD-W7-reconciler-vectorsync.md`
- `.cleo/agent-outputs/T1075-psyche-integration-plan/RCASD-W8-sigil-cant.md`
...
- PSYCHE source file references for each design decision
...
```
Wave 0 (T1144) — PREREQUISITE [SHIPPED v2026.4.110 per ADR-055: canonical package layout + PeerIdentity contract]
  ├── 0.1 PSYCHE source audit
...
Wave 7 (T1147) — Reconciler (absorbs T1139 + vector sync + DLQ)
                         │
                         ▼
Wave 8 (T1148) — Sigil + CANT representation
                         │
                         ▼
...
- T1209 — PSYCHE source audit → `PSYCHE-SOURCE-NOTES.md` ✅
- T1210 — packages/agents/ cleanup + PeerIdentity ✅ (doubles as T1144.0.2)
- T1211 — PSYCHE↔CLEO glossary → `GLOSSARY.md` ✅
...
2. ✅ PSYCHE source inventoried with all subsystem paths (done above)
3. ✅ CLEO target file inventory for memory/sessions/store/nexus/dispatch/cant/agents (done above)
4. ✅ Four missing Waves (0, 5, 6, 7) identified + fifth (Wave 8 sigil) captured
5. ⏳ New epic IDs filed for missing Waves (T1142-T1146 to be added)
6. ⏳ Handoff updated to reference this plan as the canonical PSYCHE integration master doc
...
| File / capability | Status | Task |
|---|---|---|
| `packages/core/src/memory/llm-backend-resolver.ts` | Thin | Extend with PSYCHE `llm/` pattern |
...
3. **Release R3** (PSYCHE schema + identity) — T1076 + T1081 + T1148 sigil → v2026.4.110
...
2. `cat .cleo/agent-outputs/T1075-psyche-integration-plan/PLAN.md` — read this plan (you are here).
...
*Plan authored ses_20260421173407_ac5c20 (2026-04-21, zero code). Reconciled against shipped v2026.4.115 state ses_20260422131135_5149eb (2026-04-23): Part 7b updated to reflect 3/4 substrate triad complete; Part 11 replaced obsolete worktrunk spec with shipped native implementation reference; PSYCHE Wave 0 (T1144 + T1209/T1210/T1211) marked complete; Waves 1–9 unchanged.*
