...
export function detectProjectType(projectDir) {
    ...
    const readJsonSafe = (f) => {
        try {
            return JSON.parse(readFileSync(join(projectDir, f), 'utf-8'));
        }
        catch {
            return null;
        }
    ...
    if (projectTypes.includes('node')) {
        if (exists('bun.lockb') || exists('bun.lock') || exists('bunfig.toml'))
            packageManager = 'bun';
        else if (exists('pnpm-lock.yaml') || exists('pnpm-workspace.yaml'))
            packageManager = 'pnpm';
        else if (exists('yarn.lock') || exists('.yarnrc.yml'))
            packageManager = 'yarn';
        else
            packageManager = 'npm';
    }
    ...
    else if (exists('cypress.config.ts') ||
    ...
    // Go
        ...
        testFramework = 'go';
    // Ruby
    else if (primaryType === 'ruby')
        testFramework = 'rspec';
    // Java
    else if (primaryType === 'java')
        testFramework = 'junit';
    // Bash
...
function detectBuild(primaryType, packageManager, exists, readJsonSafe) {
    const result = {};
    if (primaryType === 'node') {
        const pkg = readJsonSafe('package.json');
...
function detectTesting(projectDir, framework, packageManager, exists, readJsonSafe) {
    ...
    if (existsSync(join(projectDir, 'tests/integration')))
        dirs.integration = 'tests/integration';
    else if (existsSync(join(projectDir, 'tests/e2e')))
        dirs.integration = 'tests/e2e';
    if (Object.keys(dirs).length > 0)
        result.directories = dirs;
    return result;
}
function detectDirectories(exists) {
    const result = {};
    if (exists('src'))
        result.source = 'src';
    else if (exists('lib'))
    ...
    if (exists('docs'))
        ...
        result.docs = 'doc';
    else if (exists('documentation'))
        result.docs = 'documentation';
    return Object.keys(result).length > 0 ? result : undefined;
}
function detectConventions(projectDir, primaryType, hasTypeScript, readJsonSafe) {
    const result = {};
...
function detectFileNaming(files) {
    ...
    for (const f of sourceFiles) {
        ...
        if (/^[a-z][a-z0-9]*(-[a-z0-9]+)+$/.test(name))
            ...
            kebab++; // single word defaults to kebab
    }
...
function generateLlmHints(packageManager, testFramework, hasTypeScript, monorepo, conventions) {
    const commonPatterns = [];
    const avoidPatterns = [];
    let preferredTestStyle;
    let typeSystem;
    ...
    if (commonPatterns.length === 0 && avoidPatterns.length === 0)
        return undefined;
    return {
