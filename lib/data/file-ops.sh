...
if [[ -f "$_LIB_DIR/data/atomic-write.sh" ]]; then
    # shellcheck source=lib/data/atomic-write.sh
    source "$_LIB_DIR/data/atomic-write.sh"
else
...
if [[ -f "$_LIB_DIR/data/git-checkpoint.sh" ]]; then
    # shellcheck source=lib/data/git-checkpoint.sh
    source "$_LIB_DIR/data/git-checkpoint.sh"
fi
...
# Internal function to avoid collision with lib/data/backup.sh rotate_backups
# Arguments:
