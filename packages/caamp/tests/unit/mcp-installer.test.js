...
describe("mcp installer", () => {
    ...
    it("writes transformed config when provider transform exists", async () => {
        mocks.getTransform.mockReturnValue((_name, config) => ({ wrapped: config }));
        const result = await installMcpServer(provider({ id: "goose" }), "server", { command: "node", args: ["a.js"] });
        expect(result.success).toBe(true);
        expect(mocks.writeConfig).toHaveBeenCalledWith("/tmp/project.json", "json", "mcpServers", "server", { wrapped: { command: "node", args: ["a.js"] } });
    });
...
//# sourceMappingURL=mcp-installer.test.js.map