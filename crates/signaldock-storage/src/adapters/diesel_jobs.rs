...
#[async_trait]
impl<C> DeliveryJobRepository for DieselStore<C>
where
    ...
    C: diesel_async::pooled_connection::PoolableConnection,
{
    ...
    async fn fail_job(&self, job_id: Uuid, error: &str) -> Result<()> {
        ...
        // Read current attempt count and max to decide reschedule vs dead-letter.
        ...
        if attempts >= max_attempts {
            // Exhausted retries -- promote to dead letter.
    ...
    async fn dead_letter_job(&self, job_id: Uuid, reason: &str) -> Result<()> {
        ...
        // Read the job to get message_id and attempts for the dead letter.
        let row: DeliveryJobRow = delivery_jobs::table
            .find(job_id.to_string())
            .first(&mut conn)
            .await
