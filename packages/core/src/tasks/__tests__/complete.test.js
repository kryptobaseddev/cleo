...
describe('completeTask', () => {
    ...
    it('throws if task not found', async () => {
        await seedTasks(accessor, []);
        await expect(completeTask({ taskId: 'T999' }, env.tempDir, accessor)).rejects.toThrow('Task not found');
    });
    it('throws if task already done', async () => {
        ...
        await expect(completeTask({ taskId: 'T001' }, env.tempDir, accessor)).rejects.toThrow('already completed');
    });
    it('throws if dependencies are incomplete', async () => {
        ...
        await expect(completeTask({ taskId: 'T002' }, env.tempDir, accessor)).rejects.toThrow('incomplete dependencies');
    });
    ...
    it('blocks completion when required verification gates are incomplete', async () => {
        ...
        await expect(completeTask({ taskId: 'T001' }, env.tempDir, accessor)).rejects.toThrow('failed verification gates');
    });
