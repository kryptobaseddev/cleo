...
describe('updateTask', () => {
    ...
    it('throws if no changes specified', async () => {
        ...
        await expect(updateTask({ taskId: 'T001' }, env.tempDir, accessor)).rejects.toThrow('No changes');
    });
    it('throws if task not found', async () => {
        await seedTasks(accessor, []);
        await expect(updateTask({ taskId: 'T999', title: 'New' }, env.tempDir, accessor)).rejects.toThrow('Task not found');
    });
    ...
    it('status=done path enforces dependency checks via complete flow', async () => {
        ...
        await expect(updateTask({ taskId: 'T002', status: 'done' }, env.tempDir, accessor)).rejects.toThrow('incomplete dependencies');
    });
    it('rejects mixed status=done updates with other fields', async () => {
        ...
        await expect(updateTask({ taskId: 'T001', status: 'done', priority: 'high' }, env.tempDir, accessor)).rejects.toThrow('status=done must use complete flow');
    });
    describe('parentId (reparent via update)', () => {
        ...
        it('does not change when parentId is same as current', async () => {
            ...
            await expect(updateTask({ taskId: 'T002', parentId: 'T001' }, env.tempDir, accessor)).rejects.toThrow('No changes');
        });
