...
describe('LocalTransport', () => {
    ...
    describe('concurrent writes', () => {
        it('handles parallel pushes from multiple transports without locking', async () => {
            ...
            const sends = agents.flatMap((sender, i) => agents
                .filter((_, j) => i !== j)
        ...
        it('handles concurrent push and poll from different transports', async () => {
            ...
            const pushPromises = Array.from({ length: 100 }, (_, i) => writer.push('reader', `burst-${i}`));
...
//# sourceMappingURL=local-transport.test.js.map