...
### DHQ-100 — `pr:` evidence atom rejects all downstream-repo PRs — hardcoded required-workflow names (CI, Lockfile Check, Contracts Dep Lint) — **FIXED #1108** (`T12014` ← `T11679`)

Owner surface: CORE evidence atoms — `resolveRequiredWorkflows` in `packages/core/src/release/pr-evidence.ts` + `PR_REQUIRED_WORKFLOWS` in `packages/contracts/src/release/evidence-atoms.ts`.

Observed (gh#1104): `cleo verify T087 --gate testsPassed --evidence "pr:20"` in a downstream repo (kodomeet) with merged PRs and no `.github/workflows/` returns `E_EVIDENCE_TESTS_FAILED: Required workflows did not run for this PR: CI, Lockfile Check, Contracts Dep Lint`. The `pr:` atom resolved the required-workflow set from the env var `CLEO_PR_REQUIRED_WORKFLOWS` or fell back to the hardcoded cleocode list — names that only exist in the contract repo. A MERGED PR with `statusCheckRollup=[]` fell into `evaluateRollup`'s `missing` branch, so no task in a no-CI downstream repo could be `cleo complete`d.

Fix (`T12014` / #1108): added a project-context tier. Precedence is env var → `.cleo/project-context.json` `release.prRequiredWorkflows` (`PR_REQUIRED_WORKFLOWS_CONTEXT_KEY`) → cleocode default. An explicitly declared EMPTY array means "no required workflows", so a MERGED PR (state=MERGED) with empty/all-SUCCESS/SKIPPED checks satisfies `pr:`. cleocode's own gating is unchanged (its `.cleo/project-context.json` has no `release` key → falls through to the default). `validatePrAtom` threads the loaded project context (the sync `loadProjectContext` from `agents/variable-substitution`) into the resolver. The optional branch-protection-derivation path (`gh api .../branches/<base>/protection`) is deferred — the project-context + env tiers unblock downstream repos without a network round-trip or token-scope bump.

### DHQ-101 — `testsPassed` owner-override accepted at `verify` but rejected at `complete` (accept-then-reject) — **FIXED #1108** (`T12015` ← `T11679`)

Owner surface: CORE gate verification — the override branch of `validateGateVerify` in `packages/core/src/validation/engine-ops.ts` vs `revalidateEvidence` in `packages/core/src/tasks/evidence.ts`.

Observed (gh#1105): `CLEO_OWNER_OVERRIDE=1 cleo verify T148 --gate testsPassed --evidence "note:docs-only"` returned `success:true` (gate flips, `verification.passed=true`), then `cleo complete T148` reversed it with `E_EVIDENCE_STALE: T9245: gate 'testsPassed' rejects override-only evidence`. The verify-side guard gated on `!params.evidence` (any string present) while the complete-side `revalidateEvidence` gated on hard-atom CONTENT — so override + a soft (`note:`/`url:`) atom passed verify, was stored override-only (the parsed evidence was discarded), then failed complete. `qaPassed` survived because it is not a critical gate.

Fix (`T12015` / #1108): extracted the predicate as `isHardAtom` (single source of truth) and applied the SAME hard-atom requirement at verify time. Override-only / soft-only evidence on `implemented`/`testsPassed` is now rejected immediately at `verify` with the same T9245 wording `complete` uses, so the gate never records a green it cannot keep. A real hard atom supplied alongside an override (e.g. `commit:`) is now parsed, validated, and PRESERVED rather than silently discarded. Behavior change: override + malformed evidence now returns `E_EVIDENCE_INVALID` at verify (previously silently swallowed) — strictly more correct.

### DHQ-102 — `cleo update --blocked-by` returns `E_CLEO_NO_CHANGE` (flag dropped, not recognized as a mutation) — **FIXED #1108** (`T12016` ← `T11679`)

Owner surface: CORE/CLI task update — dispatch handler `packages/cleo/src/dispatch/domains/tasks.ts`, wrapper `taskUpdate` in `packages/core/src/tasks/update.ts`, contract `TASKS_UPDATE_INPUT_SCHEMA` in `packages/contracts/src/operations/tasks.ts`.

Observed (gh#1106): `cleo update T149 --blocked-by "Code-complete; needs cloud creds"` returns `E_CLEO_NO_CHANGE: No changes specified`. The CLI mapped the flag correctly, but `blockedBy` was dropped at three layers — the dispatch handler's `taskUpdate(...)` call object, the core `taskUpdate` EngineResult wrapper's `updates` type + forwarding object, and the `additionalProperties:false` input schema — so the value never reached the inner engine and `changes.length === 0`. Only the `clearBlockedBy` path was wired end-to-end.

Fix (`T12016` / #1108): forward `blockedBy` at all three layers (the inner `updateTask` engine already handled it). `--clear-blocked-by` was already correct and is unchanged. `--blocked-by "<text>"` now persists the free-text reason and round-trips via `cleo show`.

### DHQ-103 — `cleo lifecycle {start,complete,skip}` fails with `FOREIGN KEY constraint failed` — epic can never leave research; blocks every child's complete — **FIXED #1108** (`T12017` ← `T11679`)

Owner surface: CORE lifecycle store binding — `packages/core/src/store/tasks-schema.ts` lifecycle symbol rebind + raw-SQL sites in `lifecycle/effective-stage.ts`, `lifecycle/rollup.ts`, `tasks/compute-task-view.ts`; `orchestrate/lifecycle-ops.ts` error surfacing.

Observed (gh#1107): `cleo lifecycle start/complete/skip <epic>` fails with `FOREIGN KEY constraint failed`; `cleo orchestrate start <epic>` returns `success:true` with `initialized:true` but `cleo lifecycle show <epic>` still shows all stages `not_started` (writes zero rows). Every child `cleo complete` is then blocked by `E_LIFECYCLE_GATE_FAILED` (code 80) and owner-override does not bypass it. Root cause: the consolidation cutover (T11578/T11883) rebound the task-family drizzle symbols to the prefixed `tasks_*` tables but DEFERRED the lifecycle satellites. So `schema.lifecyclePipelines` (and the 3 raw-SQL sites) still pointed at the EMPTY bare `lifecycle_*` tables, whose `task_id` FK references the now-dead bare `tasks` table → every lifecycle insert FK-failed. `orchestrateStartup` swallowed the error and returned a false `success:true`; `getLifecycleStatus` read the empty bare table → `initialized:false` forever. It shipped uncaught because the test harness runs with `PRAGMA foreign_keys=OFF` under VITEST and the existing lifecycle integration test was orphaned (run by no config).

Fix (`T12017` / #1108): rebind the lifecycle drizzle symbols + 3 raw-SQL sites to the prefixed `tasks_lifecycle_*` tables (no data migration — the bare tables are empty; the prefixed tables already hold all 711 pipelines / 1199 stages; columns are byte-identical). `orchestrateStartup` now surfaces a genuine init failure (`E_LIFECYCLE_INIT_FAILED`) instead of a silent success-with-no-rows. New regression test `lifecycle-fk-integrity.test.ts` forces `PRAGMA foreign_keys = ON` (a regular `.test.ts` so it runs in the default CI suite) and asserts writes land in the prefixed tables, `orchestrate start` yields a readable `initialized:true` pipeline, and stages advance without an FK error.
