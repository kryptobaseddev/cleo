...
async function autoRecoverFromBackup(nativeDb, dbPath, cwd) {
    ...
    try {
        ...
        log.warn({ dbPath, backupPath: newestBackup.path, backupTasks: backupTaskCount }, `Empty database detected with ${backupTaskCount}-task backup available. ` +
...
export async function getDb(cwd) {
    ...
    _initPromise = (async () => {
        ...
        nativeDb.exec(`INSERT OR IGNORE INTO schema_meta (key, value) VALUES ('schemaVersion', '${SCHEMA_VERSION}')`);
        nativeDb.exec(`INSERT OR IGNORE INTO schema_meta (key, value) VALUES ('task_id_sequence', '{"counter":0,"lastId":"T000","checksum":"seed"}')`);
...
async function runMigrations(nativeDb, db) {
    ...
    if (tableExists(nativeDb, 'tasks') && !tableExists(nativeDb, '__drizzle_migrations')) {
      ...
      `);
            nativeDb.exec(`INSERT INTO "__drizzle_migrations" ("hash", "created_at") VALUES ('${baseline.hash}', ${baseline.folderMillis})`);
        }
    }
 ...
 */
export async function getSchemaVersion(cwd) {
