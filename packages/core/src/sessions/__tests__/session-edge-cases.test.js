...
describe('Concurrent session handling', () => {
    ...
    it('prevents same-scope epic session duplicate', async () => {
        ...
        await expect(startSession({ name: 'Duplicate', scope: 'epic:T001' }, tempDir)).rejects.toThrow('Active session already exists');
    });
    ...
    it('multiple global sessions are blocked', async () => {
        ...
        await expect(startSession({ name: 'Global 2', scope: 'global' }, tempDir)).rejects.toThrow('Active session already exists');
    });
...
describe('Session resume edge cases', () => {
    ...
    it('resume non-existent session throws', async () => {
        await expect(resumeSession('session-nonexistent', tempDir)).rejects.toThrow('Session not found');
    });
...
//# sourceMappingURL=session-edge-cases.test.js.map