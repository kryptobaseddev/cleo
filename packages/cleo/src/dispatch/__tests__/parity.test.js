...
import { buildCommanderArgs, buildCommanderOptionString, buildMcpInputSchema, camelToKebab, } from '../../dispatch/lib/param-utils.js';
import { getAllOperationSchemas, getOperationSchema } from '../../dispatch/lib/schema-utils.js';
import { getActiveDomains, getByDomain, OPERATIONS, resolve, validateRequiredParams, } from '../../dispatch/registry.js';
...
describe('Group 1: Registry completeness', () => {
    ...
    it('every required param has name, type, required, and description', () => {
        for (const op of OPERATIONS) {
            for (const param of op.params ?? []) {
                ...
                expect(['string', 'number', 'boolean', 'array'].includes(param.type), `param.type must be a valid ParamType in ${op.domain}.${op.operation}:${param.name}`).toBe(true);
    ...
    it('requiredParams array matches params.filter(required).map(name) for ops with params', () => {
        for (const op of OPERATIONS) {
            ...
            for (const name of derivedSet) {
                expect(requiredSet.has(name), `${op.domain}.${op.operation}: param "${name}" is required=true in params[] but missing from requiredParams[]`).toBe(true);
            }
        }
    });
    it('registry has the expected operation count', () => {
        ...
        expect(queryCount).toBe(120);
        expect(mutateCount).toBe(91);
        expect(OPERATIONS.length).toBe(211);
    });
    it('all operations have valid gateway values', () => {
        const validGateways = new Set(['query', 'mutate']);
        for (const op of OPERATIONS) {
            expect(validGateways.has(op.gateway), `${op.domain}.${op.operation} has invalid gateway: ${op.gateway}`).toBe(true);
        }
    });
});
// ===========================================================================
...
describe('Group 6: Per-domain operation coverage', () => {
    it('all canonical domains have at least one registered operation', () => {
        ...
        for (const domain of activeDomains) {
            expect(CANONICAL_DOMAINS.includes(domain), `Active domain "${domain}" is not in CANONICAL_DOMAINS`).toBe(true);
        }
    });
    ...
    it('every domain has operations in both query and mutate gateways, or only one', () => {
        ...
        for (const domain of activeDomains) {
            ...
            expect(queryOps.length + mutateOps.length, `Domain "${domain}" has no operations in either gateway`).toBeGreaterThan(0);
        }
    });
    it('core domains (tasks, session, admin, nexus) have both query and mutate operations', () => {
        const coreDomains = ['tasks', 'session', 'admin', 'nexus'];
