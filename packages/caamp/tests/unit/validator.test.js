...
function writeSkill(content, filename = "SKILL.md") {
...
describe("validateSkill", () => {
    ...
    it("returns error when name field is missing", async () => {
        ...
        const result = await validateSkill(filePath);
        expect(result.valid).toBe(false);
        const nameIssue = result.issues.find((i) => i.field === "name" && i.level === "error");
    ...
    it("returns error when description field is missing", async () => {
        ...
        const result = await validateSkill(filePath);
        expect(result.valid).toBe(false);
        const descIssue = result.issues.find((i) => i.field === "description" && i.level === "error");
    ...
    it("returns error when name exceeds 64 characters", async () => {
        ...
        const result = await validateSkill(filePath);
        expect(result.valid).toBe(false);
        const nameIssue = result.issues.find((i) => i.field === "name" && i.message.includes("too long"));
        expect(nameIssue).toBeDefined();
    ...
    describe("invalid name patterns", () => {
        it.each([
            ...
            const filePath = await writeSkill(`---\nname: "${name}"\ndescription: A valid description that is long enough to avoid warnings about short text.\n---\n\n# Body\n`);
            const result = await validateSkill(filePath);
            const patternIssue = result.issues.find((i) => i.field === "name" && i.message.includes("lowercase"));
    ...
    describe("reserved names", () => {
        it.each(["claude", "anthropic", "openai", "google", "microsoft", "cursor", "windsurf", "codex", "gemini", "copilot"])("returns error for reserved name '%s'", async (reserved) => {
            const filePath = await writeSkill(`---\nname: ${reserved}\ndescription: A valid description that is long enough to avoid warnings about short text.\n---\n\n# Body\n`);
            const result = await validateSkill(filePath);
            expect(result.valid).toBe(false);
            const reservedIssue = result.issues.find((i) => i.field === "name" && i.message.includes("reserved"));
    ...
    it("returns error when name contains HTML tags", async () => {
        ...
        const result = await validateSkill(filePath);
        expect(result.valid).toBe(false);
        const htmlIssue = result.issues.find((i) => i.field === "name" && i.message.includes("XML/HTML"));
    ...
    it("returns error when description exceeds 1024 characters", async () => {
        ...
        const result = await validateSkill(filePath);
        expect(result.valid).toBe(false);
        const descIssue = result.issues.find((i) => i.field === "description" && i.message.includes("too long"));
        expect(descIssue).toBeDefined();
    ...
    it("returns warning when description is shorter than 50 characters", async () => {
        ...
        const warnIssue = result.issues.find((i) => i.field === "description" && i.level === "warning");
    ...
    it("returns error when description contains HTML tags", async () => {
        ...
        const result = await validateSkill(filePath);
        expect(result.valid).toBe(false);
        const htmlIssue = result.issues.find((i) => i.field === "description" && i.message.includes("XML/HTML"));
        expect(htmlIssue).toBeDefined();
        expect(htmlIssue.level).toBe("error");
    });
    ...
    it("returns warning when body exceeds 500 lines", async () => {
        ...
        // Warnings do not cause valid:false
        expect(result.valid).toBe(true);
        const bodyIssue = result.issues.find((i) => i.field === "body" && i.level === "warning" && i.message.includes("long"));
    ...
    it("returns warning when body is empty (frontmatter only)", async () => {
        ...
        const result = await validateSkill(filePath);
        expect(result.valid).toBe(true);
        const emptyIssue = result.issues.find((i) => i.field === "body" && i.level === "warning" && i.message.includes("Empty"));
    ...
    describe("edge cases", () => {
        it("accepts a single-character name", async () => {
            const filePath = await writeSkill(`---\nname: x\ndescription: A valid description that is long enough to avoid warnings about short text.\n---\n\n# Body\n`);
            const result = await validateSkill(filePath);
            const nameErrors = result.issues.filter((i) => i.field === "name" && i.level === "error");
        ...
        it("accepts a name at exactly 64 characters", async () => {
            ...
            const filePath = await writeSkill(`---\nname: ${name64}\ndescription: A valid description that is long enough to avoid warnings about short text.\n---\n\n# Body\n`);
            const result = await validateSkill(filePath);
            const lengthIssue = result.issues.find((i) => i.field === "name" && i.message.includes("too long"));
        ...
        it("accepts a description at exactly 1024 characters", async () => {
            ...
            const result = await validateSkill(filePath);
            const lengthIssue = result.issues.find((i) => i.field === "description" && i.message.includes("too long"));
            expect(lengthIssue).toBeUndefined();
        });
        ...
        it("returns body at exactly 500 lines without warning", async () => {
            ...
            const filePath = await writeSkill(`---\nname: my-skill\ndescription: A valid description that is long enough to avoid warnings about short text.\n---\n\n${bodyLines}\n`);
            const result = await validateSkill(filePath);
            const bodyWarn = result.issues.find((i) => i.field === "body" && i.message.includes("long"));
