...
use crate::adapters::diesel_helpers::{diesel_err, now_ts, pool_err};
...
#[async_trait]
impl<C> AgentConnectionRepository for DieselStore<C>
where
    C: diesel_async::AsyncConnection<Backend = diesel::sqlite::Sqlite> + 'static,
    C: diesel_async::pooled_connection::PoolableConnection,
{
    ...
    async fn reap_stale(&self, before_ts: i64) -> Result<u64> {
        ...
        let deleted = diesel::delete(
            agent_connections::table.filter(agent_connections::last_heartbeat.lt(before_ts)),
        )
        .execute(&mut conn)
        .await
        .map_err(diesel_err)?;
