/**
 * Pipeline Manifest Compatibility Layer
 *
 * Async wrappers around manifest (JSONL) operations that return EngineResult<T>
 * format for consumption by the dispatch layer. These functions were extracted
 * from engine-compat.ts during the memory domain cutover (T5241) to separate
 * manifest/pipeline concerns from brain.db cognitive memory.
 *
 * @task T5241
 * @epic T5149
 */
...
// Re-export types for consumers that previously imported them from engine-compat
...
export function pipelineManifestShow(
  researchId: string,
  projectRoot?: string,
): EngineResult {
  if (!researchId) {
...
export function pipelineManifestList(
...
/** pipeline.manifest.find - Find manifest entries by text */
export function pipelineManifestFind(
  query: string,
...
export function pipelineManifestPending(
  epicId?: string,
  projectRoot?: string,
): EngineResult {
  const entries = readManifestEntries(projectRoot);
...
export function pipelineManifestStats(
  epicId?: string,
  projectRoot?: string,
): EngineResult {
  const entries = readManifestEntries(projectRoot);
...
export function pipelineManifestRead(
...
/** pipeline.manifest.append - Append entry to MANIFEST.jsonl */
export function pipelineManifestAppend(
...
/** pipeline.manifest.archive - Archive old manifest entries */
export function pipelineManifestArchive(
  beforeDate: string,
  projectRoot?: string,
): EngineResult {
  if (!beforeDate) {
...
/** pipeline.manifest.compact - Compact MANIFEST.jsonl by removing duplicate/stale entries */
export function pipelineManifestCompact(
  projectRoot?: string,
): EngineResult {
  const manifestPath = getManifestPath(projectRoot);
...
export function pipelineManifestValidate(
  taskId: string,
  projectRoot?: string,
): EngineResult {
  if (!taskId) {
...
export function pipelineManifestContradictions(
  projectRoot?: string,
  params?: { topic?: string },
...
export function pipelineManifestSuperseded(
  projectRoot?: string,
  params?: { topic?: string },
...
export function pipelineManifestLink(
  taskId: string,
  researchId: string,
  notes?: string,
  projectRoot?: string,
