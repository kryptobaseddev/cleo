...
describe('deleteTask', () => {
    ...
    it('throws for nonexistent task', async () => {
        await seedTasks(accessor, []);
        await expect(deleteTask({ taskId: 'T999' }, env.tempDir, accessor)).rejects.toThrow('Task not found');
    });
    it('throws when task has children without cascade/force', async () => {
        ...
        await expect(deleteTask({ taskId: 'T001' }, env.tempDir, accessor)).rejects.toThrow(/children/i);
    });
    ...
    it('throws when task has dependents without force', async () => {
        ...
        await expect(deleteTask({ taskId: 'T001' }, env.tempDir, accessor)).rejects.toThrow(/dependency/i);
    });
