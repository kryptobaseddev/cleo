...
import { engineError, engineSuccess } from '../../dispatch/engines/_error.js';
...
describe('Integration: EngineResult shape contracts', () => {
    it('engineSuccess wraps any data type and preserves shape', () => {
        ...
        const nullResult = engineSuccess(null);
...
describe('Integration: Cross-domain registry consistency', () => {
    ...
    it('operation descriptions follow naming convention', () => {
        for (const op of OPERATIONS) {
            ...
            expect(op.description.length, `${op.domain}.${op.operation} has empty description`).toBeGreaterThan(0);
        }
    });
    it('all operations have consistent tier values', () => {
        const validTiers = new Set([0, 1, 2]);
        for (const op of OPERATIONS) {
            expect(validTiers.has(op.tier), `${op.domain}.${op.operation} has invalid tier: ${op.tier}`).toBe(true);
        }
    });
    it('idempotent flag is boolean for all operations', () => {
        for (const op of OPERATIONS) {
            expect(typeof op.idempotent, `${op.domain}.${op.operation} idempotent is not boolean`).toBe('boolean');
        }
    });
...
//# sourceMappingURL=parity.integration.test.js.map