...
import { checkCleoStructure, checkConfig, checkGitignore, checkSqliteDb, createDefaultConfig, ensureCleoGitRepo, ensureCleoStructure, ensureConfig, ensureGitignore, ensureProjectContext, fileExists, generateProjectHash, getCleoVersion, getGitignoreContent, getPackageRoot, REQUIRED_CLEO_SUBDIRS, removeCleoFromRootGitignore, stripCLEOBlocks, } from '../scaffold.js';
// ── Helpers ──────────────────────────────────────────────────────────
function makeTmpDir() {
...
describe('ensureProjectContext', () => {
    ...
    it('skips when fresh (not stale)', async () => {
        const contextPath = join(tmpDir, '.cleo', 'project-context.json');
        writeFileSync(contextPath, JSON.stringify({ type: 'node', detectedAt: new Date().toISOString() }));
        const result = await ensureProjectContext(tmpDir);
        expect(result.action).toBe('skipped');
    });
