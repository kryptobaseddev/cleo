---
title: "CLEO Plugin"
description: "Claude Code companion plugin with auto-discovered skills, slash commands, autonomous agents, and session lifecycle hooks."
icon: "file-code"
---
...
│   ├── ct-start.md                    # Active task management
...
│       └── check-focus.sh             # Validate active task
│
...
description: >
  ...
  "end session",    "start task", "complete task", "list tasks", "show tasks",
...
ct start <id>                    # Set active task
ct current                       # Show active task
ct stop                          # Clear active task
```
...
ct analyze --auto-focus          # Auto-activate top task
...
3. `ct start <id>` - Activate one task
4. Work on active task
...
### For `end`:
1. Check if tasks are still in progress: `ct current`
2. If task active, ask user if they want to complete or stop work
...
#### 3.4 Active Task Commands

**File:** `commands/ct-start.md`

```markdown
---
name: ct-start
description: Start working on a task, show active task, or stop work
argument-hint: <id>
allowed-tools: ["Bash"]
---

Manage which task is currently being worked on. Only one task can be active at a time.

## Usage

```
/ct-start T042     # Start working on task T042
/ct-current        # Show active task
/ct-stop           # Stop working on active task
```
...
3. Execute: `ct start <id>`
4. Show task details after starting

### For `/ct-current`:
1. Execute: `ct current`
2. Display active task or "No active task"

### For `/ct-stop`:
1. Execute: `ct stop`
2. Confirm active task cleared

## Task Discipline

- Work on ONE task at a time
- Complete or stop task before switching
- Use session notes for session-level progress
...
/ct-analyze --auto-focus # Analyze and auto-activate top task
```
...
3. Present analysis summary:
   ...
   - Phase progress
4. If `--auto-focus` used, confirm which task was activated
...
- Always check active task state before recommending end
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "prompt",
            "prompt": "Before stopping, verify CLEO session state. Check: Is there an active CLEO session? Is a task currently active? Should the session be ended or suspended? If work is incomplete, recommend proper session handling. Return 'approve' to stop or 'block' with session cleanup recommendations.",
            "timeout": 15
          }
        ]
      }
...
#### 5.3 Check Active Task Hook Script

**File:** `hooks/scripts/check-focus.sh`

```bash
#!/usr/bin/env bash
# CLEO UserPromptSubmit hook - remind about active task if working on tasks
set -euo pipefail
...
if echo "$user_prompt" | grep -qiE '(task|todo|work on|implement|fix|add|create|build)'; then
    ...
    focus_status=$(ct current --format json 2>/dev/null || echo '{"focused": false}')
    has_focus=$(echo "$focus_status" | jq -r '.focused // false')

    if [[ "$has_focus" == "false" ]]; then
        # No active task, suggest starting one
        echo "{\"systemMessage\": \"Note: No CLEO task is currently active. Consider using 'ct start <id>' to track this work.\"}"
    fi
fi
...
- [ ] `/ct-start T001` sets active task
...
| `commands/ct-start.md` | Command | ~350 words |
