 ...
 */
export async function atomicDatabaseMigration(dbPath, tempPath, validateFn) {
 ...
 */
export async function restoreDatabaseFromBackup(dbPath, backupPath) {
    try {
 ...
 */
export async function cleanupMigrationArtifacts(backupPath) {
    try {
        if (existsSync(backupPath)) {
 ...
 */
export async function validateSqliteDatabase(dbPath) {
    try {
        const { createRequire } = await import('node:module');
        const _req = createRequire(import.meta.url);
        const { DatabaseSync } = _req('node:sqlite');
