...
| `LAFS_VIOLATION`               | `ExitCode.LAFS_VIOLATION`               | Yes      | E\_LAFS\_VIOLATION — CLI output did not conform to the LAFS envelope schema. Emitted by the CLI renderer middleware when `zod.parse()` on the emitted envelope fails. This is an INTERNAL failure: CLEO itself produced a malformed envelope.   Phase 6 — LAFS formalization + schema consolidation |
