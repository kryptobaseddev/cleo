...
describe('validateParent', () => {
    ...
    it('throws if parent is subtask', () => {
        const tasks = [{ id: 'T001', type: 'subtask' }];
...
describe('findRecentDuplicate', () => {
    ...
    it('detects recent duplicate by title', () => {
        ...
        const tasks = [{ id: 'T001', title: 'Test task', createdAt: now }];
        expect(findRecentDuplicate('Test task', undefined, tasks))?.toEqual(expect.objectContaining({ id: 'T001' }));
    });
    it('ignores old tasks outside window', () => {
        const old = new Date(Date.now() - 120000).toISOString();
        const tasks = [{ id: 'T001', title: 'Test task', createdAt: old }];
