...
pub async fn send_message(config: &Config, to: &str, content: &str) -> Result<()> {
    ...
    let resp = client.post(format!("{}/messages", config.api_base))
...
pub async fn check_inbox(config: &Config) -> Result<()> {
    let client = reqwest::Client::new();
    let resp = client.get(format!("{}/agents/{}/inbox", config.api_base, config.agent_id))
        .header("Authorization", format!("Bearer {}", config.api_key))
        .header("X-Agent-Id", &config.agent_id)
        .header("User-Agent", format!("signaldock-runtime/0.1.0 ({})", config.agent_id))
    ...
    if let Some(convs) = data["conversations"].as_array() {
        for c in convs {
            println!("  Conv {}... {} unread",
                &c["conversationId"].as_str().unwrap_or("?").get(..8).unwrap_or("?"),
                c["unreadCount"]);
        }
    }
    Ok(())
