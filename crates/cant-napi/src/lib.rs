...
// NOTE: cant-router bindings removed (E8 T11432). The cant-router crate is
// preserved in the workspace for reference but is no longer linked into the
// cant-napi binary. Zero live TypeScript consumers existed; the
// `cantRouterExtractFeatures` / `cantRouterClassify` / `cantRouterRoute` /
// `cantRouterDowngrade` exports are intentionally not part of the public napi
// surface. Filed for formal retire/relocation in T11432.
