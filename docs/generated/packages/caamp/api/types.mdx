...
| `configFormat`          | `string`                            | Yes      | Config file format identifier (e.g. `"json"`, `"jsonc"`, `"yaml"`, `"toml"`).    |
...
| `supportedTransports`   | `string[]`                          | Yes      | MCP transport protocol identifiers this provider supports.                       |
| `supportsHeaders`       | `boolean`                           | Yes      | Whether the provider supports custom HTTP headers for remote MCP servers.        |
| `priority`              | `string`                            | Yes      | Priority tier identifier (`"high"`, `"medium"`, or `"low"`).                     |
| `status`                | `string`                            | Yes      | Lifecycle status identifier (`"active"`, `"beta"`, `"deprecated"`, `"planned"`). |
| `agentSkillsCompatible` | `boolean`                           | Yes      | Whether the provider is compatible with the Agent Skills standard.               |
| `capabilities`          | `RegistryCapabilities \| undefined` | No       | Optional provider capabilities for skills, hooks, and spawn.                     |
...
| `hookConfigPath` | `string \| null` | Yes      | Path to the hook configuration file, or `null` if not applicable.    |
| `hookFormat`     | `string \| null` | Yes      | Format of the hook config file (e.g. `"json"`, `"yaml"`), or `null`. |
...
| `spawnMechanism`            | `string \| null` | Yes      | Mechanism used for spawning, or `null` if spawning is unsupported. |
...
| `hookConfigPath` | `string \| null`                                     | Yes      | Resolved path to hook configuration file, or `null`. |
| `hookFormat`     | `"json" \| "yaml" \| "toml" \| "javascript" \| null` | Yes      | Format of the hook config file.                      |
...
| `instructFile`          | `string`               | Yes      | Instruction file name (e.g. `"CLAUDE.md"`, `"AGENTS.md"`).                       |
| `configKey`             | `string`               | Yes      | Dot-notation key path for MCP server config (e.g. `"mcpServers"`).               |
| `configFormat`          | `ConfigFormat`         | Yes      | Config file format used by this provider.                                        |
...
| `agentSkillsCompatible` | `boolean`              | Yes      | Whether the provider is compatible with the Agent Skills standard.               |
| `capabilities`          | `ProviderCapabilities` | Yes      | Provider capabilities for skills, hooks, and spawn. Always populated at runtime. |
