...
describe('rankBlockedTask', () => {
    ...
    it('downstream impact: more dependents yields higher score', () => {
        ...
        const noDownstream = [];
        expect(rankBlockedTask(blocker, manyDownstream, null)).toBeGreaterThan(rankBlockedTask(blocker, noDownstream, null));
    });
    it('downstream dependents with done status are not counted', () => {
        ...
        expect(rankBlockedTask(blocker, doneDependent, null)).toBeLessThan(rankBlockedTask(blocker, pendingDependent, null));
    });
    it('older task scores higher due to age weight (capped at 30)', () => {
        ...
        expect(rankBlockedTask(old, allTasks, null)).toBeGreaterThan(rankBlockedTask(newer, allTasks, null));
    });
    it('age is capped at 30 days', () => {
        ...
        expect(rankBlockedTask(veryOld, allTasks, null)).toBe(rankBlockedTask(thirtyDaysOld, allTasks, null));
    });
    it('focus proximity: sibling of focused task gets +15 boost', () => {
        ...
        expect(rankBlockedTask(sibling, allTasks, focus)).toBeGreaterThan(rankBlockedTask(unrelated, allTasks, focus));
        ...
        const diff = rankBlockedTask(siblingBase, allTasks, focus) -
    ...
    it('focus proximity: child of focused task gets +15 boost', () => {
        ...
        const diff = rankBlockedTask(child, allTasks, focus) - rankBlockedTask(unrelated, allTasks, focus);
        expect(diff).toBe(15);
    });
    it('focus proximity: parent of focused task gets +15 boost', () => {
        ...
        const diff = rankBlockedTask(parent, allTasks, focus) - rankBlockedTask(unrelated, allTasks, focus);
        expect(diff).toBe(15);
    });
    it('urgent label boost: +8 per urgent label', () => {
        ...
        const diff = rankBlockedTask(withUrgentLabel, allTasks, null) -
    ...
    it('urgent label boost: all three urgent labels accumulate (+24)', () => {
        ...
        const diff = rankBlockedTask(withAll, allTasks, null) - rankBlockedTask(withNone, allTasks, null);
        expect(diff).toBe(24);
    });
    it('urgent label matching is case-insensitive', () => {
        ...
        expect(rankBlockedTask(withUpper, allTasks, null)).toBe(rankBlockedTask(withLower, allTasks, null));
    });
    it('staleness penalty: updated < 3 days ago subtracts 10', () => {
        ...
        const diff = rankBlockedTask(neverUpdated, allTasks, null) -
    ...
    it('staleness penalty does not apply when updated >= 3 days ago', () => {
        ...
        expect(rankBlockedTask(oldUpdate, allTasks, null)).toBe(rankBlockedTask(neverUpdated, allTasks, null));
    });
});
// ---------------------------------------------------------------------------
// getDashboard
...
describe('getDashboard', () => {
    ...
    it('custom blockedTasksLimit is respected', async () => {
        const tasks = Array.from({ length: 15 }, (_, i) => makeTask({ id: `T${String(i).padStart(3, '0')}`, title: `Task ${i}`, status: 'blocked' }));
    ...
    it('count reflects all blocked tasks even when limit is smaller', async () => {
        const tasks = Array.from({ length: 20 }, (_, i) => makeTask({ id: `T${String(i).padStart(3, '0')}`, title: `Task ${i}`, status: 'blocked' }));
        const accessor = makeAccessor(tasks);
